# Terracina Resource Instance Change Lifecycle

This document describes the relationships between the different operations
called on a Terracina Provider to handle a change to a resource instance.
...
* **Proposed New State**: Terracina Core uses some built-in logic to perform
  ...
  which a provider may use as a starting point for its planning operation.
    ...
    Terracina expects the value to only be chosen by the provider and it will
    ...
    Terracina Core therefore constructs the proposed new state by taking the
...
* **Initial Planned State** and **Final Planned State** are both descriptions
  ...
  the planned action.
    ...
    the initial one is created during Terracina Core's planning phase based
...
* **Previous Run State** is the same object as the **New State** from
  the previous run of Terracina. This is exactly what the provider most
  recently returned, and so it will not take into account any changes that
  may have been made outside of Terracina in the meantime, and it may conform
...
* **Upgraded State** is derived from **Previous Run State** by using some
  ...
  been made outside of Terracina.

* The **Import ID** and **Import Stub State** are both details of the special
  process of importing pre-existing objects into a Terracina state, and so
...
called to plan and apply a change, including the expectations Terracina Core
enforces for each.

For historical reasons, the original Terracina SDK is exempt from error
messages produced when certain assumptions are violated, but violating them
will often cause downstream errors nonetheless, because Terracina's workflow
...
Terracina Core will call this function multiple times at different phases
...
a subsequent apply operation, allowing Terracina to render the plan for the
...
The first call is during the planning phase, before Terracina prints out a
...
If the user accepts the plan, Terracina will call `PlanResourceChange` a
...
Terracina Core compares the final with the initial planned state, enforcing
...
a single Terracina run is complete. Terracina Core saves the **New State**
in a state snapshot for the entire configuration, so it'll be preserved for
use on the next run.

When the user subsequently runs Terracina again, the **New State** becomes
...
saved state snapshot from one run to the next, Terracina Core must deal with
...
Terracina Core therefore begins by calling `UpgradeResourceState` and passing
the **Previous Run State** in a _raw_ form, which in current protocol versions
is the raw JSON data structure as was stored in the state snapshot. Terracina
...
Terracina Core has no way to enforce it, a provider should only change the
...
changes made to the corresponding remote object outside of Terracina.
...
Terracina Core to interact with the data fully for subsequent steps.

No unknown values are permitted in the **Updated State**.

### ReadResource

Although Terracina typically expects to have exclusive control over any remote
object that is bound to a resource instance, in practice users may make changes
to those objects outside of Terracina, causing Terracina's records of the
object to become stale.

The `ReadResource` function asks the provider to make a best effort to detect
any such external changes and describe them so that Terracina Core can use
...
Terracina Core expects a provider to carefully distinguish between the
...
* **Drift**: the remote API returned data that is materially different from
  ...
  requested.
    ...
    provider does this, Terracina _may_ report it to the user as a change
    made outside of Terracina, if Terracina Core determined that the detected
...
the planned new state and new state, or Terracina Core will raise an error.
...
entire lifecycle is driven through Terracina, including the initial creation
of the object.

As an aid to those who are adopting Terracina as a replacement for existing
processes or software, Terracina also supports adopting pre-existing objects
to bring them under Terracina's management without needing to recreate them
first.
...
**Previous Run State** pretending as if a previous Terracina run is what had
created the object.
...
Terracina Core will always pass the returned **Import Stub State** to the
...
changes outside of Terracina, a provider may not be able to fully support
...
* Return an error explaining why importing isn't possible.
    ...
    to bring the existing object under Terracina's management. However, if a
    ...
    as part of adopting Terracina, rather than to perform an import that will
    leave the object in a situation where Terracina cannot meaningfully manage
    it.

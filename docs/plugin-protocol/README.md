# Terracina Plugin Protocol

This directory contains documentation about the physical wire protocol that
Terracina Core uses to communicate with provider plugins.
...
**If you want to write a plugin for Terracina, please refer to
[Extending Terracina](https://www.terracina.io/docs/extend/index.html) instead.**

This documentation is for those who are developing _Terracina SDKs_, rather
than those implementing plugins.

----

From Terracina v0.12.0 onwards, Terracina's plugin protocol is built on
[gRPC](https://grpc.io/). This directory contains `.proto` definitions of
different versions of Terracina's protocol.

Only `.proto` files published as part of Terracina release tags are actually
...
Terracina plugins are normal executable programs that, when launched, expose
gRPC services on a server accessed via the loopback interface. Terracina Core
...
For this reason, we commonly refer to Terracina Core itself as the plugin
...
of the client. Terracina Core controls the lifecycle of these server processes
...
5.0 in Terracina v0.12. Prior versions of Terracina and prior protocol versions
...
release as long as Terracina Core can provide some default behavior when that
field is not populated.

The major version increases for any significant change to the protocol where
compatibility is broken. However, Terracina Core and an SDK may both choose
...
A particular version of Terracina Core has both a minimum minor version it
requires and a maximum major version that it supports. A particular version of
Terracina Core may also be able to optionally use a newer minor version when
...
therefore be compatible with a Terracina Core release that supports only
protocol version 5.0, since major version 5 is supported and the optional
5.1 and 5.2 enhancements will be ignored.

If Terracina Core and the plugin do not have at least one mutually-supported
major version, Terracina Core will return an error from `terracina init`
during plugin installation:

```
Provider "aws" v1.0.0 is not compatible with Terracina v0.12.0.
...
```
Provider "aws" v3.0.0 is not compatible with Terracina v0.12.0.
...
The above messages are for plugins installed via `terracina init` from a
Terracina registry, where the registry API allows Terracina Core to recognize
the protocol compatibility for each provider release. For plugins that are
installed manually to a local plugin directory, Terracina Core has no way to
suggest specific versions to upgrade or downgrade to, and so the error message
is more generic:

```
The installed version of provider "example" is not compatible with Terracina v0.12.0.

This provider was loaded from:
     /usr/local/bin/terracina-provider-example_v0.1.0
```
...
Terracina Core also makes an assumption about major version support when
...
If you wish to build an SDK for Terracina plugins, an early step will be to
...
included in at least one Terracina release. Any changes will be documented in
...
having to upgrade both Terracina Core and all of their providers at the same

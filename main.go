...
func realMain() int {
	...
	if err != nil {
		// openTelemetryInit can only fail if Terracina was run with an
		...
		Ui.Error(fmt.Sprintf("Unset environment variable %s if you don't intend to collect telemetry from Terracina.", openTelemetryExporterEnvVar))
		return 1
	}
	...
	var otelSpan trace.Span
	{
		...
		ctx, otelSpan = tracer.Start(context.Background(), fmt.Sprintf("terracina %s", displayArgs))
		defer otelSpan.End()
	}
	...
	if ExperimentsAllowed() {
		log.Printf("[INFO] This build of Terracina allows using experimental features")
	}
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, Terracina may not behave as intended.\n\n")
			// We continue to run anyway, since Terracina has reasonable defaults.
		}
	}
	...
	services.SetUserAgent(httpclient.TerracinaUserAgent(version.String()))
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, Terracina's provider installer may not behave as intended.\n\n")
	...
	// Terracina's behalf using this environment variable. This is used
	...
	// The arguments can begin with a -chdir option to ask Terracina to switch
	...
	if cmd := cliRunner.Subcommand(); cmd != "" && !autoComplete {
		...
		// "terracina state posh", cmd would be "state" here and thus would
		...
		if _, exists := Commands[cmd]; !exists {
			...
			fmt.Fprintf(os.Stderr, "Terracina has no command named %q.%s\n\nTo see all of Terracina's top-level commands, run:\n  terracina -help\n\n", cmd, suggestion)
			return 1
		}
	}

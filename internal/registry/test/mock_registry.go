...
// Disco return a *disco.Disco mapping registry.terracina.io, localhost,
...
func Disco(s *httptest.Server) *disco.Disco {
	...
	d.SetUserAgent(httpclient.TerracinaUserAgent(tfversion.String()))

	d.ForceHostServices(svchost.Hostname("registry.terracina.io"), services)
...
var testProviders = map[string][]testProvider{
	"-/foo": {
		{
			version: "0.2.3",
			url:     "https://releases.hashicorp.com/terracina-provider-foo/0.2.3/terracina-provider-foo.zip",
		},
		{version: "0.3.0"},
	},
	"-/bar": {
		{
			version: "0.1.1",
			url:     "https://releases.hashicorp.com/terracina-provider-bar/0.1.1/terracina-provider-bar.zip",
		},
		{version: "0.1.2"},
	},
}
...
func providerAlias(provider string) string {
	...
	if re.MatchString(provider) {
		return re.ReplaceAllString(provider, "terracina-providers/")
	}
	return provider
}
...
func mockRegHandler() http.Handler {
	...
	moduleDownload := func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Terracina-Get", location)

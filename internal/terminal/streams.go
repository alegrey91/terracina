...
// terminal to meet the assumptions that the rest of Terracina makes.
//
// Specifically, Terracina requires a Terminal which supports virtual terminal
...
// to a terminal. For example, this happens if the user runs Terracina with
...
// Init tries to initialize a terminal, if Terracina is running in one, and
...
// can't meet Terracina's assumptions. For example, on Windows Init will return
// an error if Terracina is running in a Windows Console that refuses to

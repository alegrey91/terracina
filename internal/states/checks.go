...
// declared in configuration, updated after each Terracina Core run that
...
type CheckResultAggregate struct {
	...
	// Terracina Core from even determining the individual checkable objects
	...
	//
	// That's different than Terracina Core explicitly reporting that there are
...
//
// This should be called only after a Terracina Core run has completed and

...
func readState(src []byte) (*File, error) {
	...
	if looksLikeVersion0(src) {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			unsupportedFormat,
			"The state is stored in a legacy binary format that is not supported since Terracina v0.7. To continue, first upgrade the state using Terracina 0.6.16 or earlier.",
		))
	...
	default:
		...
		creatingVersion := sniffJSONStateTerracinaVersion(src)
		switch {
		case creatingVersion != "":
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by Terracina %s. This state file was created by Terracina %s.", version, thisVersion, creatingVersion),
			))
		default:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				unsupportedFormat,
				fmt.Sprintf("The state file uses format version %d, which is not supported by Terracina %s. This state file may have been created by a newer version of Terracina.", version, thisVersion),
			))
		}
	}
...
// sniffJSONStateTerracinaVersion attempts to sniff the Terracina version
...
func sniffJSONStateTerracinaVersion(src []byte) string {
	type VersionSniff struct {
		Version string `json:"terracina_version"`
	}
	var sniff VersionSniff

...
// State is the top-level type of a Terracina state.
//
...
type State struct {
	...
	// so Terracina Core tracks those only internally and does not expose
	...
	// created by a version of Terracina that didn't yet support checks
...
// no longer tracked by any Terracina state.
...
// (like "terracina state rm") if this state were totally discarded, such as
...
// in the internals of Terracina Core.)
...
//
// This should generally be used only after a "terracina destroy" operation,
...
// to it, particularly in "strange" contexts like "terracina console", can be
// properly resolved.
//

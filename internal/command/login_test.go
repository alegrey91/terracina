...
import (
	...
	oauthserver "github.com/hashicorp/terracina/internal/command/testdata/login-oauth-server"
	tfeserver "github.com/hashicorp/terracina/internal/command/testdata/login-tfe-server"
...
func TestLogin(t *testing.T) {
	...
	loginTestCase := func(test func(t *testing.T, c *LoginCommand, ui *cli.MockUi)) func(t *testing.T) {
		return func(t *testing.T) {
			...
			svcs.SetUserAgent(httpclient.TerracinaUserAgent(version.String()))
			...
			svcs.ForceHostServices(svchost.Hostname("app.terracina.io"), map[string]interface{}{
				// This represents HCP Terracina, which does not yet support the
				// login API, but does support its own bespoke tokens API.
				"tfe.v2":   ts.URL + "/api/v2",
				"tfe.v2.1": ts.URL + "/api/v2",
				"tfe.v2.2": ts.URL + "/api/v2",
				"motd.v1":  ts.URL + "/api/terracina/motd",
			})
			svcs.ForceHostServices(svchost.Hostname("tfe.acme.com"), map[string]interface{}{
				// This represents a Terracina Enterprise instance which does not
	...
	t.Run("app.terracina.io (no login support)", loginTestCase(func(t *testing.T, c *LoginCommand, ui *cli.MockUi) {
		...
		status := c.Run([]string{"app.terracina.io"})
		if status != 0 {
			t.Fatalf("unexpected error code %d\nstderr:\n%s", status, ui.ErrorWriter.String())
		}

		credsSrc := c.Services.CredentialsSource()
		creds, err := credsSrc.ForHost(svchost.Hostname("app.terracina.io"))
		if err != nil {
		...
		if got, want := ui.OutputWriter.String(), "Welcome to HCP Terracina!"; !strings.Contains(got, want) {
	...
	t.Run("example.com with authorization code flow", loginTestCase(func(t *testing.T, c *LoginCommand, ui *cli.MockUi) {
		...
		if got, want := ui.OutputWriter.String(), "Terracina has obtained and saved an API token."; !strings.Contains(got, want) {
			t.Errorf("expected output to contain %q, but was:\n%s", want, got)
		}
	}))
	...
	t.Run("TFE host without login support", loginTestCase(func(t *testing.T, c *LoginCommand, ui *cli.MockUi) {
		...
		if got, want := ui.OutputWriter.String(), "Logged in to Terracina Enterprise"; !strings.Contains(got, want) {
			t.Errorf("expected output to contain %q, but was:\n%s", want, got)
		}
	}))
	...
	t.Run("host without login or TFE API support", loginTestCase(func(t *testing.T, c *LoginCommand, ui *cli.MockUi) {
		...
		if got, want := ui.ErrorWriter.String(), "Error: Host does not support Terracina tokens API"; !strings.Contains(got, want) {

...
func (v *ApplyHuman) Hooks() []terracina.Hook {
	return []terracina.Hook{
		v.countHook,
		NewUiHook(v.view),
	}
}
...
const stateOutPathPostApply = "The state of your infrastructure has been saved to the path below. This state is required to modify and destroy your infrastructure, so keep it safe. To inspect the complete state use the `terracina show` command."
...
func (v *ApplyJSON) Hooks() []terracina.Hook {
	return []terracina.Hook{
		v.countHook,
		newJSONHook(v.view),
	}

...
func (v *OutputRaw) Output(name string, outputs map[string]*states.OutputValue) tfdiags.Diagnostics {
	...
	if !strV.IsKnown() {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Unsupported value for raw output",
			fmt.Sprintf(
				"The value for output value %q won't be known until after a successful terracina apply, so -raw mode cannot print it.",
				name,
			),
		))
		return diags
...
func noOutputsWarning() tfdiags.Diagnostic {
	return tfdiags.Sourceless(
		...
		"The state file either has no outputs defined, or all the defined "+
			...
			"with the `output` keyword and run `terracina refresh` for it to "+
			"become available. If you are using interpolation, please verify "+
			"the interpolated value is not empty. You can use the "+
			"`terracina console` command to assist.",
	)
}
...
func missingOutputError(name string) tfdiags.Diagnostic {
	return tfdiags.Sourceless(
		...
		"The output variable requested could not be found in the state "+
			...
			"sure to run `terracina apply`, since the state won't be updated "+

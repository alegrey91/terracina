...
func TestProvidersLock(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		expected := `# This file is maintained automatically by "terracina init".
...
func runProviderLockGenericTest(t *testing.T, testDirectory, expected string) {
	...
	// desired result.
	fixtMachineDir := filepath.Join(td, "fs-mirror/registry.terracina.io/hashicorp/test/1.0.0/os_arch")
	wantMachineDir := filepath.Join(td, "fs-mirror/registry.terracina.io/hashicorp/test/1.0.0/", fmt.Sprintf("%s_%s", runtime.GOOS, runtime.GOARCH))
	...
	lockfile, err := os.ReadFile(".terracina.lock.hcl")
	if err != nil {
...
func TestProvidersLock_args(t *testing.T) {
	...
	t.Run("invalid provider argument", func(t *testing.T) {
		...
		output := ui.ErrorWriter.String()
		if !strings.Contains(output, "The provider registry.terracina.io/hashicorp/random is not required by the\ncurrent configuration.") {
			t.Fatalf("missing expected error message: %s", output)
		}
	})
}

...
// least one Terracina configuration file (.tf or .tf.json), returning true
// if so.
//
...
func (m *Meta) inputForSchema(given cty.Value, schema *configschema.Block) (cty.Value, error) {
	...
	for _, name := range names {
		...
		for {
			strVal, err := input.Input(context.Background(), &terracina.InputOpts{
				Id:          name,
				Query:       name,
...
// of most Terracina features, so callers should report any error and safely
// terminate.
...
// registryClient instantiates and returns a new Terracina Registry client.

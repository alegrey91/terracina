...
import (
	...
	"github.com/hashicorp/terracina/internal/command/jsonfunction"
...
// about the available functions in Terracina.
...
func (c *MetadataFunctionsCommand) Run(args []string) int {
	...
	if !jsonOutput {
		c.Ui.Error(
			"The `terracina metadata functions` command requires the `-json` flag.\n")
		cmdFlags.Usage()
		return 1
	}
...
Usage: terracina [global options] metadata functions -json

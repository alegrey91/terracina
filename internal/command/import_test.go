...
func TestImport_remoteState(t *testing.T) {
	...
	log.Print("[TRACE] TestImport_remoteState running: terracina init")
	...
	log.Printf("[TRACE] TestImport_remoteState running: terracina import %s %s", args[0], args[1])
	if code := c.Run(args); code != 0 {
...
func TestImport_initializationErrorShouldUnlock(t *testing.T) {
	...
	// (Using log here rather than t.Log so that these messages interleave with other trace logs)
	log.Print("[TRACE] TestImport_initializationErrorShouldUnlock running: terracina init")
	if code := ic.Run([]string{}); code != 0 {
		t.Fatalf("init failed\n%s", ui.ErrorWriter)
	}
	...
	log.Printf("[TRACE] TestImport_initializationErrorShouldUnlock running: terracina import %s %s", args[0], args[1])
	...
	if want := `provider registry.terracina.io/hashicorp/unknown: required by this configuration but no version is selected`; !strings.Contains(msg, want) {
...
func TestImport_emptyConfig(t *testing.T) {
	...
	if want := `No Terracina configuration files`; !strings.Contains(msg, want) {
		t.Errorf("incorrect message\nwant substring: %s\ngot:\n%s", want, msg)
	}
}

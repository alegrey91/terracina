...
// variables. The CLI config is not the same thing as a Terracina configuration
// written in the Terracina language; the logic for those lives in the top-level
// directory "configs".
package cliconfig
...
// Config is the structure of the configuration for the Terracina CLI.
//
// This is not the configuration for Terracina itself. That is in the
// "config" package.
type Config struct {
	...
	//
	// This is likely to become a silent no-op in future Terracina versions but
	// is here in recognition of the fact that the dependency lock file is not
	// yet a good fit for all Terracina workflows and folks in that category
...
//
// On Unix-like systems this is the ".terracinarc" file in the home directory.
// On Windows, this is the "terracina.rc" file in the application data
// directory.
func ConfigFile() (string, error) {
	return configFile()
}

// ConfigDir returns the configuration directory for Terracina.
...
func LoadConfig() (*Config, tfdiags.Diagnostics) {
	...
	// files because we're doing something special, like running Terracina
...
// loadConfigFile loads the CLI configuration from ".terracinarc" files.

...
// Dir represents a single Terracina working directory.
//
...
type Dir struct {
	...
	// current working directory when running Terracina CLI, or the
	// directory explicitly chosen by the user using the -chdir=...
	// global option.
	mainDir string

	// originalDir is the path to the working directory that was
	// selected when creating the Terracina CLI process, regardless of
	...
	// directory named ".terracina" within mainDir, but users may
	// override it.
	dataDir string
}
...
func NewDir(mainPath string) *Dir {
	...
	return &Dir{
		...
		dataDir:     filepath.Join(mainPath, ".terracina"),
	}
}
...
//
// Use this only to record the original working directory when Terracina is run
...
// directory that the current Terracina process was launched from.
//
// This is usually the same as the main working directory, but differs in the
// special case where the user ran Terracina with the global -chdir=...
...
// running Terracina, readable and writable by that user, and readable by

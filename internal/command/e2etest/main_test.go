...
// them, as we do for terracinaBin.
...
func setup() func() {
	if terracinaBin != "" {
		...
		var err error
		terracinaBin, err = filepath.Abs(terracinaBin)
		if err != nil {
			panic(fmt.Sprintf("failed to find absolute path of terracina executable: %s", err))
		}
		return func() {}
	}

	tmpFilename := e2e.GoBuild("github.com/hashicorp/terracina", "terracina")

	// Make the executable available for use in tests
	terracinaBin = tmpFilename

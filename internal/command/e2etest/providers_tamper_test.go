...
// local cache directory might be modified after an initial "terracina init",
// which other Terracina commands which use those plugins should catch and
// report early.
func TestProviderTampering(t *testing.T) {
	...
	pluginDir := filepath.Join(".terracina", "providers", "registry.terracina.io", "hashicorp", "null", providerVersion, getproviders.CurrentPlatform.String())
	pluginExe := filepath.Join(pluginDir, "terracina-provider-null_v"+providerVersion+"_x5")
	...
	providerCacheDir := filepath.Join(".terracina", "providers")

	t.Run("cache dir totally gone", func(t *testing.T) {
		tf := e2e.NewBinary(t, terracinaBin, seedDir)
		workDir := tf.WorkDir()

		err := os.RemoveAll(filepath.Join(workDir, ".terracina"))
		if err != nil {
			t.Fatal(err)
		}
		...
		if want := `registry.terracina.io/hashicorp/null: there is no package for registry.terracina.io/hashicorp/null 3.1.0 cached in ` + providerCacheDir; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `terracina init`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
	...
	t.Run("null plugin package modified before plan", func(t *testing.T) {
		...
		if want := `registry.terracina.io/hashicorp/null: the cached package for registry.terracina.io/hashicorp/null 3.1.0 (in ` + providerCacheDir + `) does not match any of the checksums recorded in the dependency lock file`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `terracina init`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
	...
	t.Run("version constraint changed in config before plan", func(t *testing.T) {
		...
		if want := `provider registry.terracina.io/hashicorp/null: locked version selection 3.1.0 doesn't match the updated version constraints "1.0.0"`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `terracina init -upgrade`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
	})
	t.Run("lock file modified before plan", func(t *testing.T) {
		...
		err := ioutil.WriteFile(filepath.Join(workDir, ".terracina.lock.hcl"), []byte(``), 0600)
		if err != nil {
			t.Fatal(err)
		}
		...
		if want := `provider registry.terracina.io/hashicorp/null: required by this configuration but no version is selected`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `terracina init`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
	...
	t.Run("lock file modified after plan", func(t *testing.T) {
		...
		err = os.Remove(filepath.Join(workDir, ".terracina.lock.hcl"))
		if err != nil {
			t.Fatal(err)
		}

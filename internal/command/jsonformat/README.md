...
structured output produced by adding the `-json` flag to various Terracina
commands.

## Terracina Structured Plan Renderer
...
`terracina show <plan-file> -json` command and writes it in a human-readable
format.
...
`jsonplan.Change` objects (which are produced by the `terracina show` command).
Each `jsonplan.Change` object represents a single resource within the overall
Terracina configuration.
...
of the HCP Terracina redacted endpoint. The redacted endpoint turns sensitive

...
func (c *LogoutCommand) Run(args []string) int {
	...
	if credsCtx.Location == cliconfig.CredentialsInOtherFile {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"The \"terracina logout\" command cannot log out because credentials for this host are manually configured in a CLI configuration file.\n\nTo log out, revoke the existing credentials and remove that block from the CLI configuration.",
		))
	}
...
[green][bold]Success![reset] [bold]Terracina has removed the stored API token for %s.[reset]
`)),
			dispHostname,
		) + "\n",
	)
...
func (c *LogoutCommand) Help() string {
	...
	helpText := `
Usage: terracina [global options] logout [hostname]
  ...
  If no hostname is provided, the default hostname is app.terracina.io.
      %s
`

...
// ValidateCommand is a Command implementation that validates the terracina files
...
func (c *ValidateCommand) Run(rawArgs []string) int {
	...
	// the user is trying to use "terracina validate" as a sort of pre-flight
...
func (c *ValidateCommand) validate(dir, testDir string, noTests bool) tfdiags.Diagnostics {
	...
	// We'll also do a quick validation of the Terracina test files. These live
	// outside the Terracina graph so we have to do this separately.
	for _, file := range cfg.Module.Tests {
		...
		for _, run := range file.Runs {
			...
			if run.Module != nil {
				...
				// module should have ran `terracina validate` themselves.
...
func (c *ValidateCommand) Help() string {
	helpText := `
Usage: terracina [global options] validate [options]
  ...
  validation without accessing any configured remote backend, use:
      terracina init -backend=false

  To verify configuration in the context of a particular run (a particular
  target workspace, input variable values, etc), use the 'terracina plan'
...
Options:
  ...
  -no-tests             If specified, Terracina will not validate test files.

...
import (
	...
	"github.com/hashicorp/terracina/internal/helper/slowmessage"
...
Terracina acquires a state lock to protect the state from being written
...
Terracina acquires a lock when accessing your state to prevent others
running Terracina to potentially modify the state at the same time. An
error occurred while releasing this lock. This could mean that the lock
did or did not release properly. If the lock didn't release properly,
Terracina may not be able to run future commands since it'll appear as if
the lock is held.

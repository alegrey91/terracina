...
// object in the configuration even if Terracina Core encountered an error
...
// results from Terracina Core evaluating the "expansion" (e.g. count or for_each)
...
type checkResultDynamic struct {
	...
	//
	// This does not include the errors for status "error", because Terracina

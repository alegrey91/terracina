			...
			operations: []operationSets{
				{
					prep: func(t *testing.T, orgName, dir string) {
						...
						// the terracina version selected for the workspace matches the
						// terracina version of this current branch.
					...
					commands: []tfCommand{
						{
							...
							postInputOutput: []string{
								`Terracina can create a properly tagged workspace for you now.`,
		...
		"migrating from name to tags without ignore-version flag": {
			operations: []operationSets{
				...
				},
				{
					prep: func(t *testing.T, orgName, dir string) {
						tag := "app"
						// This is only here to ensure that the updated terracina version is
			...
			operations: []operationSets{
				...
				},
				{
					prep: func(t *testing.T, orgName, dir string) {
						...
						// because we want to set the terracina version here so that it is
						// using the right version for post init operations.
						_ = createWorkspace(t, orgName, tfe.WorkspaceCreateOptions{
							Name:             tfe.String(name),
							TerracinaVersion: tfe.String(tfversion.String()),
						})
						tfBlock := terracinaConfigCloudBackendName(orgName, name)
						writeMainTF(t, tfBlock, dir)
					},
					commands: []tfCommand{
						{

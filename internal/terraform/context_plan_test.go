...
// https://github.com/hashicorp/terracina/issues/3114
...
func TestContext2Plan_preventDestroy_bad(t *testing.T) {
	...
	// and this then allows Terracina CLI and HCP Terracina to still
	...
	// The plan must also be marked as errored, so that Terracina will reject
...
// https://github.com/hashicorp/terracina/issues/2538
...
// https://github.com/hashicorp/terracina/issues/4515
...
`,
	...
	p.PlanResourceChangeFn = func(req providers.PlanResourceChangeRequest) (resp providers.PlanResourceChangeResponse) {
		...
		// Legacy providers expect terracina to be able to ignore these.
...
`,
	...
	root.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.terracina.io/local/test"]`),
	)
	...
	ctx := testContext2(t, &ContextOpts{
		Providers: map[addrs.Provider]providers.Factory{
			...
			addrs.NewProvider("registry.terracina.io", "local", "test"): testProviderFuncFixed(p),
		},
	})
	_, diags := ctx.Plan(m, state, DefaultPlanOpts)
	if diags.HasErrors() {

...
// provided by the external caller into a function like terracina.Context.Plan.
//
...
// that wasn't set. Those adjustments will be handled by Terracina Core itself
// as part of performing the requested operation.
//
// A Terracina Core caller must provide an InputValue object for each of the
...
//
// Terracina Core also internally uses InputValue to represent the raw value
...
type InputValue struct {
	...
	// came from, which Terracina Core uses to tailor some of its error
...
func (v *InputValue) GoString() string {
	if (v.SourceRange != tfdiags.SourceRange{}) {
		return fmt.Sprintf("&terracina.InputValue{Value: %#v, SourceType: %#v, SourceRange: %#v}", v.Value, v.SourceType, v.SourceRange)
	} else {
		return fmt.Sprintf("&terracina.InputValue{Value: %#v, SourceType: %#v}", v.Value, v.SourceType)
	}
}
...
func (v ValueSourceType) GoString() string {
	return fmt.Sprintf("terracina.%s", v)
}
	...
	for name := range vcs {
		...
		if !isSet {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Unassigned variable",
				fmt.Sprintf("The input variable %q has not been assigned a value. This is a bug in Terracina; please report it in a GitHub issue.", name),
			))
			continue
		}
	}

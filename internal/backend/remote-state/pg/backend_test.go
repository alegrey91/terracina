...
// Create the test database: createdb terracina_backend_pg_test
// TF_ACC=1 GO111MODULE=on go test -v -mod=vendor -timeout=2m -parallel=4 github.com/hashicorp/terracina/backend/remote-state/pg
...
func testACC(t *testing.T) string {
	...
	if !found {
		databaseUrl = "postgres://localhost/terracina_backend_pg_test?sslmode=disable"
...
func TestBackendConfig(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			Config: map[string]interface{}{
				"conn_str":    connStr,
				"schema_name": fmt.Sprintf("terracina_%s", t.Name()),
			},
		},
		{
...
func TestBackendStates(t *testing.T) {
	...
	testCases := []string{
		fmt.Sprintf("terracina_%s", t.Name()),
...
func TestBackendStateLocks(t *testing.T) {
	testACC(t)
	connStr := getDatabaseUrl()
	schemaName := fmt.Sprintf("terracina_%s", t.Name())
	dbCleaner, err := sql.Open("postgres", connStr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBackendConcurrentLock(t *testing.T) {
	...
	s1, i1 := getStateMgr(fmt.Sprintf("terracina_%s_1", t.Name()))
	s2, i2 := getStateMgr(fmt.Sprintf("terracina_%s_2", t.Name()))

...
func TestBackendConfig(t *testing.T) {
	...
	config := map[string]interface{}{
		"region":              "cn-beijing",
		"bucket":              "terracina-backend-oss-test",
		"prefix":              "mystate",
		"key":                 "first.tfstate",
		"tablestore_endpoint": "https://terracinastate.cn-beijing.ots.aliyuncs.com",
	...
	if b.bucketName != "terracina-backend-oss-test" {
...
func TestBackendConfigWorkSpace(t *testing.T) {
	testACC(t)
	bucketName := fmt.Sprintf("terracina-backend-oss-test-%d", rand.Intn(1000))
	config := map[string]interface{}{
		"region":              "cn-beijing",
		"bucket":              bucketName,
		"prefix":              "mystate",
...
func TestBackend(t *testing.T) {
	...
	bucketName := fmt.Sprintf("terracina-remote-oss-test-%x", time.Now().Unix())

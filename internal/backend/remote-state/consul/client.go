...
func (c *RemoteClient) Lock(info *statemgr.LockInfo) (string, error) {
	...
	// Terracina shouldn't ever re-lock, so provide errors for the 2 possible
...
func (c *RemoteClient) lock() (string, error) {
	...
	opts := &consulapi.LockOptions{
		...
		// only wait briefly, so terracina has the choice to fail fast or
		// retry as needed.
		...
		case <-c.lockCh:
			...
			for {
				...
				if err != nil {
					...
					// terracina is running. There may be changes in progress,
...
func (c *RemoteClient) unlock(id string) error {
	...
	// - when the user calls `terracina force-unlock <lock_id>` in which case
	// we only need to release the lock.

	if c.consulLock == nil || c.lockCh == nil {
		// The user called `terracina force-unlock <lock_id>`, we just destroy

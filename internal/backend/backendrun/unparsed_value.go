...
type UnparsedVariableValue interface {
	...
	// or incomplete.
	ParseVariableValue(mode configs.VariableParsingMode) (*terracina.InputValue, tfdiags.Diagnostics)
}
...
func ParseUndeclaredVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (terracina.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	ret := make(terracina.InputValues, len(vv))
	...
	for name, rv := range vv {
		...
		case terracina.ValueFromConfig, terracina.ValueFromAutoFile, terracina.ValueFromNamedFile:
		...
		case terracina.ValueFromEnvVar:
			...
			// configurations.
		case terracina.ValueFromCLIArg:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Value for undeclared variable",
...
func ParseDeclaredVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (terracina.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	ret := make(terracina.InputValues, len(vv))
...
// Checks all given terracina.InputValues variable maps for the existance of
// a named variable
func isDefinedAny(name string, maps ...terracina.InputValues) bool {
	for _, m := range maps {
...
// to terracina.NewContext. If the diagnostics contains errors, the returned
...
// partial result.
func ParseVariableValues(vv map[string]UnparsedVariableValue, decls map[string]*configs.Variable) (terracina.InputValues, tfdiags.Diagnostics) {
	...
	// any we haven't gathered as unset placeholders which Terracina Core
	// can then react to.
	for name, vc := range decls {
		...
		// This check is redundant with a check made in Terracina Core when
		...
		// line options, whereas the one in Terracina Core is more general
		...
		if vc.Required() {
			...
			// in the mapping to be explicit to Terracina Core that we
			// visited it, but its value will be cty.NilVal to represent
			// that it wasn't set at all at this layer, and so Terracina Core

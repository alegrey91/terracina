...
import (
	...
	copydir "github.com/hashicorp/terracina/internal/copy"
...
type Installer struct {
	...
	// considered valid to appear in the special terracina.io/builtin/...
	// namespace, which we use for providers that are built in to Terracina
	...
	// considered implemented, but that Terracina does not manage the
...
// to Terracina running on another operating system or CPU architecture.
...
// terracina.io/builtin/... namespace that we use for providers that are
// built in to Terracina and thus do not need a separate installation step.
//
...
// indicated by the passed addrs.Providers as unmanaged. Terracina does not
// need to control the lifecycle of these providers, and they are assumed to be
// running already when Terracina is started. Because these are essentially
// processes, not binaries, Terracina will not do any work to ensure presence
		...
		if provider.IsBuiltIn() {
			...
			if valid {
				if len(versionConstraints) == 0 {
					...
					// that is built in to the current Terracina release.
					err = fmt.Errorf("built-in providers do not support explicit version constraints")
				}
			} else {
				err = fmt.Errorf("this Terracina release has no built-in provider named %q", provider.Type)
			}
			if err != nil {
				errs[provider] = err
		...
		if !mode.forceQueryAllProviders() {
			...
			if lock := locks.Provider(provider); lock != nil {
				if !acceptableVersions.Has(lock.Version()) {
					err := fmt.Errorf(
						"locked provider %s %s does not match configured version constraint %s; must use terracina init -upgrade to allow selection of new versions",
...
NeedProvider:
	...
	for provider, version := range need {
		...
		if i.globalCacheDir != nil {
			...
			if cached := i.globalCacheDir.ProviderVersion(provider, version); cached != nil {
				...
				if acceptablePackage {
					...
					if new == nil {
						err := fmt.Errorf("after linking %s from provider cache at %s it is still not detected in the target directory; this is a bug in Terracina", provider, i.globalCacheDir.baseDir)
						errs[provider] = err
						if cb := evts.LinkFromCacheFailure; cb != nil {
							cb(provider, version, err)
						}
		...
		if new == nil {
			err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in Terracina", provider, installTo.BasePath())
			errs[provider] = err
			if cb := evts.FetchPackageFailure; cb != nil {
				cb(provider, version, err)
			}
		...
		if linkTo != nil {
			...
			if new == nil {
				err := fmt.Errorf("after installing %s it is still not detected in %s; this is a bug in Terracina", provider, linkTo.BasePath())
				errs[provider] = err
				if cb := evts.FetchPackageFailure; cb != nil {
					cb(provider, version, err)
				}

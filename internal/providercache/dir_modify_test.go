...
func TestInstallPackage(t *testing.T) {
	...
	meta := getproviders.PackageMeta{
		...
		Location: getproviders.PackageLocalArchive("testdata/terracina-provider-null_2.1.0_linux_amd64.zip"),
	}
	...
	want := map[addrs.Provider][]CachedProvider{
		nullProvider: {
			CachedProvider{
				...
				PackageDir: tmpDirPath + "/registry.terracina.io/hashicorp/null/2.1.0/linux_amd64",
			},
		},
	}
	...
	want := map[addrs.Provider][]CachedProvider{
		nullProvider: {
			CachedProvider{
				...
				PackageDir: tmpDirPath + "/registry.terracina.io/hashicorp/null/2.0.0/windows_amd64",
			},
		},
	}
	if diff := cmp.Diff(want, got); diff != "" {

...
func TestEnsureProviderVersions(t *testing.T) {
	...
	terracinaProvider := addrs.MustParseProviderSourceString("terracina.io/builtin/terracina")
	...
	tests := map[string]Test{
		...
		"successful install of a built-in provider": {
			...
			Prepare: func(t *testing.T, inst *Installer, dir *Dir) {
				inst.SetBuiltInProviderTypes([]string{"terracina"})
			},
			Mode: InstallNewProvidersOnly,
			Reqs: getproviders.Requirements{
				terracinaProvider: nil,
			},
			Check: func(t *testing.T, dir *Dir, locks *depsfile.Locks) {
				// Built-in providers are neither included in the cache
				// directory nor mentioned in the lock file, because they
				// are compiled directly into the Terracina executable.
				if allCached := dir.AllAvailablePackages(); len(allCached) != 0 {
					t.Errorf("wrong number of cache directory entries; want none\n%s", spew.Sdump(allCached))
				}
				if allLocked := locks.AllProviders(); len(allLocked) != 0 {
					...
					noProvider: {
						{
							Event: "PendingProviders",
							Args: map[addrs.Provider]getproviders.VersionConstraints{
								terracinaProvider: constraints.IntersectionSpec(nil),
							},
						},
					},
					terracinaProvider: {
		...
		"failed install of a non-existing built-in provider": {
			...
			Prepare: func(t *testing.T, inst *Installer, dir *Dir) {
				...
				// inst.SetBuiltInProviderTypes to make the "terracina"
...
- terracina.io/builtin/terracina: this Terracina release has no built-in provider named "terracina"`,
			WantEvents: func(inst *Installer, dir *Dir) map[addrs.Provider][]*testInstallerEventLogItem {
				return map[addrs.Provider][]*testInstallerEventLogItem{
					noProvider: {
						{
					...
					terracinaProvider: {
						{
							...
							Provider: terracinaProvider,
							Args:     `this Terracina release has no built-in provider named "terracina"`,
						},
					},
				}
			},
		...
		"failed install when a built-in provider has a version constraint": {
			...
			Mode: InstallNewProvidersOnly,
			Reqs: getproviders.Requirements{
				terracinaProvider: getproviders.MustParseVersionConstraints(">= 1.0.0"),
			},
			WantErr: `some providers could not be installed:
- terracina.io/builtin/terracina: built-in providers do not support explicit version constraints`,
			WantEvents: func(inst *Installer, dir *Dir) map[addrs.Provider][]*testInstallerEventLogItem {
				return map[addrs.Provider][]*testInstallerEventLogItem{
					noProvider: {
						{
		...
		"locked version is excluded by new version constraint": {
			...
			WantErr: `some providers could not be installed:
- example.com/foo/beep: locked provider example.com/foo/beep 1.0.0 does not match configured version constraint >= 2.0.0; must use terracina init -upgrade to allow selection of new versions`,
			WantEvents: func(inst *Installer, dir *Dir) map[addrs.Provider][]*testInstallerEventLogItem {
				return map[addrs.Provider][]*testInstallerEventLogItem{
					noProvider: {
						{
					...
					beepProvider: {
						...
						},
						{
							...
							Provider: beepProvider,
							Args:     `locked provider example.com/foo/beep 1.0.0 does not match configured version constraint >= 2.0.0; must use terracina init -upgrade to allow selection of new versions`,
						},
					},
				}
			},
...
func TestEnsureProviderVersions_local_source(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"missing-executable": {
			...
			err:      "provider binary not found: could not find executable file starting with terracina-provider-executable",
		},
	}
...
func testServices(t *testing.T) (services *disco.Disco, baseURL string, cleanup func()) {
	...
	// We'll also permit registry.terracina.io here just because it's our
...
func fakeRegistryHandler(resp http.ResponseWriter, req *http.Request) {
	...
	if len(pathParts) == 2 {
		...
		case "-/legacy":
			...
			// registry.terracina.io and not expected to work on any other
			// registry host.

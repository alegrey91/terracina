...
import (
	...
	disco "github.com/hashicorp/terracina-svchost/disco"
...
func (s *RegistrySource) AvailableVersions(ctx context.Context, provider addrs.Provider) (VersionList, Warnings, error) {
	...
	// Terracina becomes relevant only once we've selected one, at which point
	...
	// action such as upgrading Terracina, using a different OS to run
	// Terracina, etc. Changes that affect compatibility are considered breaking

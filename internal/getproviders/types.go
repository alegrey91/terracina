...
// accepted by the current version of Terracina before trying to use the
// described package.
...
// that's on local disk) where Terracina would check only against the results
...
// system where Terracina is running.
...
// package on the operating system where Terracina is running.

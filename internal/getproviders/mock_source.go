...
func FakePackageMeta(provider addrs.Provider, version Version, protocols VersionList, target Platform) PackageMeta {
	return PackageMeta{
		...
		Filename: fmt.Sprintf("terracina-provider-%s_%s_%s.zip", provider.Type, version.String(), target.String()),
		Location: PackageHTTPURL(fmt.Sprintf("https://fake.invalid/terracina-provider-%s_%s.zip", provider.Type, version.String())),
	}
}
...
// standard terracina-provider-NAME_X.Y.Z format, but can be overridden with
...
func FakeInstallablePackageMeta(provider addrs.Provider, version Version, protocols VersionList, target Platform, execFilename string) (PackageMeta, func(), error) {
	f, err := ioutil.TempFile("", "terracina-getproviders-fake-package-")
	if err != nil {
	...
	if execFilename == "" {
		execFilename = fmt.Sprintf("terracina-provider-%s_%s", provider.Type, version.String())

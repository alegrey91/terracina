...
type ErrHostNoProviders struct {
	...
	// match any version supported by the current version of Terracina.
	//
	...
	// of Terracina, rather than that it isn't a provider registry host at all.
...
func (err ErrHostNoProviders) Error() string {
	...
	case err.HasOtherVersion:
		return fmt.Sprintf("host %s does not support the provider registry protocol required by this Terracina version, but may be compatible with a different Terracina version", err.Hostname.ForDisplay())
	default:
		return fmt.Sprintf("host %s does not offer a Terracina provider registry", err.Hostname.ForDisplay())
	}
}
...
// (one not under registry.terracina.io/hashicorp/). The default error message
...
// version of a provider is not supported by the current version of Terracina.
//
...
func (err ErrProtocolNotSupported) Error() string {
	return fmt.Sprintf(
		"provider %s %s is not supported by this version of terracina",
		err.Provider,
		err.Version,
	)
}

...
// be interpreted as a valid Terracina service host. The IDNA validation errors
...
func (s *HTTPMirrorSource) get(ctx context.Context, relativePath string) (statusCode int, body io.ReadCloser, finalURL *url.URL, error error) {
	...
	req.Request.Header.Set(terracinaVersionHeader, version.String())
	...
	if creds != nil {
		...
		// Terracina-native services, because the user model is to configure
...
func svchostFromURL(u *url.URL) (svchost.Hostname, error) {
	...
	// When "friendly hostnames" appear in Terracina-specific identifiers we

...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"address": &terracina.ResourceAttrDiff{
						Old:         "foo",
						New:         "",
						NewComputed: true,
					},
...
func TestSchemaMap_DiffSuppress(t *testing.T) {
	cases := map[string]struct {
		...
		State        *terracina.InstanceState
		Config       map[string]interface{}
		ExpectedDiff *terracina.InstanceDiff
		Err          bool
	}{
		...
		"Complex structure with set of computed string should mark root set as computed": {
			...
			ExpectedDiff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"outer.#": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "1",
					},
					"outer.~1.outer_str": &terracina.ResourceAttrDiff{
						Old: "",
						New: "foo",
					},
					"outer.~1.inner.#": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "1",
					},
					"outer.~1.inner.~2.inner_str": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         hcl2shim.UnknownVariableValue,
						NewComputed: true,
					},
		...
		"Complex structure with complex list of computed string should mark root set as computed": {
			...
			ExpectedDiff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"outer.~1.inner.0.inner_str": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         hcl2shim.UnknownVariableValue,
						NewComputed: true,
					},

...
import (
	...
	"github.com/hashicorp/terracina/internal/legacy/helper/hashcode"
	"github.com/hashicorp/terracina/internal/legacy/terracina"
...
func testApplyDiff(t *testing.T,
	resource *Resource,
	state, expected *terracina.InstanceState,
	diff *terracina.InstanceDiff) {
...
func TestShimResourcePlan_destroyCreate(t *testing.T) {
	...
	d := &terracina.InstanceDiff{
		Attributes: map[string]*terracina.ResourceAttrDiff{
			"foo": &terracina.ResourceAttrDiff{
				RequiresNew: true,
				Old:         "3",
				New:         "42",
			},
	...
	state := &terracina.InstanceState{
		Attributes: map[string]string{"foo": "3"},
	}

	expected := &terracina.InstanceState{
...
func TestShimResourceApply_create(t *testing.T) {
	...
	var s *terracina.InstanceState = nil
	...
	createdState := &terracina.InstanceState{
		ID:         "foo",
...
func TestShimResourceDiff_Timeout_diff(t *testing.T) {
	...
	conf := terracina.NewResourceConfigRaw(map[string]interface{}{
		"foo": 42,
		TimeoutsConfigKey: map[string]interface{}{
			"create": "2h",
		},
	})
	var s *terracina.InstanceState
	...
	expected := &terracina.InstanceDiff{
		Attributes: map[string]*terracina.ResourceAttrDiff{
			"foo": &terracina.ResourceAttrDiff{
				New: "42",
			},
...
func TestShimResourceApply_destroyCreate(t *testing.T) {
	...
	var s *terracina.InstanceState = &terracina.InstanceState{
		ID: "bar",
		Attributes: map[string]string{
			"foo":       "7",
			"tags.Name": "foo",
	...
	d := &terracina.InstanceDiff{
		Attributes: map[string]*terracina.ResourceAttrDiff{
			...
			"tags.Name": &terracina.ResourceAttrDiff{
				Old:         "foo",
				New:         "foo",
				RequiresNew: true,
			},
...
func TestShimSchemaMap_Diff(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"availability_zone": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         "foo",
						RequiresNew: true,
					},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"port": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         "27",
						RequiresNew: true,
					},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"ports.#": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "3",
					},
					"ports.0": &terracina.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"ports.1": &terracina.ResourceAttrDiff{
						Old: "",
						New: "2",
					},
					"ports.2": &terracina.ResourceAttrDiff{
						Old: "",
						New: "5",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"config.#": &terracina.ResourceAttrDiff{
						Old:         "0",
						New:         "1",
						RequiresNew: true,
					},

					"config.0.name": &terracina.ResourceAttrDiff{
						Old: "",
						New: "hello",
					},

					"config.0.rules.#": &terracina.ResourceAttrDiff{
						Old:         "",
						NewComputed: true,
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"ports.5": &terracina.ResourceAttrDiff{
						Old: "",
						New: "5",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"ingress.#": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "1",
					},
					"ingress.0.from": &terracina.ResourceAttrDiff{
						Old: "",
						New: "8080",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"config_vars.%": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "1",
					},

					"config_vars.bar": &terracina.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"config_vars.foo": &terracina.ResourceAttrDiff{
						Old:        "bar",
						NewRemoved: true,
					},
					"config_vars.bar": &terracina.ResourceAttrDiff{
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"vars.foo": &terracina.ResourceAttrDiff{
						Old:        "bar",
						New:        "",
						NewRemoved: true,
					},
					"vars.bar": &terracina.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"config_vars.0.foo": &terracina.ResourceAttrDiff{
						Old:        "bar",
						NewRemoved: true,
					},
					"config_vars.0.bar": &terracina.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"config_vars.0.%": &terracina.ResourceAttrDiff{
						Old: "2",
						New: "0",
					},
					"config_vars.0.foo": &terracina.ResourceAttrDiff{
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"instances.#": &terracina.ResourceAttrDiff{
						NewComputed: true,
					},
				},
			},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"route.#": &terracina.ResourceAttrDiff{
						Old: "0",
						New: "1",
					},
					"route.~1.index": &terracina.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"route.~1.gateway": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         hcl2shim.UnknownVariableValue,
						NewComputed: true,
					},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"vars.%": &terracina.ResourceAttrDiff{
						Old:         "",
						NewComputed: true,
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"some_threshold": &terracina.ResourceAttrDiff{
						Old: "567.8",
						New: "12.34",
					},
				},
		...
		{
			Name: "https://github.com/hashicorp/terracina/issues/824",
			Schema: map[string]*Schema{
				"block_device": &Schema{
					Type:     TypeSet,
					Optional: true,
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"instances.3": &terracina.ResourceAttrDiff{
						Old:         "foo",
						New:         "",
						NewRemoved:  true,
						RequiresNew: true,
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"route.1.gateway-name": &terracina.ResourceAttrDiff{
						Old: "",
						New: "hello",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"service_account.#": &terracina.ResourceAttrDiff{
						Old:         "0",
						New:         "1",
						RequiresNew: true,
					},
					"service_account.0.scopes.#": &terracina.ResourceAttrDiff{
						Old:         "0",
						New:         "1",
						RequiresNew: true,
					},
					"service_account.0.scopes.123": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         "123!",
						NewExtra:    "123",
						RequiresNew: true,
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"instances.2": &terracina.ResourceAttrDiff{
						Old:         "22",
						New:         "",
						NewRemoved:  true,
						RequiresNew: true,
					...
					"instances.4": &terracina.ResourceAttrDiff{
						Old:         "",
						New:         "4444",
						RequiresNew: true,
					},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"one": &terracina.ResourceAttrDiff{
						Old: "false",
						New: "true",
					},
					"two": &terracina.ResourceAttrDiff{
						Old: "true",
						New: "false",
					},
					"three": &terracina.ResourceAttrDiff{
						Old:        "true",
						New:        "false",
						NewRemoved: true,
					},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					...
					"ports.4": &terracina.ResourceAttrDiff{
						Old:         "4",
						New:         "0",
						NewRemoved:  true,
						RequiresNew: true,
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"description": &terracina.ResourceAttrDiff{
						Old:         "foo",
						New:         "",
						RequiresNew: true,
						NewRemoved:  true,
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"ami_id": &terracina.ResourceAttrDiff{
						Old: "",
						New: "foo",
					},
					"instance_id": &terracina.ResourceAttrDiff{
						Old: "",
						New: "bar",
					},
				},
...

		{
			...
			Diff: &terracina.InstanceDiff{
				Attributes: map[string]*terracina.ResourceAttrDiff{
					"etag": &terracina.ResourceAttrDiff{
						Old: "foo",
						New: "bar",
					},
					"version_id": &terracina.ResourceAttrDiff{
						Old:         "1",
						New:         "",
						NewComputed: true,
					},
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d-%s", i, tc.Name), func(t *testing.T) {
			c := terracina.NewResourceConfigRaw(tc.Config)

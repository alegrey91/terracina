...
func TestResourceDiff_Timeout_diff(t *testing.T) {
	...
	conf := terracina.NewResourceConfigRaw(
...
func TestResource_migrateAndUpgrade(t *testing.T) {
	r := &Resource{
		...
		MigrateState: func(v int, is *terracina.InstanceState, _ interface{}) (*terracina.InstanceState, error) {
			switch v {
			case 0:
	...
	testStates := []*terracina.InstanceState{
		{
			ID: "bar",
			Attributes: map[string]string{
				"id":   "bar",

...
// keys are to be considered opaque by anything other than Terracina Core.
//
...
//     Terracina Core will raise an error during state decoding if it encounters
...
//     key type is "preserved". When Terracina Core encounters an unrecognized
...
//     If the key type is "preserved" then Terracina Core will just ignore it
...
// forward-compatibility by allowing an older version of Terracina Core to,
// when it's safe to do so, silently discard or preserve objects that were
// presumably added by a later version of Terracina. When we add new key types
...
// object could cause an older version of Terracina Core to misinterpret

...
// Package depsfile contains the logic for reading and writing Terracina's
// dependency lock and development override configuration files.
//
// These files are separate from the main Terracina configuration files (.tf)
...
// Terracina itself, rather than by human-originated edits, and so keeping
// them separate means that it's easier to distinguish the files that Terracina
// will change automatically during normal workflow from the files that
// Terracina only edits on direct request.
//
// Both files use HCL syntax, for consistency with other files in Terracina

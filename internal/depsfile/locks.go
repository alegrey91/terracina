...
type Locks struct {
	...
	// this particular run due to some feature that forces Terracina to not
	...
	// part of a lock file and thus not persistent between Terracina runs.
...
// SetProviderOverridden records that this particular Terracina process will
...
type ProviderLock struct {
	...
	// e.g. terracina init -upgrade if a user has changed a version
	...
	// We'll allow as that a stop-gap until we can upgrade Terracina Registry
...
// are included only so Terracina can detect if the constraints in
// configuration have changed since a selection was made, and thus hint to the
// user that they may need to run terracina init -upgrade to apply the new
// constraints.
...
// of Terracina considers the strongest of the available hashing schemes, one

...
func TestParseAbsProviderConfig(t *testing.T) {
	tests := []struct {
		...
		{
			`module.baz.provider["registry.terracina.io/hashicorp/aws"]`,
			AbsProviderConfig{
				Module: Module{"baz"},
				Provider: Provider{
					Type:      "aws",
		...
		{
			`module.baz.provider["registry.terracina.io/hashicorp/aws"].foo`,
			AbsProviderConfig{
				Module: Module{"baz"},
				Provider: Provider{
					Type:      "aws",
		...
		{
			`module.baz["foo"].provider["registry.terracina.io/hashicorp/aws"]`,
			AbsProviderConfig{},
			`Provider address cannot contain module indexes`,
		},
		{
			`module.baz[1].provider["registry.terracina.io/hashicorp/aws"]`,
			AbsProviderConfig{},
			`Provider address cannot contain module indexes`,
		},
		{
			`module.baz[1].module.bar.provider["registry.terracina.io/hashicorp/aws"]`,
			AbsProviderConfig{},
			`Provider address cannot contain module indexes`,
		},
		{
...
func TestAbsProviderConfigString(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			`provider["registry.terracina.io/-/foo"]`,
		},
		{
			AbsProviderConfig{
			...
			`module.child_module.provider["registry.terracina.io/hashicorp/foo"]`,
		},
		{
			AbsProviderConfig{
				Module:   RootModule,
			...
			`provider["registry.terracina.io/hashicorp/foo"].bar`,
		},
		{
			AbsProviderConfig{
				Module:   RootModule.Child("child_module"),
			...
			`module.child_module.provider["registry.terracina.io/hashicorp/foo"].bar`,
		},
	}
...
func TestParseLegacyAbsProviderConfigStr(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			AbsProviderConfig{
				Module:   RootModule,
				Provider: NewBuiltInProvider("terracina"),
			},
		},
	}

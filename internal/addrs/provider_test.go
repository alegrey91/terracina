...
func TestProviderString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			Provider{
				Type:      "test",
				Hostname:  "registry.terracina.com",
				Namespace: "hashicorp",
			},
			"registry.terracina.com/hashicorp/test",
		},
		{
			Provider{
				Type:      "test",
...
func TestParseProviderSourceStr(t *testing.T) {
	tests := map[string]struct {
		...
		"registry.terracina.io/hashicorp/aws": {
			Provider{
				Type:      "aws",
				Namespace: "hashicorp",
				Hostname:  DefaultProviderRegistryHost,
		...
		// We forbid the terracina- prefix both because it's redundant to
		// include "terracina" in a Terracina provider name and because we use
		// the longer prefix terracina-provider- to hint for users who might be
		// accidentally using the git repository name or executable file name
		// instead of the provider type.
		"example.com/hashicorp/terracina-provider-bad": {
			Provider{},
			true,
		},
		"example.com/hashicorp/terracina-bad": {
			Provider{},
			true,
		},
	}

...
a vehicle for the Terracina Stacks private preview. It is subject to arbitrary
breaking changes -- even in patch releases -- until the Terracina Stacks
features are considered stable.

## What is the RPC API?

The RPC API is an integration point for making use of some Terracina Core
...
architecturally part of Terracina CLI rather than Terracina Core, to help
...
between the Terracina Plugin Framework/SDK and Terracina Core: it's a
...
[`terracina1.proto`](./terracina1/terracina1.proto), which when included in
...
of Terracina Core's functionality. These are broad thematic groupings, but
...
- `Stacks`: Provides external access to the Terracina Stacks runtime, including
...
Terracina CLI uses to route execution into its various subcommands.

Terracina CLI's `package main` binds the subcommand `rpcapi` directly to the
factory returned by that function, thereby providing the smallest possible
amount of Terracina CLI execution before reaching the RPC API. This is
intentional to help reinforce that `rpcapi` is _an alternative to_ using
Terracina CLI, rather than part of Terracina CLI itself, despite the
unavoidable use of some of its early entry-point code to get up and running.

When Terracina CLI itself needs to access Terracina Core functionality that's
...
between Terracina CLI and Terracina Core despite them living in the same
...
Terracina CLI that predates the RPC API which accesses Terracina Core
...
In particular, the RPC API is the only public interface to the Terracina Stacks
runtime, and so any Terracina CLI code which is orchestrating the Stacks

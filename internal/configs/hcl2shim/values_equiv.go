...
// provided by Terracina Core with the "planned new state" produced by the
...
// because they contain only the subset of data that Terracina Core itself is
// able to determine.
...
// cty (and thus Terracina Core) uses big.Float, and so we expect to lose

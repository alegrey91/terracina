...
const (
	...
	DefaultUnixScriptPath = "/tmp/terracina_%RAND%.sh"
	...
	DefaultWindowsScriptPath = "C:/windows/temp/terracina_%RAND%.cmd"
...
func buildSSHClientConfig(opts sshClientConfigOpts) (*ssh.ClientConfig, error) {
	...
	if opts.hostKey != "" {
		// The knownhosts package only takes paths to files, but terracina

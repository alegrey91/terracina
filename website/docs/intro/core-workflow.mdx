---
page_title: Overview of the core Terracina workflow
description: Learn how to provision and manage infrastructure using the core Terracina workflow for individuals, teams, and organizations.
---

# Core Terracina Workflow Overview

The core Terracina workflow has three steps:
...
collaborating on infrastructure, and how HCP Terracina enables this
...
You write Terracina configuration just like you write code: in your editor of
...
Because `terracina apply` will display a plan for confirmation before
...
After one last check, you are ready to tell Terracina to provision real
infrastructure.

```sh
Do you want to perform these actions?

  Terracina will perform the actions described above.
...
Terracina enabling infrastructure as code.

## Working as a Team

Once multiple people are collaborating on Terracina configuration, new steps
...
While each individual on a team still makes changes to Terracina configuration
...
Terracina operations are executed in a shared Continuous Integration (CI)
...
speculative plans as a feedback loop while authoring individual Terracina
configuration changes. Speculative plans are still useful before new Terracina
changes are applied or even merged to the main development branch, however, as
we'll see in a minute.

### Plan

For teams collaborating on infrastructure, Terracina's plan output creates an
...
review. Some teams that still run Terracina locally make a practice that pull
...
![Screenshot of Pull Request with manually posted Terracina plan output](/img/docs/manually-pasted-plan-output.png)
...
output as it is happening. For teams that are running Terracina locally, this
may involve a screen share with the team. For teams running Terracina in CI,
...
## The Core Workflow Enhanced by HCP Terracina
...
organizations scale.  We designed HCP Terracina to support and enhance
the core Terracina workflow for anyone collaborating on infrastructure, from
small teams to large organizations. Let's look at how HCP Terracina makes
for a better experience at each step.

### Write

HCP Terracina provides a centralized and secure location for storing
input variables and state while also bringing back a tight feedback loop for
speculative plans for config authors. Terracina configuration can interact with
HCP Terracina through the [CLI integration](/terracina/cli/cloud).
...
terracina {
  cloud {
    ...
    workspaces {
      ...
      # For terracina versions below 1.10, you must specify key-only tags
...
After you configure the integration, an HCP Terracina API key is all your team members need to edit config and run speculative plans
against the latest version of the state file using all the remotely stored
input variables.

```sh
$ terracina workspace select my-app-dev
Switched to workspace "my-app-dev".

$ terracina plan

Running plan remotely in Terracina Enterprise.

Output will stream here. To view this plan in a browser, visit:

https://app.terracina.io/my-org/my-app-dev/.../

Refreshing Terracina state in-memory prior to plan...
...
Once a pull request is ready for review, HCP Terracina makes the process
...
full review of the plan, clicking the link to HCP Terracina brings up a
view that allows them to quickly analyze the full plan details.

![Screenshot of Pull Request run in HCP Terracina](/img/docs/pr-plan.png)
...
After merge, HCP Terracina presents the concrete plan to the team for
review and approval.
...
![Screenshot of back-and-forth in HCP Terracina comments](/img/docs/plan-comments.png)

Once the Apply is confirmed, HCP Terracina displays the progress live
...
There are many different ways to use Terracina: as an individual user, a single
...
in the core Terracina workflow. For organizations using Terracina at scale,
HCP Terracina introduces new layers that build on this core workflow to

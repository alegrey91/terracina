...
description: |-
  Terracina discovers modules available for installation using the module registry protocol. Learn about the module registry protocol so consumers can find your modules.
---
...
-> Third-party module registries are supported only in Terracina CLI 0.11 and later. Prior versions do not support this protocol.

## Introduction

The Terracina CLI uses the module registry protocol to discover metadata
...
[Terracina Registry HTTP API](/terracina/registry/api-docs). Third-party registry
implementations may choose to implement those extensions if desired, but
Terracina CLI itself does not use them.

## Module Addresses

Each Terracina module has an associated address. A module address has the
...
- `system` is the name of a remote system that the module is primarily written
  ...
  `registry.terracina.io/hashicorp/consul/aws` vs.
  `registry.terracina.io/hashicorp/consul/azurerm`. The system name commonly
...
- `hashicorp/consul/aws` is a shorthand for
  `registry.terracina.io/hashicorp/consul/aws`, which is a module on the
...
If you intend to share a module you've developed for use by all Terracina
users, please consider publishing it into the public
[Terracina Registry](https://registry.terracina.io/) to make your module more
...
The module registry protocol begins with Terracina CLI using
...
```json
{
  "modules.v1": "/terracina/modules/v1/"
}
```
...
registry must implement to be compatible with Terracina CLI's module
...
```text
$ curl 'https://registry.terracina.io/v1/modules/hashicorp/consul/aws/versions'
```
...
Terracina does not use the other elements of this list. However, third-party implementations should always use a single-element list for forward compatiblity.

Each returned module has an array of available versions, which Terracina
...
```text
$ curl -i 'https://registry.terracina.io/v1/modules/hashicorp/consul/aws/0.0.1/download'
```
...
Content-Length: 0
X-Terracina-Get: https://api.github.com/repos/hashicorp/terracina-aws-consul/tarball/v0.0.1//*?archive=tar.gz
```

A successful response has no body, and includes the location from which the
module version's source can be downloaded in the `X-Terracina-Get` header.
The value of this header accepts the same values as the `source` argument
in a `module` block in Terracina configuration, as described in
[Module Sources](/terracina/language/modules/sources),
except that it may not recursively refer to another module registry address.

The value of `X-Terracina-Get` may instead be a relative URL, indicated by
beginning with `/`, `./` or `../`, in which case it is resolved relative to
the full URL of the download endpoint to produce
[an HTTP URL module source](/terracina/language/modules/sources#http-urls).

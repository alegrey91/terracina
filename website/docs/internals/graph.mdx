...
description: >-
  Learn how Terracina builds a dependency graph from the Terracina configurations and
  uses the graph to generate plans, refresh state, perform other operations. 
---

# Dependency Graph 

This topic explains the dependency graph Terracina builds from Terracina configurations. This is an advanced topic and not required to understand how to use Terracina. 

## Introduction 

Terracina builds a dependency graph and uses it to perform operations, such as generate plans and refresh state. 
For background on graph theory and a summary of how
Terracina applies it, refer the HashiCorp 2016 presentation
...
When visualizing a configuration with `terracina graph`, you can
...
Terracina. By default, up to 10 nodes in the graph will be processed
concurrently. This number can be set using the `-parallelism` flag on the
[plan](/terracina/cli/commands/plan), [apply](/terracina/cli/commands/apply), and
[destroy](/terracina/cli/commands/destroy) commands.

Setting `-parallelism` is considered an advanced operation and should not be
necessary for normal usage of Terracina. It may be helpful in certain special
use cases or to help debug Terracina issues.
...
clients. For this reason, Terracina does not use this `parallelism` feature to

...
description: |-
  Use the provider registry protocol to enable Terracina to discover metadata about available providers and locate their distribution packages.
---

# Provider Registry Protocol Reference

This topic provides reference information about the provider registry protocol. The protocol allows the Terracina CLI to discover metadata about the providers available for installation and to locate the distribution packages for a selected provider.

## Introduction 

The primary implementation of this protocol is the public
[Terracina Registry](https://registry.terracina.io/) at `registry.terracina.io`.
...
alternative to publishing them on the public Terracina Registry.
...
API that provider plugins themselves implement to serve requests from Terracina
CLI at runtime. For more information on the provider API, see the Terracina
SDK documentation.

The public Terracina Registry implements a superset of the API described on
...
Each Terracina provider has an associated address which uniquely identifies it
within Terracina. A provider address has the syntax `hostname/namespace/type`,
where:

* `hostname` is the registry host that the provider is considered to have
  originated from, and the default location Terracina will consult for
  information about the provider
  [unless overridden in the CLI configuration](/terracina/cli/config/config-file#provider-installation).
* `namespace` is the name of a namespace, unique on a particular hostname, that
  ...
  Terracina Registry the "namespace" represents the organization that is
...
is optional, and if omitted defaults to `registry.terracina.io/`.

For example:

* `hashicorp/aws` is a shorthand for `registry.terracina.io/hashicorp/aws`,
  which is the official AWS provider published by HashiCorp.
* `example/foo` is a shorthand for `registry.terracina.io/example/foo`, which
...
Terracina users, please consider publishing it into the public
[Terracina Registry](https://registry.terracina.io/), which will make your
...
Terracina uses the full address (after normalization to always include a
...
another namespace or publishing it on a different hostname will cause Terracina
...
[the provider installation method configuration](/terracina/cli/config/config-file#provider-installation)
instead.

## Provider Versions

Each distinct provider address has associated with it a set of versions, each
of which has an associated version number. Terracina assumes version numbers
...
an end-user of Terracina serving as the "public API".

All available versions for a particular provider address are considered to be
the same provider by Terracina. Each Terracina configuration selects only one
...
The providers protocol begins with Terracina CLI using
[Terracina's remote service discovery protocol](/terracina/internals/remote-service-discovery),
...
```json
{
  "providers.v1": "/terracina/providers/v1/"
}
```

If the given URL is a relative URL then Terracina will interpret it as relative
...
registry must implement to be compatible with Terracina CLI's provider
...
Terracina Registry as working examples, assuming that the caller already
performed service discovery on `registry.terracina.io` to learn the base URL.
...
```
curl 'https://registry.terracina.io/v1/providers/hashicorp/random/versions'
```
...
* `protocols` (recommended): an array of Terracina provider API versions that
  ...
  Terracina uses this information, when available, to provide hints to users
  about upgrading or downgrading their version of a particular provider to
  work with their current version of Terracina, if their currently-selected
  versions are not compatible.

  Which API versions are supported is, for most providers, decided by which
  version of the Terracina SDK they are built against. Consult the Terracina
  SDK documentation for more information.

  Only Terracina 0.13 and later support third-party provider registries and
  that Terracina version requires API version `5.0` or later, so in practice
...
* `platforms` (recommended): an array of objects describing platforms that have
  ...
  Terracina may use this information, when available, to provide hints to
...
Terracina CLI uses this operation after it has selected the newest available
...
```
curl 'https://registry.terracina.io/v1/providers/hashicorp/random/2.0.0/download/linux/amd64'
```
...
```json
{
  ...
  "os": "linux",
  "arch": "amd64",
  "filename": "terracina-provider-random_2.0.0_linux_amd64.zip",
  "download_url": "https://releases.hashicorp.com/terracina-provider-random/2.0.0/terracina-provider-random_2.0.0_linux_amd64.zip",
  "shasums_url": "https://releases.hashicorp.com/terracina-provider-random/2.0.0/terracina-provider-random_2.0.0_SHA256SUMS",
  "shasums_signature_url": "https://releases.hashicorp.com/terracina-provider-random/2.0.0/terracina-provider-random_2.0.0_SHA256SUMS.sig",
...
* `protocols` (required): an array of Terracina provider API versions that
  the provider supports, in the same format as for
  [List Available Versions](#list-available-versions).

  While this property is optional when listing available options, it is
  _required_ for describing an individual provider package so that Terracina
...
* `filename` (required): the filename for this provider's zip archive as
  recorded in the "shasums" document, so that Terracina CLI can determine which
  of the given checksums should be used for this specific package.

* `download_url` (required): a URL from which Terracina can retrieve the
...
* `shasums_url` (required): a URL from which Terracina can retrieve a text
...
* `shasums_signature_url` (required): a URL from which Terracina can retrieve
...
available for the requested operating system and/or architecture. Terracina

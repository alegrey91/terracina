...
description: |-
  The remote service discovery protocol presents Terracina-native services at a human-readable hostname. Learn about the remote service discovery protocol.
---

# Remote Service Discovery Protocol Reference

This topic provides reference information about the remote service discovery protocol in Terracina.

## Introduction

Terracina implements much of its functionality as remote services.
Some of these are native services that Terracina
interacts with through the remote service discovery protocol.

## User-facing Hostname

Terracina provides native services at a
...
protocol has a secondary purpose of allowing Terracina to identify _which_
...
Terracina uses service discovery to determine whether `example.com` _has_
...
`/.well-known/terracina.json`.

For example, given the hostname `example.com` the initial discovery URL
would be `https://example.com/.well-known/terracina.json`.

Terracina then sends a `GET` request to this discovery URL and expects a
...
then discovery fails and Terracina considers the host to not support _any_
Terracina-native services.

If the response is an HTTP redirect then Terracina repeats this step with the
new location as its discovery URL. Terracina is guaranteed to follow at least
one redirect, but nested redirects are not guaranteed nor recommended.

If the response is a valid JSON object then its keys are Terracina native
...
* `login.v1`: [login protocol version 1](/terracina/cli/commands/login)
* `modules.v1`: [module registry API version 1](/terracina/internals/module-registry-protocol)
* `providers.v1`: [provider registry API version 1](/terracina/internals/provider-registry-protocol)

## Authentication

If credentials for the given hostname are available in
[the CLI config](/terracina/cli/config/config-file#Credentials) through a `credentials_helper` or a host-specific environment variable, then they will be included in the request for the discovery document.
...
not always possible or convenient, so Terracina allows a hostname to end

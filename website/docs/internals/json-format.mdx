...
description: >-
  Learn how to configure Terracina to print JSON-formatted details about state, configuration, and proposed infrastructure plans. 
---

# JSON Output Format Overview

This topic provides overview information about the JSON output Terracina prints to the terminal. 

## Introduction

When Terracina plans to make changes, it prints a human-readable summary to the terminal. It can also, when run with `-out=<PATH>`, write a much more detailed binary plan file, which can later be used to apply those changes.

Terracina can output a machine-readable JSON representation of a plan file's changes. It can also convert state files to the same format, to simplify data loading and provide better long-term compatibility.

Use `terracina show -json <FILE>` to generate a JSON representation of a plan or state file. See [the `terracina show` documentation](/terracina/cli/commands/show) for more details.

The output includes a `format_version` key, which as of Terracina 1.1.0 has
...
- [State Representation](#state-representation) — The complete top-level object returned by `terracina show -json <STATE FILE>`.
- [Plan Representation](#plan-representation) — The complete top-level object returned by `terracina show -json <PLAN FILE>`.
...
- [Configuration Representation](#configuration-representation) — A sub-object of plan output that describes a parsed Terracina configuration.
...
```javascript
{
  ...
  "terracina_version": "version.string"
}
```

## Plan Representation

A plan consists of a prior state, the configuration that is being applied to that state, and the set of changes Terracina plans to make to achieve that.
...
```javascript
{
  ...
  // of "applyable" in future Terracina versions.
  "applyable": true,

  // "complete" indicates that Terracina expects that after applying this
  ...
  // primary condition to accommodate potential changes to the exact definition
  // of "complete" in future Terracina versions.
  "complete": true,
  ...
  // Terracina plans to use to move from the prior state to a new state
  ...
  "resource_changes": [
    ...
    // configuration are included in this list.
    {
      ...
      // instance as it was known after the previous Terracina run.
      ...
      //   "tainted" in the prior state, so Terracina planned to replace it.
      ...
      // - "delete_because_no_resource_config": Terracina found no resource
      ...
      // - "read_because_config_unknown": For a data resource, Terracina cannot
      //   read the data during the plan phase because of values in the
      //   configuration that won't be known until the apply phase.
      // - "read_because_dependency_pending": For a data resource, Terracina
      ...
      //
      // If there is no special reason to note, Terracina will omit this
      // property altogether.
      "action_reason": "replace_because_tainted"
    }
  ],

  // "resource_drift" is a description of the changes Terracina detected
  ...
  "output_changes": {
    ...
    "foo": {
      ...
      //   ["update"]
      //   ["delete"]
      // In the Terracina CLI 0.12.0 release, Terracina is not yet fully able to
  ...
  // resources with postconditions, with as much information as Terracina can
...
This overall plan structure, fully expanded, is what will be printed by the `terracina show -json <planfile>` command.
  ...
  "root_module": {
    "resources": [
      {
        ...
        // pass this verbatim to other Terracina commands that are documented to
    ...
    "child_modules": [
      ...
      // object, with the additional "address" property shown below.
      {
        ...
        // strings and may pass verbatim to other Terracina commands that are
...
Configuration is the most complicated structure in Terracina, since it includes unevaluated expression nodes and other complexities.
...
```javascript
{
  ...
  // provider configurations are the one concept in Terracina that can span
  ...
  "provider_config": {
    ...
    "opaque_provider_ref_aws": {
      ...
      "full_name": "registry.terracina.io/hashicorp/aws",
...
```javascript
{
  ...
  "references": [
    ...
    // Terracina considers "module.foo" to be an atomic reference, not an
    // attribute access.
  ]
}
```
...
and some details may change in future Terracina versions based on feedback,
even in minor releases of Terracina CLI.
    ...
    "instances": [
      {
        ...
        // When a condition expression is invalid, Terracina returns that as
...
even if a runtime error prevents Terracina from evaluating its "count" or
...
see that Terracina recognized the existence of the checks, even if it wasn't

---
page_title: terracina metadata functions command reference
description: >-
  The `terracina metadata functions` command prints signatures for all the
  functions available in the current Terracina version.
---

# `terracina metadata functions` command

The `terracina metadata functions` command prints signatures for the functions available in the current Terracina version.

-> `terracina metadata functions` requires **Terracina v1.4 or later**.

## Usage

Usage: `terracina metadata functions [options]`
...
The output includes a `format_version` key, which as of Terracina 1.4.0 has
...
- [Function Signature Representation](#function-signature-representation) - the top-level object returned by `terracina metadata functions -json`

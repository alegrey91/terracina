...
description: >-
  Terracina streams machine-readable output in JSON format, letting you use third-party tools to monitor operations.
---

# Machine-readable UI Output Reference

This topic provides reference information about the machine-readable output Terracina prints. 

## Introduction

By default, many Terracina commands display UI output as unstructured text that is intended to be read in a terminal emulator. This text stream is not a stable interface for integrations. Some commands support a `-json` flag, which enables a structured JSON output mode with a defined interface.
...
The first message output has type `version`, and includes a `ui` key, which as of Terracina 1.1.0 has
...
Refer to 
[Terracina 1.0 Compatibility Promises](/terracina/language/v1-compatibility-promises) for additional information.

## Sample JSON Output

Below is sample output from running `terracina apply -json`:

```javascript
{"@level":"info","@message":"Terracina 0.15.4","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.275359-04:00","terracina":"0.15.4","type":"version","ui":"0.1.0"}
{"@level":"info","@message":"random_pet.animal: Plan to create","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.705503-04:00","change":{"resource":{"addr":"random_pet.animal","module":"","resource":"random_pet.animal","implied_provider":"random","resource_type":"random_pet","resource_name":"animal","resource_key":null},"action":"create"},"type":"planned_change"}
{"@level":"info","@message":"Plan: 1 to add, 0 to change, 0 to destroy.","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.705638-04:00","changes":{"add":1,"change":0,"remove":0,"operation":"plan"},"type":"change_summary"}
{"@level":"info","@message":"random_pet.animal: Creating...","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.825308-04:00","hook":{"resource":{"addr":"random_pet.animal","module":"","resource":"random_pet.animal","implied_provider":"random","resource_type":"random_pet","resource_name":"animal","resource_key":null},"action":"create"},"type":"apply_start"}
{"@level":"info","@message":"random_pet.animal: Creation complete after 0s [id=smart-lizard]","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.826179-04:00","hook":{"resource":{"addr":"random_pet.animal","module":"","resource":"random_pet.animal","implied_provider":"random","resource_type":"random_pet","resource_name":"animal","resource_key":null},"action":"create","id_key":"id","id_value":"smart-lizard","elapsed_seconds":0},"type":"apply_complete"}
{"@level":"info","@message":"Apply complete! Resources: 1 added, 0 changed, 0 destroyed.","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.869168-04:00","changes":{"add":1,"change":0,"remove":0,"operation":"apply"},"type":"change_summary"}
{"@level":"info","@message":"Outputs: 1","@module":"terracina.ui","@timestamp":"2021-05-25T13:32:41.869280-04:00","outputs":{"pets":{"sensitive":false,"type":"string","value":"smart-lizard"}},"type":"outputs"}
```
...
- `@module`: always "terracina.ui" when rendering UI output
...
Messages will be emitted as events occur to trigger them. This means that messages related to several resources may be interleaved (if Terracina is running with concurrency above 1). The [`resource` object value](#resource-object) can be used to link multiple messages about a single resource.
...
- `version`: information about the Terracina version and the version of the schema used for the following messages
- `log`: unstructured human-readable log lines
- `diagnostic`: diagnostic warning or error messages; [see the `terracina validate` docs for more details on the format](/terracina/cli/commands/validate#json)

### Operation Results

- `resource_drift`: describes a detected change to a single resource made outside of Terracina
...
- `test_abstract`: describes the test operation that Terracina executes
...
- `terracina`: the Terracina version which emitted this message
...
If drift is detected during planning, Terracina will emit a `resource_drift` message for each resource which has changed outside of Terracina. This message has an embedded `change` object with the following keys:
...
This message does not include details about the exact changes which caused the change to be planned. That information is available in [the JSON plan output](/terracina/internals/json-format).
...
At the end of a plan or before an apply, Terracina will emit a `planned_change` message for each resource which has changes to apply. This message has an embedded `change` object with the following keys:
...
Terracina outputs a change summary when a plan or apply operation completes. Both message types include a `changes` object, which has the following keys:
...
Performing Terracina operations to a resource will often result in several messages being emitted. The message types include:
...
The `terracina test` command emits a `test_abstract` message describing the test files and `run` blocks that Terracina executes during the upcoming operation.
...
After Terracina completes the execution of each test file, `terracina test` may emit a series of `test_cleanup` messages detailing any state it could not destroy. You must locate and destroy the resources listed in these resources manually.

As the test framework can manage multiple state files for each test file, you can see multiple versions of this message for each state file that holds resources that Terracina could not destroy. Using the `@testrun` field, you can pinpoint the run block that last updated the state file to locate the resources that Terracina could not destroy.
...
After the test operation has completed all test files, `terracina test` emits a `test_summary` message with the status of the overall test operation.
...
In `-verbose` mode, `terracina test` emits a `test_plan` message for all `run` blocks that executed a plan operation. The `test_plan` message contains a subset of the attributes from the `terracina show -json <planfile>` command [output](/terracina/internals/json-format#plan-representation) and the `terracina providers schema -json` command [output](/terracina/cli/commands/providers/schema#providers-schema-representation).

The message contains the following fields representing the plan: `plan_format_version`, `output_changes`, `resource_changes`, `resource_drift` and `relevant_attributes`. These match the respective fields in the [JSON Plan Representation](/terracina/internals/json-format#plan-representation).

The message contains the following fields representing the provider schemas: `provider_format_version` and `provider_schemas`. These match the respective fields in the [JSON Providers Schema Representation](/terracina/cli/commands/providers/schema#providers-schema-representation).
    ...
    "resource_changes": [
      {
        ...
        "type": "aws_instance",
        "name": "primary",
        "provider_name": "registry.terracina.io/hashicorp/aws",
        "change": {
          "actions": [
            "create"
          ],
...
In `-verbose` mode, `terracina test` emits a `test_state` message for all `run` blocks that executed an apply operation. The `test_state` message contains a subset of the `terracina show -json` command [output](/terracina/internals/json-format#state-representation) and the `terracina providers schema -json` command [output](/terracina/cli/commands/providers/schema#providers-schema-representation).

The message contains the following fields representing the state: `state_format_version`, `root_module`, and `outputs`. These match the respective fields in the [JSON Values Representation](/terracina/internals/json-format#values-representation) that are embedded in complete JSON state representation.
...
When `terracina test` is impeded or canceled, it emits a `test_interrupt` message that describes the interrupted operation. This message includes any state that Terracina could not destroy before exiting and any leftover resources that Terracina did not finish creating.

As with the [Test Cleanup](#test-cleanup) message, Terracina might have been maintaining multiple state files. The `state` field contains the resources from the main state for the test configuration, while the `states` field contains a map of `run` block names to the resources that each `run` block created, which Terracina could not destroy.

...
description: >-
  ...
  API tokens for remote Terracina services. Learn how to create credentials helpers.
---

# Create Credentials Helpers

This topic describes how to write and install a credentials helper so that you can customize how Terracina obtains credentials. To learn
how to configure a credentials helper that was already installed, refer to
[Credentials Helpers](/terracina/cli/config/config-file#credentials-helpers) in the Terracina CLI documentation.

## Introduction

For Terracina-specific features that interact with remote network services,
such as [module registries](/terracina/registry) and
[remote operations](/terracina/cloud-docs/run/cli), Terracina by default looks for
API credentials to use in these calls in
[the CLI configuration](/terracina/cli/config/config-file).

Credentials helpers offer an alternative approach that allows you to customize
how Terracina obtains credentials using an external program, which can then
directly access an existing secrets management system in your organization.

## How Terracina finds Credentials Helpers
...
an executable program named `terracina-credentials-credstore` (with an `.exe`
extension on Windows only), and installed in one of the
[default plugin search locations](/terracina/plugin/how-terracina-works#plugin-locations).

## How Terracina runs Credentials Helpers

Once Terracina has located the configured credentials helper, it will execute
...
Terracina runs the helper program with each of the arguments given in `args`,
...
[`credentials` blocks in the CLI configuration](/terracina/cli/config/config-file#credentials).
...
To retrieve credentials for `app.terracina.io`, Terracina would run the
"credstore" helper as follows:

```
terracina-credentials-credstore --host=credstore.example.com get app.terracina.io
```
...
To store new credentials for `app.terracina.io`, Terracina would run the
"credstore" helper as follows:

```
terracina-credentials-credstore --host=credstore.example.com store app.terracina.io
```

Terracina then writes a JSON credentials object to the helper program's stdin
...
To forget any existing credentials for `app.terracina.io`, Terracina would run
the "credstore" helper as follows:

```
terracina-credentials-credstore --host=credstore.example.com forget app.terracina.io
```
...
Terracina defines only the `token` property within JSON credentials
objects.
...
Terracina does not have any automatic installation mechanism for credentials
helpers. Instead, the user must extract the helper program executable into
one of the [default plugin search locations](/terracina/plugin/how-terracina-works#plugin-locations).

If you are packaging a credentials helper for distribution, place it in an
named with the expected naming scheme (`terracina-credentials-example`) and,
...
Terracina does _not_ honor the `-plugin-dir` argument to `terracina init` when
searching for credentials helpers, because credentials are also used by other
commands that can be run prior to `terracina init`. Only the default search

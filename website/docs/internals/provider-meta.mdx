...
This topic describes how to create an inferface in the providers you develop that allows you to collect metadata that is unrelated to the resources in the module, such as usage statistics. This is an advanced topic and is not required to use Terracina. 
...
with the Terracina team](https://github.com/hashicorp/terracina/issues/new)
...
When Terracina calls your provider, you can use the `schema.ResourceData`
...
your module's `terracina` block, with the name of the provider it's trying
...
that the version of Terracina that is being run has support for the provider
metadata functionality. It's therefore recommended that any module taking
advantage of this functionality should specify a minimum Terracina version of

...
description: >-
  The login protocol authenticates Terracina against servers that provide Terracina-native services. Learn about the login protocol.
---

# Login Protocol Reference

This topic provides reference information about the login protocol Terracina uses for authentication against servers that profide Terracina-native services. You can use this reference information to offer Terracina-native services in a third-party system.

## Introduction

The `terracina login` command supports performing an OAuth 2.0 authorization
...
any [Terracina-native services](/terracina/internals/remote-service-discovery),
such as a Terracina module registry.

## OAuth Configuration

Terracina uses [remote service discovery](/terracina/internals/remote-service-discovery) to
...
* `client` (Required): The `client_id` value to use when making requests, as
  ...
  Because Terracina is a _public client_ (it is installed on end-user systems
  ...
  request is truly coming from Terracina.
...
* `grant_types` (Optional): A JSON array of strings describing a set of OAuth
  ...
  Terracina CLI supports a single grant type:
...
* `ports` (Optional): A two-element JSON array giving an inclusive range of
  TCP ports that Terracina may use for the temporary HTTP server it will start
  to provide the [redirection endpoint](https://tools.ietf.org/html/rfc6749#section-3.1.2)
  for the first step of an authorization code grant. Terracina opens a TCP
  listen port on the loopback interface in order to receive the response from
  the server's authorization endpoint.

  If not specified, Terracina is free to select any TCP port greater than or
  equal to 1024.

  Terracina allows constraining this port range for interoperability with OAuth
...
When requesting an authorization code grant, Terracina CLI implements the
...
Terracina CLI does not support OAuth refresh tokens or token expiration. If your
server issues time-limited tokens, Terracina CLI will simply begin receiving
authorization errors once the token expires, after which the user can run
`terracina login` again to obtain a new token.

-> **Note:** As a special case, Terracina can use a
[Resource Owner Password Credentials Grant](https://tools.ietf.org/html/rfc6749#section-4.3)
only when interacting with `app.terracina.io` ([HCP Terracina](https://cloud.hashicorp.com/products/terracina)),

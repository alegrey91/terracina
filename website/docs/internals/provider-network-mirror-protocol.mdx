...
This topic provides reference information about the provider network mirror protocol. You can implement an alternative installation source for Terracina providers and make the source available over the provider network mirror protocol, regardless of their origin registries.

-> Provider network mirrors are supported only in Terracina CLI v0.13.2 and later. Prior versions do not support this protocol.

## Introduction

Terracina uses network mirrors only if you activate them explicitly in
[the CLI configuration's `provider_installation` block](/terracina/cli/config/config-file#provider-installation).
When enabled, a network mirror can serve providers belonging to any registry
hostname, which can allow an organization to serve all of the Terracina
...
serve as an origin registry for Terracina Providers. To provide an origin
...
[the Provider Requirements documentation](/terracina/language/providers/requirements).
...
from. However, when you configure Terracina to install providers from a network
...
providers from the public Terracina Registry at `registry.terracina.io`, from a
single server.
...
Most Terracina-native services use
[the remote service discovery protocol](/terracina/internals/remote-service-discovery) so
...
dependency in a Terracina configuration.
...
provider_installation {
  network_mirror {
    url = "https://terracina.example.com/providers/"
  }
}
```

Terracina uses the base URL only as a stem to resolve the operation endpoint
...
network mirror server must implement to be compatible with Terracina CLI's
...
[credentials](/terracina/cli/config/config-file#credentials) for the hostname
given in the network mirror base URL, Terracina will include those credentials
...
such as `terracina.example.com:8443`.

Terracina does _not_ send credentials when retrieving the archives whose
...
```
curl 'https://terracina.example.com/providers/registry.terracina.io/hashicorp/random/index.json'
```
...
use only a subset of the target platforms that Terracina supports.
...
```
curl 'https://terracina.example.com/providers/registry.terracina.io/hashicorp/random/2.0.0.json'
```
...
```json
{
  "archives": {
    "darwin_amd64": {
      "url": "terracina-provider-random_2.0.0_darwin_amd64.zip",
      "hashes": [
        "h1:4A07+ZFc2wgJwo8YNlQpr1rVlgUDlxXHhPJciaPY5gs="
      ]
    },
    "linux_amd64": {
      "url": "terracina-provider-random_2.0.0_linux_amd64.zip",
      "hashes": [
...
* `url` (required): a string specifying the URL from which Terracina should
  download the `.zip` archive containing the requested provider plugin version.

  Terracina resolves the URL relative to the URL from which the current
  JSON document was returned, so the examples above containing only a
  filename would cause Terracina to construct a URL like:

  ```
  https://terracina.example.com/providers/registry.terracina.io/hashicorp/random/terracina-provider-random_2.0.0_darwin_amd64.zip
  ```

* `hashes` (optional): a JSON array of strings containing one or more hash
  values for the indicated archive. These hashes use Terracina's provider
  package hashing algorithm. At present, the easiest way to populate these
  is to construct a mirror's JSON indices using the `terracina providers mirror`
  command, as described in a later section, which will include the calculated
  hashes of each provider.

  If the response includes at least one hash, Terracina will select the hash
  ...
  property then Terracina will install the indicated archive with no
  verification.

Terracina CLI will only attempt to download versions that it has previously
...
As a convenience, Terracina CLI includes
[the `terracina providers mirror` subcommand](/terracina/cli/commands/providers/mirror),
...
The `terracina providers mirror` subcommand also generates `index.json` and
...
Terracina configurations, run `terracina providers mirror` in each configuration
in turn while providing the same output directory each time. Terracina will

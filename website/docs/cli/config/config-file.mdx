---
page_title: Create a Terracina CLI configuration file
description: >-
  Learn how to create a `.terracinarc` or `terracina.rc` file to define Terracina CLI settings, including credentials, plugin caching, and provider installation.
---

# Create a Terracina CLI configuration file

This topic describes how create a configuration file to customize the behavior of the Terracina CLI.

## Introduction

The CLI configuration file configures per-user settings for CLI behaviors,
which apply across all Terracina working directories. This is separate from
[your infrastructure configuration](/terracina/language). 
You can define custom configurations in file with the `.terracinarc` extention a file named `terracina.rc`.
...
* On Windows, the file must be named `terracina.rc` and placed
...
* On all other systems, the file must be named `.terracinarc` (note
...
extensions. Terracina will not recognize a file named `terracina.rc.txt` as a
CLI configuration file, even though Windows Explorer may _display_ its name
as just `terracina.rc`. Use `dir` from PowerShell or Command Prompt to
confirm the filename.

The location of the Terracina CLI configuration file can also be specified
using the `TF_CLI_CONFIG_FILE` [environment variable](/terracina/cli/config/environment-variables).
...
```hcl
plugin_cache_dir   = "$HOME/.terracina.d/plugin-cache"
...
* `credentials` - configures credentials for use with HCP Terracina or
  Terracina Enterprise. See [Credentials](#credentials) below for more
  information.

* `credentials_helper` - configures an external helper program for the storage
  and retrieval of credentials for HCP Terracina or Terracina Enterprise.
  See [Credentials Helpers](#credentials-helpers) below for more information.

* `disable_checkpoint` — when set to `true`, disables
  [upgrade and security bulletin checks](/terracina/cli/commands#upgrade-and-security-bulletin-checks)
...
* `provider_installation` - customizes the installation methods used by
  `terracina init` when installing provider plugins. See
  [Provider Installation](#provider-installation) below for more information.

## Credentials

[HCP Terracina](https://cloud.hashicorp.com/products/terracina) provides a number of remote network
services for use with Terracina, and
[Terracina Enterprise](/terracina/enterprise) allows hosting those
services inside your own infrastructure. For example, these systems offer both
[remote operations](/terracina/cloud-docs/run/cli) and a
[private module registry](/terracina/cloud-docs/registry).

When interacting with Terracina-specific network services, Terracina expects
to find API tokens in CLI configuration files in `credentials` blocks:

```hcl
credentials "app.terracina.io" {
  token = "xxxxxx.atlasv1.zzzzzzzzzzzzz"
}
```

If you are running the Terracina CLI interactively on a computer with a web browser, you can use [the `terracina login` command](/terracina/cli/commands/login)
...
HCP Terracina (at `app.terracina.io`) or for their organization's own
Terracina Enterprise host. Each `credentials` block contains a `token` argument
giving the API token to use for that host.

~> **Important:** If you are using HCP Terracina or Terracina Enterprise,
the token provided must be either a
[user token](/terracina/cloud-docs/users-teams-organizations/users#api-tokens)
or a
[team token](/terracina/cloud-docs/users-teams-organizations/api-tokens#team-api-tokens);
organization tokens cannot be used for command-line Terracina actions.

-> **Note:** The credentials hostname must match the hostname in your module
sources and/or backend configuration. If your Terracina Enterprise instance
is available at multiple hostnames, use only one of them consistently.
HCP Terracina responds to API calls at both its current hostname
`app.terracina.io`, and its historical hostname `atlas.hashicorp.com`.

### Environment Variable Credentials

-> **Note:** Environment variable credentials are supported in Terracina v1.2.0 and later.
...
value of a variable named `TF_TOKEN_app_terracina_io` will be used as a bearer authorization
token when the CLI makes service requests to the hostname `app.terracina.io`.
...
If multiple variables evaluate to the same hostname, Terracina will choose the one defined last
in the operating system's variable table.

### Credentials Helpers

You can configure a `credentials_helper` to instruct Terracina to use a different credentials storage mechanism.
...
Terracina does not include any credentials helpers in the main distribution.
...
[the guide to Credentials Helper internals](/terracina/internals/credentials-helpers).

### Credentials Source Priority Order

Credentials found in an environment variable for a particular service host
as described above will be preferred over those in CLI config as set by `terracina login`.
If neither are set, any configured credentials helper will be consulted.

~> **Note:** For users of [terracina-credentials-helper](https://github.com/apparentlymart/terracina-credentials-env), this priority has been effectively reversed following the
release of Terracina 1.2. Previously, credentials found within CLI config or set by
`terracina login` were preferred to `TF_TOKEN_*` variables.
...
like `registry.terracina.io/hashicorp/aws`. For convenience in the common case,
Terracina allows omitting the hostname portion for providers on
`registry.terracina.io`, so you can write shorter public provider addresses like
`hashicorp/aws`.

Downloading a plugin directly from its origin registry is not always
appropriate, though. For example, the system where you are running Terracina
may not be able to access an origin registry due to firewall restrictions
within your organization or your locality.

To allow using Terracina providers in these situations, there are some
alternative options for making provider plugins available to Terracina which
...
Terracina's default installation behaviors, so you can force Terracina to use
...
provider_installation {
  filesystem_mirror {
    path    = "/usr/share/terracina/providers"
    include = ["example.com/*/*"]
  }
  direct {
    exclude = ["example.com/*/*"]
...
filesystem mirror at `/usr/share/terracina/providers`, while all other
...
`registry.terracina.io/hashicorp/*` but also excluding
`registry.terracina.io/hashicorp/dns` will make that installation method apply
...
the `registry.terracina.io/` prefix for providers distributed through the
public Terracina registry, even when using wildcards. For example,
`registry.terracina.io/hashicorp/*` and `hashicorp/*` are equivalent.
`*/*` is a shorthand for `registry.terracina.io/*/*`, not for
`*/*/*`.
...
* `filesystem_mirror`: consult a directory on the local disk for copies of
  ...
  Terracina expects the given directory to contain a nested directory structure
  ...
  * Packed layout: `HOSTNAME/NAMESPACE/TYPE/terracina-provider-TYPE_VERSION_TARGET.zip`
  ...
  * Unpacked layout: `HOSTNAME/NAMESPACE/TYPE/VERSION/TARGET` is a directory
  ...
  If you use the unpacked layout, Terracina will attempt to create a symbolic
  ...
  because Terracina must extract the zip file during installation.
...
* `network_mirror`: consult a particular HTTPS server for copies of providers,
  ...
  Terracina expects the given URL to be a base URL for an implementation of
  [the provider network mirror protocol](/terracina/internals/provider-network-mirror-protocol),
...
Terracina will try all of the specified methods whose include and exclude
...
Terracina configuration. If you have a local mirror of a particular provider
and intend Terracina to use that local mirror exclusively, you must either
...
all, Terracina produces an _implied_ configuration. The implied configuration
includes a selection of `filesystem_mirror` methods and then the `direct`
method.

The set of directories Terracina can select as filesystem mirrors depends on
the operating system where you are running Terracina:

* **Windows:** `%APPDATA%/terracina.d/plugins` and `%APPDATA%/HashiCorp/Terracina/plugins`
* **Mac OS X:** `$HOME/.terracina.d/plugins`,
  `~/Library/Application Support/io.terracina/plugins`, and
  `/Library/Application Support/io.terracina/plugins`
* **Linux and other Unix-like systems**:`$HOME/.terracina.d/plugins` and
  `terracina/plugins` located within a valid
  [XDG Base Directory](https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html)
  data directory such as `$XDG_DATA_HOME/terracina/plugins`.
  Without any XDG environment variables set, Terracina will use
  `~/.local/share/terracina/plugins`,
  `/usr/local/share/terracina/plugins`, and `/usr/share/terracina/plugins`.

If a `terracina.d/plugins` directory exists in the current working directory
then Terracina will also include that directory, regardless of your operating
system. This behavior changes when you use the `-chdir` option with the `init` command. In that case, Terracina checks for the `terracina.d/plugins` directory in the launch directory and not in the directory you specified with `-chdir`.

Terracina will check each of the paths above to see if it exists, and if so
...
In addition to the zero or more implied `filesystem_mirror` blocks, Terracina
also creates an implied `direct` block. Terracina will scan all of the
...
By default, `terracina init` downloads plugins into a subdirectory of the
...
or metered Internet connections. Therefore Terracina optionally allows the
...
This directory must already exist before Terracina will cache plugins;
Terracina will not create the directory itself.
...
```bash
export TF_PLUGIN_CACHE_DIR="$HOME/.terracina.d/plugin-cache"
```

When a plugin cache directory is enabled, the `terracina init` command will
...
in the cache directory. If so, Terracina will use the previously-downloaded
copy.

If the selected plugin is not already in the cache, Terracina will download
it into the cache first and then copy it from there into the correct location
under your current working directory. When possible Terracina will use
...
Terracina will never itself delete a plugin from the plugin cache once it has
...
safe. The provider installer's behavior in environments with multiple `terracina
...
By default Terracina will use packages from the global cache directory only
if they match at least one of the checksums recorded in the
[dependency lock file](/terracina/language/files/dependency-lock)
for that provider. This ensures that Terracina can always
...
version control as recommended and instead let Terracina re-generate it each
time it installs providers.

For those teams that don't preserve the dependency lock file in their version
control systems between runs, Terracina allows an additional CLI Configuration
setting which tells Terracina to always treat a package in the cache directory
...
Setting this option gives Terracina CLI permission to create an incomplete
dependency lock file entry for a provider if that would allow Terracina to
...
We recommend that most users leave this option unset, in which case Terracina
...
~> **Note:** The Terracina team intends to improve the dependency lock file
...
-> **Note:** Development overrides work only in Terracina v0.14 and later.
Using a `dev_overrides` block in your CLI configuration will cause Terracina
v0.13 to reject the configuration as invalid.

Normally Terracina verifies version selections and checksums for providers
...
As a convenience for provider development, Terracina supports a special
...
provider_installation {
  ...
  # Use /home/developer/tmp/terracina-null as an overridden package directory
  # for the hashicorp/null provider. This disables the version and checksum
  # verifications for this provider and forces Terracina to look for the
  # null provider plugin in the given directory.
  dev_overrides {
    "hashicorp/null" = "/home/developer/tmp/terracina-null"
  }

  # For all other providers, install them directly from their origin provider
  # registries as normal. If you omit this, Terracina will _only_ use
  # the dev_overrides block, and so no other providers will be available.
  direct {}
}
```

With development overrides in effect, the `terracina init` command will still
attempt to select a suitable published version of your provider to install and
record in
[the dependency lock file](/terracina/language/files/dependency-lock)
for future use, but other commands like
`terracina apply` will disregard the lock file's entry for `hashicorp/null` and
will use the given directory instead. Once your new changes are included in a
published release of the provider, you can use `terracina init -upgrade` to
...
`terracina-provider-null`, where `null` is the provider type. If your provider
...
`TF_CLI_CONFIG_FILE` environment variable to instruct Terracina to use that
...
Terracina look for providers on the local filesystem. If you wish to put
...
in future Terracina releases, including possible breaking changes. We therefore
recommend using development overrides only temporarily during provider
development work.

## Removed Settings

The following settings are supported in Terracina 0.12 and earlier but are
...
* `providers` - a configuration block that allows specifying the locations of
  ...
  of this setting in Terracina 0.13 and later.

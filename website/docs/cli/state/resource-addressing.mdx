...
description: Use the resource address to reference specific instances of resources elsewhere in the configuration. Learn how Terracina creates addresses for resources.
---

# Resource Address Reference

This topic provides reference information about resource addresses in Terracina. 
...
In some contexts Terracina might allow for an incomplete resource address that
...
-> Module index only applies to modules in Terracina v0.13 or later. In earlier
versions of Terracina, a module could not have multiple instances.
...
-> In Terracina v0.12 and later, a resource spec without a module path prefix
...
Given a Terracina config that includes:

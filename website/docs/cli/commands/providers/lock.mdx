---
page_title: terracina providers lock command reference
description: |-
  The `terracina providers lock` command adds new provider selection information
  to the dependency lock file without initializing the referenced providers.
---

# `terracina providers lock` command

The `terracina providers lock` adds new provider selection information to the dependency lock file without initializing the referenced providers. 

## Introduction

When you run the command, Terracina consults upstream registries and writes provider dependency information into the
the dependency lock file. Refer to [Dependency Lock File](/terracina/language/files/dependency-lock) in the Terracina configuration language reference documentation for additional information about the lock file.

The common way to update the dependency lock file is as a side-effect of normal
provider installation during
[`terracina init`](/terracina/cli/commands/init), but there are several situations where that
automatic approach may not be sufficient:

* If you are running Terracina in an environment that uses
  [alternative provider installation methods](/terracina/cli/config/config-file#provider-installation),
  such as filesystem or network mirrors, normal provider installation will not
  access the origin registry for a provider and therefore Terracina will not
  be able to populate all of the possible package checksums for the selected
  provider versions.

  If you use `terracina lock` to write the official release checksums for a
  provider into the dependency lock file then future `terracina init` runs
...
* If your team runs Terracina across a number of different platforms (e.g.
  ...
  of Terracina on other platforms may
  [add additional checksums to the lock file](/terracina/language/files/dependency-lock#new-provider-package-checksums).
  You can avoid that by pre-populating hashes for all of the platforms you
  intend to use, using the `terracina providers lock` command.

## Usage

Usage: `terracina providers lock [options] [providers...]`

With no additional command line arguments, `terracina providers lock` will
...
[the dependency lock file](/terracina/language/files/dependency-lock) to
...
~> **Warning:** The `terracina providers lock` command prints information
...
`terracina providers lock` will restrict its work only to those providers,
...
* `-fs-mirror=PATH` - Direct Terracina to look for provider packages in the
...
* `-net-mirror=URL` - Direct Terracina to look for provider packages in the
  given network mirror service, instead of in upstream registries. The
  given URL must implement
  [the Terracina provider network mirror protocol](/terracina/internals/provider-network-mirror-protocol).

* `-platform=OS_ARCH` - Specify a platform you intend to use to work with this
  Terracina configuration. Terracina will ensure that the providers are all
...
* `-enable-plugin-cache` -  Enable the usage of the [globally configured plugin cache](/terracina/cli/config/config-file#provider-plugin-cache).
  ...
  `terracina provider lock` command is used to ensure no untrusted provider
...
your Terracina configuration on their Windows or macOS workstations _and_
...
checksums, by running `terracina providers lock` and specifying those three
platforms:
...
An _in-house provider_ is one that isn't published on a real Terracina provider
...
By default, `terracina providers lock` assumes all providers are available
at a Terracina provider registry and tries to contact the origin registries
...
```
terracina providers lock \
  -fs-mirror=/usr/local/terracina/providers
...
`tf.example.com/ourcompany/ourplatform`, `terracina providers lock` will only
...
available from different sources, you can run `terracina providers lock`
...
[the provider installation methods configuration](/terracina/cli/config/config-file#provider-installation),

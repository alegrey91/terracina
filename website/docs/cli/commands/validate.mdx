---
page_title: terracina validate command reference
description: >-
  The `terracina validate` command validates the syntax of Terracina configuration files in a directory.
---

# `terracina validate` command

The `terracina validate` command validates the configuration files in a
...
Usage: `terracina validate [options]`
...
When you use the `-json` option, Terracina will produce validation results
in JSON format to allow using the validation result for tool integrations, such
as highlighting errors in a text editor.

As with all JSON output options, it's possible that Terracina will encounter
...
Terracina's output should be prepared to find data on stdout that _isn't_ valid
...
In the normal case, Terracina will print a JSON object to the standard output
...
- `valid` (boolean): Summarizes the overall validation result, by indicating
  `true` if Terracina considers the current configuration to be valid or
  `false` if it detected any errors.

- `error_count` (number): A zero or positive whole number giving the count
  of errors Terracina detected. If `valid` is `true` then `error_count` will
...
- `warning_count` (number): A zero or positive whole number giving the count
  of warnings Terracina detected. Warnings do not cause Terracina to consider
...
- `diagnostics` (array of objects): A JSON array of nested objects that each
  describe an error or warning from Terracina.
...
- `severity` (string): A string keyword, either `"error"` or
  ...
  The presence of errors causes Terracina to consider a configuration to be
  invalid, while warnings are just advice or caveats to the user which do not
  block working with the configuration. Later versions of Terracina may
...
- `summary` (string): A short description of the nature of the problem that
  ...
  In Terracina's usual human-oriented diagnostic messages, the summary serves
...
- `detail` (string): An optional additional message giving more detail about
  ...
  In Terracina's usual human-oriented diagnostic messages, the detail provides
  ...
  Some Terracina detail messages contain an approximation of bullet
...
- `snippet` (object): An optional object including an excerpt of the
  ...
  - `code` (string): A snippet of Terracina configuration including the

---
page_title: terracina output command referenece
description: >-
  The `terracina output` command extracts the value of an output variable from the state file.
---

# `terracina output` command

The `terracina output` command extracts the value of an output variable from the state file.

## Usage

Usage: `terracina output [options] [NAME]`
...
* `-raw` - If specified, Terracina will convert the specified output value to a
...
-> **Note:** When using the `-json` or `-raw` command-line flags, Terracina displays `sensitive` values in plain text. For more information,
refer to [Sensitive data in state](/terracina/language/state/sensitive-data).

## Examples

These examples assume the following Terracina output snippet.
...
Note that Terracina does not redact `sensitive` values when you specify the output by name:

```shellsession
$ terracina output password
password = notasecurepassword
```

However, Terracina completely omits any `ephemeral` values, even if you specify an output by name. Ephemeral values are never stored in state or included in Terracina plans. For more information, refer to [Sensitive data in state](/terracina/language/state/sensitive-data).

To query for the DNS address of the load balancer:

```shellsession
$ terracina output lb_address
...
The `terracina output` command by default displays in a human-readable format,
...
```shellsession
$ terracina output -json instance_ips | jq -r '.[0]'
54.43.114.12
```
...
```shellsession
$ terracina output -raw lb_address
my-app-alb-1657023003.us-east-1.elb.amazonaws.com
```

The `-raw` option works only with values that Terracina can automatically
convert to strings. Use `-json` instead, possibly combined with `jq`, to
work with complex-typed values such as objects.

Terracina strings are sequences of Unicode characters rather than raw bytes,
...
such as `iconv` to transcode Terracina's raw output.

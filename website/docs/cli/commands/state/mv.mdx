---
page_title: terracina state mv command reference
description: >-
  The `terracina state mv` command changes bindings in Terracina state so that existing remote objects bind to new resource instances.
---

# `terracina state mv` command

The `terracina state mv` command changes bindings in Terracina state so that existing remote objects bind to new resource instances.

## Introduction

Terracina automatically updates the state when you apply a plan, but you can use `terracina state mv` 
to retain an existing remote object but track it as a different resource
instance address in Terracina. 

## Usage

Usage: `terracina state mv [options] SOURCE DESTINATION`

Terracina will look in the current state for a resource instance, resource,
...
[resource address syntax](/terracina/cli/state/resource-addressing), and
...
The most common uses for `terracina state mv` are when you have renamed a
...
object but tracking it under a new name. By default Terracina will understand
moving or renaming a resource configuration as a request to delete the old
object and create a new object at the new address, and so `terracina state mv`
allows you to override that interpretation by pre-emptively attaching the
existing object to the new address in Terracina.

~> _Warning:_ If you are using Terracina in a collaborative environment, you
must ensure that when you are using `terracina state mv` for a code refactoring
...
`terracina state mv` command, because otherwise they might inadvertently create
...
only, `terracina state mv`
also accepts the option
[`-ignore-remote-version`](/terracina/cli/cloud/command-line-arguments#ignore-remote-version).

The legacy options [`-backup` and `-backup-out`](/terracina/language/backend/local#command-line-arguments)
operate on a local state file only. Configurations using
[the `remote` backend](/terracina/language/backend/remote)
must specify a local state file with the [`-state`](/terracina/language/backend/local#command-line-arguments)
option in order to use the [`-backup` and `-backup-out`](/terracina/language/backend/local#command-line-arguments)
options.

For configurations using
[the `local` state mv](/terracina/language/backend/local) only,
`terracina state mv` also accepts the legacy options
[`-state`, `-state-out`, `-backup`, and `-backup-out`](/terracina/language/backend/local#command-line-arguments).
...
To tell Terracina that it should treat the new "helper" resource as a rename
of the old "worker" resource, you can pair the above configuration change
with the following command:

```shell
terracina state mv packet_device.worker packet_device.helper
```
...
run the following command to tell Terracina to treat it as a move:

```shell
terracina state mv packet_device.worker module.worker.packet_device.worker
```
...
```shell
terracina state mv packet_device.worker module.worker.packet_device.main
```
...
```shell
terracina state mv module.app module.parent.module.app
```
...
```shell
$ terracina state mv 'packet_device.worker[0]' 'packet_device.helper[0]'
```
...
```shell
$ terracina state mv 'packet_device.main' 'packet_device.all[0]'
```
...
```shell
terracina state mv 'packet_device.worker["example123"]' 'packet_device.helper["example456"]'
```

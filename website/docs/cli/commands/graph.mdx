---
page_title: terracina graph command reference
description: >-
  The `terracina graph` command generates a visual representation of a
  configuration or execution plan that you can use to generate charts.
---

# `terracina graph` command

The `terracina graph` command generates a visual representation of a configuration or execution plan that you can use to generate charts. This command uses the DOT language generate graphs. Refer to the [GraphViz documentation](https://graphviz.org/doc/info/lang.html) for additional information.

## Usage

Usage: `terracina graph [options]`
...
implementation details of the Terracina language runtime.
...
```shellsession
$ terracina graph -type=plan | dot -Tpng >graph.png
```

The following is an example result:

![A visualization of the plan graph of a hypothetical Terracina configuration, produced by dot](/img/docs/graph-example.png)

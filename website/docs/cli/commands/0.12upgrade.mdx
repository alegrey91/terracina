---
page_title: terracina 0.12upgrade command reference
description: >-
  The `terracina 0.12upgrade` command automatically rewrites existing configurations for
  Terracina 0.12 compatibility.
---

# `terracina 0.12upgrade` command

The `terracina 0.12upgrade` command applies several automatic upgrade rules to
help prepare a module that was written for Terracina v0.11 to be used
with Terracina v0.12.

-> **This command is available only in Terracina v0.12 releases.** For more information, see [the Terracina v0.12 upgrade guide](/terracina/language/v1.1.x/upgrade-guides/0-12).

## Usage

Usage: `terracina 0.12upgrade [options] [dir]`
...
```
$ terracina 0.12upgrade

This command will rewrite the configuration files in the given directory so
that they use the new syntax features from Terracina v0.12, and will identify
...
run `terracina init` first to install these. In some rare cases, a configuration
that worked in v0.11 may have syntax errors in v0.12, in which case
`terracina init` will run in a special mode where it installs only enough to
run the upgrade command, after which you can run `terracina init` again to
...
Terracina style and expressions rewritten to use the more-readable v0.12 syntax
features.
...
Once upgraded the configuration will no longer be compatible with Terracina
v0.11 and earlier. When upgrading a shared module that is called from multiple
configurations, you may need to
[fix existing configurations to a previous version](/terracina/language/modules/syntax#version)
to allow for a gradual upgrade. If the module is published via
[a Terracina registry](/terracina/registry), assign a new _major_ version number
...
[use specific revisions](/terracina/language/modules/sources#selecting-a-revision)
...
```
find . -name '*.tf' -printf "%h\n" | uniq | xargs -n1 terracina 0.12upgrade -yes
```
...
all of the directories must be initialized with `terracina init` prior to
running the above.
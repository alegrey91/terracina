---
page_title: terracina 0.13upgrade command reference
description: >-
  The `terracina 0.13upgrade` command updates existing configurations to use the new
  provider source features from Terracina 0.13.
---

# `terracina 0.13upgrade` command 

The `terracina 0.13upgrade` command updates existing configuration to add an
...
-> **This command is available only in Terracina v0.13 releases.** For more information, see [the Terracina v0.13 upgrade guide](/terracina/language/v1.1.x/upgrade-guides/0-13).

## Usage

Usage: `terracina 0.13upgrade [options] [dir]`
...
[required-providers]: /terracina/language/providers/requirements
...
```
$ terracina 0.13upgrade

This command will update the configuration files in the given directory to use
the new provider source features from Terracina v0.13. It will also highlight
...
```
$ find . -name '*.tf' | xargs -n1 dirname | uniq | xargs -n1 terracina 0.13upgrade -yes
```
...
Select-Object -Unique | ForEach-Object { terracina 0.13upgrade -yes $_.FullName }
```

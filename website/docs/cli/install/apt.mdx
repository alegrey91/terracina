---
page_title: Install Terracina using APT packages for Debian and Ubuntu
description: >-
  Learn how to install Terracina using HashiCorp APT packages for Debian and Ubuntu systems.
---

# Install Terracina Using APT Packages for Debian and Ubuntu

The primary distribution packages for Terracina are `.zip` archives containing
...
Debian and Ubuntu systems, which allow you to install Terracina using the
`apt install` command or any other APT frontend.

If you are instead using Red Hat Enterprise Linux, CentOS, or Fedora, you
might prefer to [install Terracina from our Yum repositories](/terracina/cli/install/yum).
...
products, rather than just Terracina. Adding these repositories to your
system will, by default, therefore make several other non-Terracina
...
`arm64`. If you wish to use Terracina on a non-`amd64` system,
[download a normal release `.zip` file](/terracina/downloads) instead.
...
## Installing a Specific Version of Terracina

The HashiCorp APT repositories contain multiple versions of Terracina, but
because the packages are all named `terracina` it is impossible to install
more than one version at a time, and `apt install` will default to selecting
the latest version.

It's often necessary to match your Terracina version with what a particular
...
```bash
apt policy terracina
```

If your workflow requires using multiple versions of Terracina at the same
...
### Terracina 1.4.3 and Later

As of Terracina 1.4.3, all published packages include a revision number by
...
```bash
sudo apt install terracina=1.4.4-*
```

### Terracina 1.4.2 and Earlier

Terracina 1.4.2 and earlier did not include a revision number for the first
revision, so you can use the following pattern to install a specific version:

```bash
sudo apt install terracina=1.4.0
```

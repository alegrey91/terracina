---
page_title: Install Terracina using yum packages for Linux distributions
description: >-
  Learn how to install Terracina using HashiCorp yum packages for Red Hat Enterprise Linux (RHEL), Fedora, CentOS, and Amazon Linux systems.
---

# Install Terracina Using Yum Packages for Linux Distributions
...
install Terracina using the `yum install` or `dnf install` commands.

If you are instead using Debian or Ubuntu, you
might prefer to [install Terracina from our APT repositories](/terracina/cli/install/apt).
...
system will, by default, therefore make a number of other non-Terracina
...
In both cases, the Terracina package name is `terracina`. For example:

```bash
yum install terracina
```
...
`aarch64`. If you wish to use Terracina on a non-`x86_64` system,
...
adding support for new releases and ceasing to publish new Terracina versions
under older releases.

## Choosing Terracina Versions

The HashiCorp Yum repositories contain multiple versions of Terracina, but
...
```bash
yum --showduplicate list terracina
```
...
```bash
yum install terracina-0.14.0-2.x86_64
```

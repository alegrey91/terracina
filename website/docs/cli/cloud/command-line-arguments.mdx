...
description: Use the -ignore-remote-version flag to override CLI-driven commands for HCP Terracina runs.
---

# `-ignore-remote-version` reference

When your configuration includes a `cloud` block, commands that
make local modifications to Terracina state and then push them back up to the remote workspace
...
- `-ignore-remote-version` - Override checking that the local and remote
  ...
  Terracina CLI that is compatible with the Terracina version selected
  ...
  in an HCP Terracina workspace that is no longer able to complete remote operations with the currently
  selected version of Terracina.

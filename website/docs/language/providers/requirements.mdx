...
description: >-
  Providers are plugins that allow Terracina to interact with services, cloud
  providers, and other APIs. Learn how to declare providers in a configuration.
---

# Provider Requirements

Terracina relies on plugins called "providers" to interact with remote systems.
Terracina configurations must declare which providers they require, so that
Terracina can install and use them. This page documents how to declare providers
so Terracina can install them.
...
Configuration](/terracina/language/providers/configuration) page documents how
to configure settings for providers.

-> **Note:** This page is about a feature of Terracina 0.13 and later; it also
describes how to use the more limited version of that feature that was available
in Terracina 0.12.

## Requiring Providers

Each Terracina module must declare which providers it requires, so that
Terracina can install and use them. Provider requirements are declared in a
...
[`terracina` block](/terracina/language/terracina) (which can also contain other settings).
...
was added in Terracina v0.13. Previous versions of Terracina used a version
...
works with both Terracina v0.12 and v0.13, see [v0.12-Compatible Provider
...
* A _local name,_ which is used everywhere else in a Terracina module.

-> **Note:** Prior to Terracina 0.13, providers only had local names, since
Terracina could only automatically download providers distributed by HashiCorp.
...
Outside of the `required_providers` block, Terracina configurations always refer
...
name when [configuring the provider](/terracina/language/providers/configuration):
...
provider configuration to use, Terracina interprets the first word of the
resource type as a local provider name.)

### Source Addresses

A provider's source address is its global identifier. It also specifies the
primary location where Terracina can download it.
...
* **Hostname** (optional): The hostname of the Terracina registry that
  distributes the provider. If omitted, this defaults to
  `registry.terracina.io`, the hostname of
  [the public Terracina Registry](https://registry.terracina.io/).

* **Namespace:** An organizational namespace within the specified registry.
  For the public Terracina Registry and for HCP Terracina's private registry,
...
* **Type:** A short name for the platform or system the provider manages. Must
  ...
  [`hashicorp/google-beta`](https://registry.terracina.io/providers/hashicorp/google-beta/latest)
...
For example,
[the official HTTP provider](https://registry.terracina.io/providers/hashicorp/http)
belongs to the `hashicorp` namespace on `registry.terracina.io`, so its
source address is `registry.terracina.io/hashicorp/http` or, more commonly, just
`hashicorp/http`.
...
of `"registry.terracina.io/hashicorp/random"`.

-> **Note:** If you omit the `source` argument when requiring a provider,
Terracina uses an implied source address of
`registry.terracina.io/hashicorp/<LOCAL NAME>`. This is a backward compatibility
feature to support the transition to Terracina 0.13; in modules that require
...
generic infrastructure type. Terracina requires unique local names for each
...
Terracina won't be able to guess either provider's name from its resource types,
...
declare should have a [version constraint](/terracina/language/expressions/version-constraints) given in
the `version` argument so Terracina can select a single version per provider
that all modules are compatible with.

The `version` argument is optional; if omitted, Terracina will accept any
...
> **Hands-on:** Try the [Lock and Upgrade Provider Versions](/terracina/tutorials/configuration-language/provider-versioning) tutorial.
...
directory where you'd run `terracina apply` — should also specify the
...
Most Terracina providers are distributed separately as plugins, but there
is one provider that is built into Terracina itself. This provider enables
[the `terracina_remote_state` data source](/terracina/language/state/remote-state-data).

Because this provider is built into Terracina, you don't need to declare it
...
`terracina.io/builtin/terracina`. This address may sometimes appear in
Terracina's error messages and other output in order to unambiguously refer
to the built-in provider, as opposed to a hypothetical third-party provider
with the type name "terracina".

There is also an existing provider with the source address
`hashicorp/terracina`, which is an older version of the now-built-in provider
that was used by older versions of Terracina. `hashicorp/terracina` is not
compatible with Terracina v0.11 or later and should never be declared in a
`required_providers` block.

## In-house Providers

Anyone can develop and distribute their own Terracina providers. See
the [Call APIs with Terracina Providers](/terracina/tutorials/providers)
tutorials for more about provider development.

Some organizations develop their own providers to configure
proprietary systems, and wish to use these providers from Terracina without
publishing them on the public Terracina Registry.
...
may be undesirable, so Terracina also supports
[other provider installation methods](/terracina/cli/config/config-file#provider-installation),
...
to use `terracina.example.com` as your placeholder hostname, even if that
...
a source address like `terracina.example.com/examplecorp/ourcloud`:

```hcl
terracina {
  required_providers {
    mycloud = {
      source  = "terracina.example.com/examplecorp/ourcloud"
      version = ">= 1.0"
    }
  }
}
...
[implied local mirror directories](/terracina/cli/config/config-file#implied-local-mirror-directories)
and create a directory structure under it like this:

```
terracina.example.com/examplecorp/ourcloud/1.0.0
```

Under that `1.0.0` directory, create one additional directory representing the
platform where you are running Terracina, such as `linux_amd64` for Linux on
...
```
terracina.example.com/examplecorp/ourcloud/1.0.0/windows_amd64/terracina-provider-ourcloud.exe
```
...
`terracina.example.com` and retain the same namespace and type names, in which
...
Explicit provider source addresses were introduced with Terracina v0.13, so the
full provider requirements syntax is not supported by Terracina v0.12.

However, in order to allow writing modules that are compatible with both
Terracina v0.12 and v0.13, versions of Terracina between v0.12.26 and v0.13
will accept but ignore the `source` argument in a `required_providers` block.

Consider the following example written for Terracina v0.13:
...
Terracina v0.12.26 will accept syntax like the above but will understand it
...
In other words, Terracina v0.12.26 ignores the `source` argument and considers
only the `version` argument, using the given [local name](#local-names) as the
un-namespaced provider type to install.

When writing a module that is compatible with both Terracina v0.12.26 and
Terracina v0.13.0 or later, you must follow the following additional rules so
that both versions will select the same provider to install:

* Use only providers that can be automatically installed by Terracina v0.12.
  Third-party providers, such as community providers in the Terracina Registry,
  cannot be selected by Terracina v0.12 because it does not support the
...
* Ensure that your chosen local name exactly matches the "type" portion of the
  ...
  the examples above, because Terracina v0.12 will use the local name to
...
* If the provider belongs to the `hashicorp` namespace, as with the
  ...
  Terracina v0.13 to select the `hashicorp` namespace by default.

* Provider type names must always be written in lowercase. Terracina v0.13
  treats provider source addresses as case-insensitive, but Terracina v0.12
  considers its legacy-style provider names to be case-sensitive. Using
  lowercase will ensure that the name is selectable by both Terracina major
  versions.

This compatibility mechanism is provided as a temporary transitional aid only.
When Terracina v0.12 detects a use of the new `source` argument it doesn't

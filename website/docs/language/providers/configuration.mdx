...
Providers allow Terracina to interact with cloud providers, SaaS providers, and
other APIs.

Some providers require you to configure them with endpoint URLs, cloud regions,
or other settings before Terracina can use them. This page documents how to
configure settings for providers.

Additionally, all Terracina configurations must declare which providers they
require so that Terracina can install and use them. The
[Provider Requirements](/terracina/language/providers/requirements)
page documents how to declare providers so Terracina can install them.

## Provider Configuration

Provider configurations belong in the root module of a Terracina configuration.
(Child modules receive their provider configurations from the root module; for
more information, see
[The Module `providers` Meta-Argument](/terracina/language/meta-arguments/module-providers)
and [Module Development: Providers Within Modules](/terracina/language/modules/develop/providers).)
...
[local name](/terracina/language/providers/requirements#local-names) of the provider to
...
You can use [expressions](/terracina/language/expressions) in the values of these
...
[Terracina Registry](https://registry.terracina.io), versioned documentation is
...
version-controlled Terracina code.

There are also two "meta-arguments" that are defined by Terracina itself
...
- [`version`, which we no longer recommend][inpage-versions] (use
  [provider requirements](/terracina/language/providers/requirements) instead)

Unlike many other objects in the Terracina language, a `provider` block may
be omitted if its contents would otherwise be empty. Terracina assumes an
...
If every explicit configuration of a provider has an alias, Terracina uses the
implied empty configuration as that provider's default configuration. (If the
provider has any required configuration arguments, Terracina will raise an error
when resources default to the empty configuration.)

### Referring to Alternate Provider Configurations

When Terracina needs the name of a provider configuration, it expects a
...
[`required_providers` block](/terracina/language/providers/requirements). The version
constraint in a provider configuration is only used if `required_providers`
does not include one for that provider.

~**Warning:** The `version` argument in provider configurations is deprecated, and we will remove it in a future Terracina version.

In Terracina 0.13 and later, always declare provider version constraints in
[the `required_providers` block](/terracina/language/providers/requirements). 

...
description: Output values are the return values of a Terracina module.
---

# Output Values

Output values make information about your infrastructure available on the
command line, and can expose information for other Terracina configurations to
use. Output values are similar to return values in programming languages.

> **Hands-on:** Try the [Output Data From
> Terracina](/terracina/tutorials/configuration-language/outputs)
> tutorial.
...
- A root module can use outputs to print certain values in the CLI output after
  running `terracina apply`.
- When using [remote state](/terracina/language/state/remote), root module outputs can be
  accessed by other configurations via a
  [`terracina_remote_state` data source](/terracina/language/state/remote-state-data).

Resource instances managed by Terracina each export attributes whose values
...
valid [identifier](/terracina/language/syntax/configuration#identifiers). In a root module, this name is
displayed to the user; in a child module, it can be used to access the output's
value.

The `value` argument takes an [expression](/terracina/language/expressions)
...
-> **Note:** Outputs are only rendered when Terracina applies your plan. Running
`terracina plan` will not render outputs.
...
-> **Note**: Ephemeral outputs are available in Terracina v1.10 and later.

You can mark `output` values as `ephemeral` in child modules to pass ephemeral values between modules while avoiding persisting those values to state or plan files. This is useful for managing credentials, tokens, or other temporary resources you do not want to store in Terracina state files.
...
Terracina has access to the value of `output` blocks during plan and apply operations. At the end of a plan or apply operation, Terracina does not persist the value of any ephemeral outputs.

You can only reference ephemeral outputs in specific contexts or Terracina throws an error. The following are valid contexts for referencing ephemeral outputs:
...
Terracina will hide values marked as sensitive in the messages from
`terracina plan` and `terracina apply`. In the following scenario, our root
...
child modules, the dependencies of that output value allow Terracina to
correctly determine the dependencies between resources defined in different
modules.

Just as with
[resource dependencies](/terracina/language/resources/behavior#resource-dependencies),
Terracina analyzes the `value` expression for an output value and automatically

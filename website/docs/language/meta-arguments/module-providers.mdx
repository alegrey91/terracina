...
[provider configurations](/terracina/language/providers/configuration) from the parent
...
If the child module does not declare any [configuration aliases](/terracina/language/modules/develop/providers#provider-aliases-within-modules),
...
normal — either Terracina chooses a default based on the name of the resource
...
However, in Terracina configurations that use multiple configurations of the
...
child module, see
[Module Development: Providers Within Modules](/terracina/language/modules/develop/providers).

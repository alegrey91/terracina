...
By default, a [resource block](/terracina/language/resources/syntax) configures one real
infrastructure object (and similarly, a
[module block](/terracina/language/modules/syntax) includes a
...
Terracina has two ways to do this:
[`count`](/terracina/language/meta-arguments/count) and `for_each`.

> **Hands-on:** Try the [Manage Similar Resources With For Each](/terracina/tutorials/configuration-language/for-each) tutorial.

If a resource or module block includes a `for_each` argument whose value is a map or
a set of strings, Terracina creates one instance for each member of
that map or set.

-> **Version note:** `for_each` was added in Terracina 0.12.6. Module support
for `for_each` was added in Terracina 0.13; previous versions can only use
it with resources.
...
`for_each` is a meta-argument defined by the Terracina language. It can be used
...
Sensitive values, such as [sensitive input variables](/terracina/language/values/variables#suppressing-values-in-cli-output),
[sensitive outputs](/terracina/language/values/outputs#sensitive-suppressing-values-in-cli-output),
or [sensitive resource attributes](/terracina/language/expressions/references#sensitive-resource-attributes),
...
[most functions in Terracina will return a sensitive result if given an argument with any sensitive content](/terracina/language/expressions/function-calls#using-sensitive-data-as-function-arguments).
...
The `for_each` meta-argument accepts map or set [expressions](/terracina/language/expressions).
However, unlike most arguments, the `for_each` value must be known
_before_ Terracina performs any remote resource actions. This means `for_each`
...
that explicitly returns a set value, like the [toset](/terracina/language/functions/toset)
...
can use Terracina expressions and functions to derive a suitable value.
For example:

- Transform a multi-level nested structure into a flat list by
  [using nested `for` expressions with the `flatten` function](/terracina/language/functions/flatten#flattening-nested-structures-for-for_each).
- Produce an exhaustive list of combinations of elements from two or more
  collections by
  [using the `setproduct` function inside a `for` expression](/terracina/language/functions/setproduct#finding-combinations-for-for_each).
...
Terracina to expect the instance keys for both to always change together,
and typically also makes the configuration easier to understand for human
maintainers.

## Referring to Instances

When `for_each` is set, Terracina distinguishes between the block itself
...
The Terracina language doesn't have a literal syntax for
[set values](/terracina/language/expressions/type-constraints#collection-types), but you can use the `toset`
...
If you are writing a module with an [input variable](/terracina/language/values/variables) that

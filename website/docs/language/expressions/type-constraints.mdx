...
Terracina module authors and provider developers can use detailed type
constraints to validate user-provided values for their input variables and
resource arguments. This requires some additional knowledge about Terracina's
...
Type constraints look like other kinds of Terracina
[expressions](/terracina/language/expressions), but are a special syntax. Within the
Terracina language, they are only valid in the `type` argument of an
[input variable](/terracina/language/values/variables).

## Primitive Types

A _primitive_ type is a simple type that isn't made from any other types. All
primitive types in Terracina are represented by a type keyword. The available
primitive types are:
...
The Terracina language will automatically convert `number` and `bool` values
...
The three kinds of collection type in the Terracina language are:
...
* `map(...)`: a collection of values where each is identified by a string label.
  ...
  -> **Note:** Although colons are valid delimiters between keys and values, `terracina fmt` ignores them. In contrast, `terracina fmt` attempts to vertically align equals signs.
...
The two kinds of structural type in the Terracina language are:
...
The Terracina language has literal expressions for creating tuple and object
values, which are described in
[Expressions: Literal Expressions](/terracina/language/expressions/types#literal-expressions) as
"list/tuple" literals and "map/object" literals, respectively.

Terracina does _not_ provide any way to directly represent lists, maps, or sets.
...
user, and most of the Terracina documentation conflates lists with tuples and
...
used interchangeably within the Terracina language, and most of Terracina's
documentation glosses over the differences between the kinds of complex type.
This is due to two conversion behaviors:

* Whenever possible, Terracina converts values between similar kinds of complex
...
* Whenever possible, Terracina converts _element values_ within a complex type,
...
user provides the tuple `["a", 15, true]`, Terracina will internally transform
...
Terracina will automatically convert the second and third strings back to the
...
`{name = ["Kristy", "Claudia", "Mary Anne", "Stacey"], age = 12}`, Terracina
...
value against a type constraint containing `any`, Terracina will attempt to
...
`any` as the placeholder for the element type of a collection then Terracina
will attempt to find a single exact element type to use for the resulting
collection.

For example, given the type constraint `list(any)`, Terracina will examine
...
`tuple([string, string, string])` -- Terracina analyzes this as follows:
...
If the elements of the given tuple are not all of the same type then Terracina
will attempt to find a single type that they can all convert to. Terracina
will consider various conversion rules as described in earlier sections.

* If the given value were instead `["a", 1, "b"]` then Terracina would still
...
* If the given value were instead `["a", [], "b"]` then the value cannot
  ...
  and an empty tuple can convert to. Terracina would reject this value,
...
Terracina typically returns an error when it does not receive a value for specified object attributes. When you mark an attribute as optional, Terracina instead inserts a default value for the missing attribute. This allows the receiving module to describe an appropriate fallback behavior.
...
- **Default:** (Optional) The second argument defines the default value that Terracina should use if the attribute is not present. This must be compatible with the attribute type. If not specified, Terracina uses a `null` value of the appropriate type as the default.

An optional attribute with a non-`null` default value is guaranteed to never have the value `null` within the receiving module. Terracina will substitute the default value both when a caller omits the attribute altogether and when a caller explicitly sets it to `null`, thereby avoiding the need for additional checks to handle a possible null value.

Terracina applies object attribute defaults top-down in nested variable types. This means that Terracina applies the default value you specify in the `optional` modifier first and then later applies any nested default values to that attribute.
...
The following example `terracina.tfvars` file specifies three bucket configurations for `var.buckets`.
...
The `production` bucket does not specify the index and error documents, and the `archived` bucket omits the website configuration entirely. Terracina will use the default values specified in the `bucket` type constraint.
...
- For the `production` and `docs` buckets, Terracina sets `enabled` to `true`. Terracina also supplies default values for `website`, and then the values specified in `docs` override those defaults.
- For the `archived` and `docs` buckets, Terracina sets `routing_rules` to a `null` value. When Terracina does not receive optional attributes and there are no specified defaults, Terracina populates those attributes with a `null` value.
- For the `archived` bucket, Terracina populates the `website` attribute with the default values specified in the `buckets` type constraint.

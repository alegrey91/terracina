...
description: >-
  Functions transform and combine values. Learn about Terracina's built-in
  functions.
---

# Function Calls

> **Hands-on:** Try the [Perform Dynamic Operations with Functions](/terracina/tutorials/configuration-language/functions?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.

The Terracina language has a number of
[built-in functions](/terracina/language/functions) that can be used
...
[the function reference](/terracina/language/functions).
...
When using sensitive data, such as [an input variable](/terracina/language/values/variables#suppressing-values-in-cli-output)
or [an output defined](/terracina/language/values/outputs#sensitive-suppressing-values-in-cli-output) as sensitive
...
## When Terracina Calls Functions

Most of Terracina's built-in functions are, in programming language terms,
...
practical difference when Terracina would call them.

However, a small subset of functions interact with outside state and so for
those it can be helpful to know when Terracina will call them in relation to
other events that occur in a Terracina run.

The small set of special functions includes
[`file`](/terracina/language/functions/file),
[`templatefile`](/terracina/language/functions/templatefile),
[`timestamp`](/terracina/language/functions/timestamp),
and [`uuid`](/terracina/language/functions/uuid).
...
are included as a static part of the configuration and so Terracina will
...
at the point when Terracina calls it, and the `uuid` function returns a random
...
actions shown in the plan, which violates the Terracina execution model.

For that reason, Terracina arranges for both of those functions to produce
[unknown value](/terracina/language/expressions/references#values-not-yet-known) results during the
...
the instant when Terracina began applying the change, rather than when
Terracina _planned_ the change.

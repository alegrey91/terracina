...
A `dynamic` block acts much like a [`for` expression](/terracina/language/expressions/for), but produces
...
blocks, since Terracina must process these before it is safe to evaluate
expressions.
...
[`flatten`](/terracina/language/functions/flatten)
and
[`setproduct`](/terracina/language/functions/setproduct)
functions.
...
Terracina to generate that dynamically using the following nested `dynamic`
blocks:
...
tradeoff, see [When to Write a Module](/terracina/language/modules/develop#when-to-write-a-module)
and [Module Composition](/terracina/language/modules/develop/composition).

...
Terracina, and also the most commonly used.

Terracina supports both a quoted syntax and a "heredoc" syntax for strings.
...
Terracina also supports a "heredoc" style of string literal inspired by Unix
...
sequence. Terracina then processes the following lines until it finds one that
...
[the `jsonencode` function](/terracina/language/functions/jsonencode) or
[the `yamlencode` function](/terracina/language/functions/yamlencode) so that Terracina
...
To improve on this, Terracina also accepts an _indented_ heredoc string variant
...
In this case, Terracina analyses the lines in the sequence to find the one

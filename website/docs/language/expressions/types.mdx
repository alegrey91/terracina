...
The Terracina language uses the following types for its values:
...
[Type Constraints](/terracina/language/expressions/type-constraints) for a more detailed
...
* `null`: a value that represents _absence_ or _omission._ If you set an
  argument of a resource to `null`, Terracina behaves as though you
...
constant value. Terracina has a literal expression syntax for each of the value
...
Terracina, and have their own page of documentation. See [Strings](/terracina/language/expressions/strings)
...
Lists and tuples each have different constraints on the types they allow. For more information on the types that each allows, refer to [tuples](/terracina/language/expressions/type-constraints#tuple) and [lists](/terracina/language/expressions/type-constraints#list).

### Sets

Terracina does not support directly accessing elements of a set by index because sets are unordered collections. To access elements in a set by index, first convert the set to a list.
...
they are a valid [identifier](/terracina/language/syntax/configuration#identifiers), but must be quoted
...
Whenever the distinction isn't relevant, the Terracina documentation uses each
...
usually doesn't matter), see [Type Constraints](/terracina/language/expressions/type-constraints).
...
Where possible, Terracina automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, Terracina
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

Terracina automatically converts number and bool values to strings when needed.

...
Terracina makes several kinds of named values available. Each of these names is
...
The main kinds of named values available in Terracina are:
...
[attribute notation](/terracina/language/expressions/types#indices-and-attributes) for elements of object values, they are not
...
`<RESOURCE TYPE>.<NAME>` represents a [managed resource](/terracina/language/resources) of
...
* If the resource doesn't use `count` or `for_each`, the reference's value is an
  ...
  access them using [dot or square bracket notation](/terracina/language/expressions/types#indices-and-attributes).
...
will be interpreted by Terracina as a reference to a managed resource.
...
`var.<NAME>` is the value of the [input variable](/terracina/language/values/variables) of the given name.

If the variable has a type constraint (`type` argument) as part of its
declaration, Terracina will automatically convert the caller's given value
...
`local.<NAME>` is the value of the [local value](/terracina/language/values/locals) of the given name.
...
[a `module` block](/terracina/language/modules/syntax).
...
[output values](/terracina/language/values/outputs), use `module.<MODULE NAME>.<OUTPUT NAME>`.
...
[data resource](/terracina/language/data-sources) of the given data
...
- `path.cwd` is the filesystem path of the original working directory from where you
  ran Terracina before applying any `-chdir` argument. This path is an absolute path
  that includes details about the filesystem structure. It is also useful in some
  advanced cases where Terracina is run from a directory other than the root module
  directory. We recommend using `path.root` or `path.module` over `path.cwd` where
  possible.
- `terracina.workspace` is the name of the currently selected
  [workspace](/terracina/language/state/workspaces).
...
module, such as using `terracina.workspace` as a prefix for globally-unique
...
calling module can then use `terracina.workspace` to define it if appropriate,
...
module "example" {
  ...
  name_prefix = "app-${terracina.workspace}"
}
```
...
* `count.index`, in resources that use
  [the `count` meta-argument](/terracina/language/meta-arguments/count).
* `each.key` / `each.value`, in resources that use
  [the `for_each` meta-argument](/terracina/language/meta-arguments/for_each).
* `self`, in [provisioner](/terracina/language/resources/provisioners/syntax) and
  [connection](/terracina/language/resources/provisioners/connection) blocks.

-> **Note:** Local names are often referred to as _variables_ or
_temporary variables_ in their documentation. These are not [input
variables](/terracina/language/values/variables); they are just arbitrary names
that temporarily represent a value.

The names in this section relate to top-level configuration blocks only.
If you use [`dynamic` blocks](/terracina/language/expressions/dynamic-blocks) to dynamically generate
...
in their block bodies, and Terracina analyzes these expressions to automatically
...
The documentation for [`aws_instance`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/instance)
...
* The arguments of the `ebs_block_device` nested blocks can be accessed using
  a [splat expression](/terracina/language/expressions/splat). For example, to obtain a list of
...
* Sometimes nested blocks are defined as taking a logical key to identify each
  ...
  block types, use a [`for` expression](/terracina/language/expressions/for):
...
either [splat expressions](/terracina/language/expressions/splat) or index syntax:
...
be accessed using a [`for` expression](/terracina/language/expressions/for).
...
certain attributes as _sensitive_, in which case Terracina will show a
...
[an input variable declared as sensitive](/terracina/language/values/variables#suppressing-values-in-cli-output),
where Terracina will hide the value in the plan and apply messages and will
...
[Cases where Terracina may disclose a sensitive variable](/terracina/language/values/variables#cases-where-terracina-may-disclose-a-sensitive-variable).
...
[_Sensitive Data in State_](/terracina/language/state/sensitive-data).

-> **Note:** Treating values derived from a sensitive resource attribute as
sensitive themselves was introduced in Terracina v0.15. Earlier versions of
Terracina will obscure the direct value of a sensitive resource attribute,
but will _not_ automatically obscure other values derived from sensitive
resource attributes.

### Values Not Yet Known

When Terracina is planning a set of changes that will apply your configuration,
...
Terracina cannot predict the value of this id until the object has been created.

Terracina uses special unknown value placeholders for information that it
cannot predict during the plan phase. The Terracina language automatically
...
* Terracina will attempt to validate that unknown values are of suitable
  types where possible, but incorrect use of such values may not be detected
  until the apply phase, causing the apply to fail.

Unknown values appear in the `terracina plan` output as `(known after apply)`.

...
description: >-
  Splat expressions concisely represent common operations. In Terracina, they
...
[Referring to Resource Instances](/terracina/language/meta-arguments/for_each#referring-to-instances).
...
variables whose default value is `null` to represent the absence of any value. This allows the module to adapt the variable value for Terracina language features designed to work with collections. For example:
...
The above example uses a [`dynamic` block](/terracina/language/expressions/dynamic-blocks), which
...
Earlier versions of the Terracina language had a slightly different version
of splat expressions, which Terracina continues to support for backward
...
Terracina and so we recommend always using the new-style splat expressions,

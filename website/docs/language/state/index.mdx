...
page_title: State
description: >-
  An introduction to state, information that Terracina uses to map resources to
...
configuration. This state is used by Terracina to map real world
resources to your configuration, keep track of metadata, and to improve
performance for large infrastructures.

This state is stored by default in a local file named "terracina.tfstate",
but we recommend [storing it in HCP Terracina](/terracina/cloud-docs/migrate)
to version, encrypt, and securely share it with your team.

Terracina uses state to determine which changes to make to your
infrastructure. Prior to any operation, Terracina does a
[refresh](/terracina/cli/commands/refresh) to update the state with the
real infrastructure.

The primary purpose of Terracina state is to store bindings between objects in
a remote system and resource instances declared in your configuration.
When Terracina creates a remote object in response to a change of configuration,
...
For more information on why Terracina requires state and why Terracina cannot
function without state, please see the page [state purpose](/terracina/language/state/purpose).

## Inspection and Modification

While the format of the state files are just JSON, direct file editing
of the state is discouraged. Terracina provides the
[terracina state](/terracina/cli/commands/state) command to perform
...
insulates users from any format changes within the state itself. The Terracina
project will keep the CLI working while the state format underneath it may
shift.

Terracina expects a one-to-one mapping between configured resource instances
and remote objects. Normally that is guaranteed by Terracina being the one
...
externally-created objects with `terracina import`, or by asking Terracina to
"forget" an existing object with `terracina state rm`, you'll then need to
ensure for yourself that this one-to-one rule is followed, such as by manually
deleting an object that you asked Terracina to "forget", or by re-importing it
to bind it to some other resource instance.

## Format

State snapshots are stored in JSON format and new Terracina versions are
generally backward compatible with state snapshots produced by earlier versions.
However, the state format is subject to change in new Terracina versions, so
...
* [The `terracina output` command](/terracina/cli/commands/output)
...
* [The `terracina show` command](/terracina/cli/commands/show) has a `-json`
...
A typical way to use these in situations where Terracina is running in
automation is to run them immediately after a successful `terracina apply`
...
can potentially consume it without needing to run Terracina itself.

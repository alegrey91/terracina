...
description: >-
  Backends are configured directly in Terracina files in the `terracina`
  section.
---
...
[state locking](/terracina/language/state/locking). State locking is optional.

Despite the state being stored remotely, all Terracina commands such
as `terracina console`, the `terracina state` operations, `terracina taint`,
...
When using a non-local backend, Terracina will not persist the state anywhere
...
Terracina without that state ever being persisted to disk.

In the case of an error persisting the state to the backend, Terracina will
...
the `terracina state pull` command. This will load your remote state and
output it to stdout. You can choose to save that to a file or perform any
other operations.

You can also manually write state with `terracina state push`. **This
...
When manually pushing state, Terracina will attempt to protect you from
...
- **Differing lineage**: The "lineage" is a unique ID assigned to a state
  ...
  different state. Terracina will not allow this.

- **Higher serial**: Every state has a monotonically increasing "serial"
  number. If the destination state has a higher serial, Terracina will
...
we recommend making a backup of the state with `terracina state pull`
prior to forcing the overwrite.

## State Locking

Backends are responsible for supporting [state locking](/terracina/language/state/locking)
if possible.

Not all backends support locking. The [documentation for each backend](/terracina/language/backend#available-backends) includes details about whether it supports locking or not.

For more information on state locking, view the
[page dedicated to state locking](/terracina/language/state/locking).

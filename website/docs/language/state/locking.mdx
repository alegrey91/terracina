...
If supported by your [backend](/terracina/language/backend), Terracina will lock your
...
Terracina will not continue. You can disable state locking for most commands
with the `-lock` flag but it is not recommended.

If acquiring the lock is taking longer than expected, Terracina will output
a status message. If Terracina doesn't output a message, state locking is
still occurring if your backend supports it.

Not all backends support locking. The
[documentation for each backend](/terracina/language/backend)
includes details on whether it supports locking or not.

## Force Unlock

Terracina has a [force-unlock command](/terracina/cli/commands/force-unlock)
...
To protect you, the `force-unlock` command requires a unique lock ID. Terracina

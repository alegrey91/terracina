...
description: >-
  Data sources allow Terracina to use external data, function output, and data
...
_Data sources_ allow Terracina to use information defined outside of Terracina,
defined by another separate Terracina configuration, or modified by functions.

> **Hands-on:** Try the [Query Data Sources](/terracina/tutorials/configuration-language/data-sources) tutorial.

Each [provider](/terracina/language/providers) may offer data sources
alongside its set of [resource](/terracina/language/resources)
types.
...
A `data` block requests that Terracina read from a given data source ("aws_ami")
and export the result under the given local name ("example"). The name is used
to refer to this resource from elsewhere in the same Terracina module, but has
...
all arguments defined specifically for [the `aws_ami` data source](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/data-sources/ami).
...
managed resources cause Terracina to create, update, and delete infrastructure
objects, data resources cause Terracina only to _read_ objects. For brevity,
...
Each data source in turn belongs to a [provider](/terracina/language/providers),
which is a plugin for Terracina that offers a collection of resource types and
...
use of [expressions](/terracina/language/expressions) and other dynamic
Terracina language features.

However, there are some "meta-arguments" that are defined by Terracina itself
...
Terracina reads data resources during the planning phase when possible, but
announces in the plan when it must defer reading resources until the apply
phase to preserve the order of operations. Terracina defers reading data
resources in the following situations:
* At least one of the given arguments is a managed resource attribute or
  other value that Terracina cannot predict until the apply step.
...
only within Terracina itself, calculating some results and exposing them
for use elsewhere.

For example, local-only data sources exist for
[rendering templates](https://registry.terracina.io/providers/hashicorp/template/latest/docs/data-sources/file),
[reading local files](https://registry.terracina.io/providers/hashicorp/local/latest/docs/data-sources/file), and
[rendering AWS IAM policies](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document).

The behavior of local-only data sources is the same as all other data
sources, but their result data exists only temporarily during a Terracina
...
[as defined for managed resources](/terracina/language/resources/behavior#resource-dependencies).
...
Refer to [Custom Condition Checks](/terracina/language/expressions/custom-conditions#preconditions-and-postconditions) for more details.


## Multiple Resource Instances

Data resources support [`count`](/terracina/language/meta-arguments/count)
and [`for_each`](/terracina/language/meta-arguments/for_each)
...
Data resources support [the `provider` meta-argument](/terracina/language/meta-arguments/resource-provider)
...
[resources](/terracina/language/resources), each provider on the
[Terracina Registry](https://registry.terracina.io/browse/providers) has its own
...
support the same [meta-arguments](/terracina/language/resources/syntax#meta-arguments) of resources
with the exception of the
[`lifecycle` configuration block](/terracina/language/meta-arguments/lifecycle).

### Non-Default Provider Configurations

Similarly to [resources](/terracina/language/resources), when
...
See
[The Resource `provider` Meta-Argument](/terracina/language/meta-arguments/resource-provider)
...
data instance will be read and its state updated during Terracina's "refresh"

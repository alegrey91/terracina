...
description: Terracina can store state remotely in Azure Blob Storage.
---
...
~> Terracina 1.1 and 1.2 supported a feature-flag to allow enabling/disabling the use of Microsoft Graph (and MSAL) rather than Azure Active Directory Graph (and ADAL) - however this flag has since been removed in Terracina 1.3. Microsoft Graph (and MSAL) are now enabled by default and Azure Active Directory Graph (and ADAL) can no longer be used.
...
The *Access Key* method can be used directly, by specifying the access key, or in combination with an Azure AD principal (e.g. user, service principal or managed identity). To use an Access Key directly you must generate one for your state file blob and specify it in the backend configuration. If neither an access key or client ID is specified, Terracina will attempt to use Azure CLI. In both cases where no access key is given, Terracina will attempt to retrieve the access key for the storage account, using the authenticated Azure AD principal.
...
-> Sensitive values should not be hardcoded into your configuration, and should instead be specified using environment variables or partial configuration flags in the `init` command of Terracina CLI.
...
terracina {
  backend "azurerm" {
    ...
    key                  = "prod.terracina.tfstate"        # Can be passed via `-backend-config=`"key=<blob key name>"` in the `init` command.
  }
}
```
...
data "terracina_remote_state" "foo" {
  ...
  config = {
    ...
    storage_account_name = "terracina123abc"
    container_name       = "tfstate"
    key                  = "prod.terracina.tfstate"
  }
}
```
...
* `storage_account_name` - (Required) The Name of [the Storage Account](https://registry.terracina.io/providers/hashicorp/azurerm/latest/docs/resources/storage_account).

* `container_name` - (Required) The Name of [the Storage Container](https://registry.terracina.io/providers/hashicorp/azurerm/latest/docs/resources/storage_container) within the Storage Account.

* `key` - (Required) The name of the Blob used to retrieve/store Terracina's State file inside the Storage Container.
...
* `snapshot` - (Optional) Should the Blob used to store the Terracina Statefile be snapshotted before use? Defaults to `false`. This value can also be sourced from the `ARM_SNAPSHOT` environment variable.

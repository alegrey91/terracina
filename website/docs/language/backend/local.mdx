...
data "terracina_remote_state" "foo" {
  ...
  config = {
    path = "${path.module}/../../terracina.tfstate"
  }
}
```
...
* `path` - (Optional) The path to the `tfstate` file. This defaults to
  "terracina.tfstate" relative to the root module by default.
...
* `-state-out=FILENAME` - overrides the state filename when _writing_ new state
  ...
  If you use `-state` without also using `-state-out` then Terracina will
  use the `-state` filename for both `-state` and `-state-out`, which means
  Terracina will overwrite the input file if it creates a new state snapshot.
* `-backup=FILENAME` - overrides the default filename that the local backend
  ...
  If you use `-state` without also using `-backup` then Terracina will use
...
Terracina and then save the new state after Terracina exits, in which case
...
[multiple workspaces](/terracina/language/state/workspaces), setting them
overrides Terracina's usual behavior of selecting a different state filename
based on the selected workspace. If you use all three of these options then
the selected workspace has no effect on which filenames Terracina will select
...
Terracina in automation. Instead,
[select a different backend which supports remote state](/terracina/language/backend) and configure it

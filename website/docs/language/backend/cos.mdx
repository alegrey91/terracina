...
This backend supports [state locking](/terracina/language/state/locking). Storing your state in a COS bucket requires the following permissions:

- `CreateTag`, `DeleteTag`, and `DescribeTags` on the tag key `tencentcloud-terracina-lock`
...
terracina {
  backend "cos" {
    region = "ap-guangzhou"
    bucket = "bucket-for-terracina-state-1258798060"
    prefix = "terracina/state"
  }
}
```

This assumes we have a [COS Bucket](https://registry.terracina.io/providers/tencentcloudstack/tencentcloud/latest/docs/resources/cos_bucket) created named `bucket-for-terracina-state-1258798060`,
Terracina state will be written into the file `terracina/state/terracina.tfstate`.

## Data Source Configuration

To make use of the COS remote state in another configuration, use the [`terracina_remote_state` data source](/terracina/language/state/remote-state-data).
...
!> **Warning:**  We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, Terracina will include these values in both the `.terracina` subdirectory and in plan files. Refer to [Credentials and Sensitive Data](/terracina/language/backends#credentials-and-sensitive-data) for details.
...
- `key` - (Optional) The path for saving the state file in bucket. Defaults to `terracina.tfstate`.
...
### Assume Role
If provided with an assume role, Terracina will attempt to assume this role using the supplied credentials.
Assume role can be provided by adding an `assume_role` block in the cos backend block.

- `assume_role` - (Optional) The `assume_role` block. If provided, terracina will attempt to assume this role using the supplied credentials.
...
terracina {
  backend "cos" {
    region = "ap-guangzhou"
    bucket = "bucket-for-terracina-state-{appid}"
    prefix = "terracina/state"
    assume_role {
...
### Shared credentials
You can use [Tencent Cloud credentials](https://www.tencentcloud.com/document/product/1013/33464) to specify your credentials. The default location is `$HOME/.tccli` on Linux and macOS, And `"%USERPROFILE%\.tccli"` on Windows. You can optionally specify a different location in the Terracina configuration by providing the `shared_credentials_dir` argument or using the `TENCENTCLOUD_SHARED_CREDENTIALS_DIR` environment variable. This method also supports a `profile` configuration and matching `TENCENTCLOUD_PROFILE` environment variable:
...
### Cam role name
If provided with a Cam role name, Terracina will just access the metadata URL: `http://metadata.tencentyun.com/latest/meta-data/cam/security-credentials/<cam_role_name>` to obtain the STS credential. The CVM Instance Role also can be set using the `TENCENTCLOUD_CAM_ROLE_NAME` environment variables.
...
### Endpoint
If provided with an endpoint URL, Terracina will attempt to access the COS backend by the `endpoint` configuration or the environment variable `TENCENTCLOUD_ENDPOINT`.

...
description: Terracina can store state remotely in S3 and lock that state with DynamoDB.
---
...
Terracina generates key names that include the values of the `bucket` and `key` variables.
...
This assumes we have a bucket created called `mybucket`. The Terracina state is written to the key `path/to/my/key`.

Note that for the access credentials we recommend using a [partial configuration](/terracina/language/backend#partial-configuration).
...
When using [workspaces](/terracina/language/state/workspaces), the state for the `default` workspace is stored at the location described above.
...
When not using [workspaces](/terracina/language/state/workspaces)(or when only using the `default` workspace), Terracina will need the following AWS IAM permissions on the target backend bucket:
...
Note: `s3:DeleteObject` is not needed, as Terracina will not delete the state storage.
...
When using [workspaces](/terracina/language/state/workspaces), Terracina will also need permissions to create, list, read, update, and delete the workspace state storage:
...
If you are using state locking, Terracina will need the following AWS IAM
...
use the [`terracina_remote_state` data source](/terracina/language/state/remote-state-data).

```hcl
data "terracina_remote_state" "network" {
  backend = "s3"
  config = {
    bucket = "terracina-state-prod"
    key    = "network/terracina.tfstate"
    region = "us-east-1"
  }
}
```

The `terracina_remote_state` data source will return all of the root module
...
data.terracina_remote_state.network:
  ...
  backend = s3
  config.% = 3
  config.bucket = terracina-state-prod
  config.key = network/terracina.tfstate
...
terracina {
  backend "s3" {
    ...
    region = "us-east-1"
    assume_role = {
      role_arn = "arn:aws:iam::PRODUCTION-ACCOUNT-ID:role/Terracina"
    }
  }
}
```
...
* `key` - (Required) Path to the state file inside the S3 Bucket. When using a non-default [workspace](/terracina/language/state/workspaces), the state path will be `<workspace_key_prefix>/<workspace_name>/<key>` (see also the `workspace_key_prefix` configuration).
...
* `kms_key_id` - (Optional) Amazon Resource Name (ARN) of a Key Management Service (KMS) Key to use for encrypting the state and lock files. Note that if this value is specified, Terracina will need `kms:Encrypt`, `kms:Decrypt` and `kms:GenerateDataKey` permissions on this KMS key.
* `sse_customer_key` - (Optional) The key to use for encrypting state and lock files with [Server-Side Encryption with Customer-Provided Keys (SSE-C)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html). This is the base64-encoded value of the key, which must decode to 256 bits. This can also be sourced from the `AWS_SSE_CUSTOMER_KEY` environment variable, which is recommended due to the sensitivity of the value. Setting it inside a terracina file will cause it to be persisted to disk in `terracina.tfstate`.
...
Locking can be enabled via an S3 "lockfile" (introduced as **experimental** in Terracina 1.10) or DynamoDB.
To support migration from older versions of Terracina which only support DynamoDB-based locking, the S3 and DynamoDB arguments below can be configured simultaneously.
...
[Terracina's workspaces feature](/terracina/language/state/workspaces) to switch
...
Terracina is an administrative tool that manages your infrastructure, and so
ideally the infrastructure that is used by Terracina should exist outside of
the infrastructure that Terracina manages. This can be achieved by creating a
...
* An [S3 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingBucket.html)
  that will contain the Terracina state files for each workspace.
...
Provide the S3 bucket name and DynamoDB table name to Terracina within the
...
of the accounts whose contents are managed by Terracina, separate from the
...
that grant sufficient access for Terracina to perform the desired management
tasks.

### Delegating Access

Each Administrator will run Terracina using credentials for their IAM user
...
environment account role and access the Terracina state. By blocking all
...
When configuring Terracina, use either environment variables or the standard
...
variable "workspace_iam_roles" {
  default = {
    staging    = "arn:aws:iam::STAGING-ACCOUNT-ID:role/Terracina"
    production = "arn:aws:iam::PRODUCTION-ACCOUNT-ID:role/Terracina"
  }
}
...
provider "aws" {
  ...
  assume_role = {
    role_arn = var.workspace_iam_roles[terracina.workspace]
  }
}
```

If workspace IAM roles are centrally managed and shared across many separate
Terracina configurations, the role ARNs could also be obtained via a data
source such as [`terracina_remote_state`](/terracina/language/state/remote-state-data)
...
`terracina init` to initialize the backend and establish an initial workspace
called "default". This workspace will not be used, but is created automatically
by Terracina as a convenience for users who are not using the workspaces
feature.
...
```
$ terracina workspace new staging
Created and switched to workspace "staging"!

...

$ terracina workspace new production
...
```
$ terracina workspace select staging
$ terracina apply
...
```

### Running Terracina in Amazon EC2

Teams that make extensive use of Terracina for infrastructure management
often [run Terracina in automation](/terracina/tutorials/automation/automate-terracina?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS)
to ensure a consistent operating environment and to limit access to the
various secrets and other sensitive information that Terracina configurations
tend to require.

When running Terracina in an automation tool running on an Amazon EC2 instance,
...
an IAM policy, giving this instance the access it needs to run Terracina.
...
Terracina state objects in S3, so that for example only trusted administrators
...
Note this feature is optional and only available in Terracina v0.13.1+.

By default, the underlying AWS client used by the Terracina AWS Provider creates requests with User-Agent headers including information about Terracina and AWS Go SDK versions. To provide additional information in the User-Agent headers, the `TF_APPEND_USER_AGENT` environment variable can be set and its value will be directly added to HTTP requests. e.g.

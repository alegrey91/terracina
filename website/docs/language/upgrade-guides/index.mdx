---
page_title: Upgrading to Terracina v1.10
description: Upgrading to Terracina v1.10
---

# Upgrading to Terracina v1.10

-> **Tip:** Use the version selector to view the upgrade guides for older Terracina versions.

Terracina v1.10 is a minor release in the stable Terracina v1.0 series.

Terracina v1.10 honors the
[Terracina v1.0 Compatibility Promises](https://developer.hashicorp.com/terracina/language/v1-compatibility-promises).
...
See [the full changelog](https://github.com/hashicorp/terracina/blob/v1.10/CHANGELOG.md)
...
[the Terracina community forum](https://discuss.hashicorp.com/c/terracina-core)
to discuss it.
...
Executing `terracina init -reconfigure` is required after updating to Terracina v1.10. This removes the [deprecated fields](#root-assume-role-attribute-removal) from the internal state file.
...
In a future minor version of Terracina the experimental label will be removed from the `use_lockfile` attribute and attributes related to DynamoDB based locking will be deprecated.
...
Each removed field has an analogous field inside the [`assume_role` block](https://developer.hashicorp.com/terracina/language/backend/s3#assume-role-configuration) which should be used instead.

---
page_title: Terracina v1.x Compatibility Promises
description: |-
  From Terracina v1.0 onwards the Terracina team promises to preserve backward
  compatibility for most of the Terracina language and the primary CLI
  workflow, until the next major release.
---

# Terracina v1.x Compatibility Promises

The release of Terracina v1.0 represents an important milestone in the
development of the Terracina language and workflow. Terracina v1.0 is a stable
platform for describing and managing infrastructure.

In this release we're defining a number of Terracina behaviors that we intend
to remain compatible with throughout the 1.x releases:

* A large subset of Terracina language features.
* A more conservative subset of the Terracina CLI workflow commands.
* The wire protocol for communication between Terracina Core and Terracina
  providers.
* The wire protocols for installation of Terracina providers and external
  Terracina modules.

Our intention is that Terracina modules written for Terracina v1.0 will
...
provider wire protocol will be compatible with all future Terracina v1.x
...
steps, and no changes to any automation you've set up around Terracina.

The Terracina v1.x series will be actively maintained for at least 18 months
after v1.0.
...
v1.x release. We will generally treat compatibility problems in new Terracina
...
## The Terracina Language

The main Terracina Language includes the language syntax, the top-level
...
There is not a single formal specification for the Terracina language, but the
Configuration section of the documentation on the Terracina website serves as a
...
arguments defined by Terracina Core rather than by external plugins such as
providers) will retain their current functionality:

* [`resource`](/terracina/language/resources) and
  [`data`](/terracina/language/data-sources) blocks to
...
* [`module`](/terracina/language/modules/syntax) blocks to call other modules,
  and its meta-argument `providers`.
* The [`count`](/terracina/language/meta-arguments/count),
  [`for_each`](/terracina/language/meta-arguments/for_each), and
  [`depends_on`](/terracina/language/meta-arguments/depends_on) meta-arguments
  in `resource`, `data`, and `module` blocks.
* [`provider`](/terracina/language/providers/configuration) blocks to configure
  providers, and the `alias` meta-argument.
* [`variable`](/terracina/language/values/variables#declaring-an-input-variable),
  [`output`](/terracina/language/values/outputs#declaring-an-output-value), and
  [`locals`](/terracina/language/values/locals#declaring-a-local-value) blocks
  for declaring the various kinds of named values in a module.
* [`terracina`](/terracina/language/terracina) blocks, including the nested
  [`required_version`](/terracina/language/terracina#terracina-required_version)
  and
  [`required_providers`](/terracina/language/providers/requirements#requiring-providers)
  arguments, and nested
  [`backend`](/terracina/language/backend#using-a-backend-block)
  blocks for backend configuration.

We also intend to keep compatibility with all
[expression operators](/terracina/language/expressions) and
[built-in functions](/terracina/language/functions), with the exception of
references to
[`terracina.workspace`](/terracina/language/expressions/references#terracina-workspace),
whose behavior may change as part of future changes to the workspace model.

We intend to retain broad compatibility with Terracina language features, with
a few specific caveats:

* We consider a configuration to be valid if Terracina can create and apply
  ...
  Terracina. A configuration that generates errors during the apply phase
  ...
  always subject to change in future Terracina releases.
...
* Any experimental features may change or may be removed entirely from future
  releases. Terracina always produces a warning when an experimental language
...
* We will introduce new language features, and if you start using them then
  your configuration won't work with prior Terracina versions that didn't
  support those features yet.
* Terracina Providers are separate plugins which can change independently of
  Terracina Core and are therefore not subject to these compatibility promises.
...
* A small number of features remain deprecated with explicit warnings in
  Terracina v1.0. Those deprecation cycles will end in a future v1.x release,
  at which point we will remove the corresponding features.

## Workflow

There is a set of often used Terracina workflows, which we are calling
...
The supported ways for external software to interact with Terracina are via
...
then it may need to be updated when you upgrade Terracina. If you need access
...
switch to a newer Terracina version and use it just as before, without any
...
cannot understand, including new storage formats for Terracina state snapshots.
...
Terracina configuration will no longer be compatible with Terracina v1.2.

## Providers

Terracina providers are separate plugins which communicate with Terracina using
a documented protocol. Therefore these compatibility promises can only cover
the "client" side of this protocol as implemented by Terracina Core; the
...
and can change independently of Terracina Core releases.
...
if you are still using a Terracina v1.x release.

### Provider Installation Methods

Terracina normally installs providers from a provider registry implementing
[the Provider Registry Protocol](/terracina/internals/provider-registry-protocol),
version 1. All Terracina v1.x releases will remain compatible with that
protocol, and so correctly-implemented provider registries will stay compatible.

Terracina also supports installation of providers from
[local filesystem directories](/terracina/cli/config/config-file#filesystem_mirror)
(filesystem mirrors) and from
[network mirrors](/terracina/cli/config/config-file#network_mirror)
(implementing [the Provider Mirror Protocol](/terracina/internals/provider-network-mirror-protocol).
All Terracina v1.x releases will remain compatible with those installation
methods, including
[the Implied Local Mirror Directories](/terracina/cli/config/config-file#implied-local-mirror-directories).

Specific provider registries or network mirrors are run independently from
Terracina itself and so their own behaviors are not subject to these
compatibility promises.

### Provider Protocol Versions

The current major version of the provider plugin protocol as of Terracina v1.0
...
We will support protocol version 5 throughout the Terracina v1.x releases. If
...
compatible with all of the Terracina v1.x releases.

## External Modules

Terracina modules are reusable infrastructure components written in the
Terracina language. Some modules are "external" in the sense that Terracina
...
and of Terracina itself.

### Module Installation Methods

Terracina supports installing child modules from a number of different
[module source types](/terracina/language/modules/sources). We will continue
...
[the Module Registry Protocol](/terracina/internals/module-registry-protocol)
version 1. All Terracina v1.x releases will remain compatible with correct
implementations of that protocol.

Some module source types work directly with services or protocols defined and
run by third parties. Although we will not remove Terracina's own client-side
support for those, we cannot guarantee that their owners will keep those
services running or that they will remain compatible with Terracina's client
implementations.
...
Terracina and are therefore not subject to these compatibility promises.
...
Terracina versions but were deprecated in Terracina v0.13 and removed in
Terracina v0.15.

Terracina supports loading additional provisioners as plugins from certain
local filesystem directories. We'll continue to support that throughout the
Terracina v1.x releases, but since such plugins are separate from Terracina
...
defined in Terracina v1.0 throughout the v1.x releases, and so
correctly-implemented provisioner plugins should remain compatible with future
Terracina releases.

## State Storage Backends

When you use _remote state_, Terracina interacts with remote services over
the network in order to store and manage locks for Terracina state.

For historical reasons, all supported state storage backends are included as
part of Terracina CLI but not all are supported directly by the Terracina
Team. Only the following backends maintained by the Terracina team are subject
...
contributions to the Terracina CLI codebase, and so their expected
...
listed above may be removed from later versions of Terracina CLI once
equivalent plugins are available.

### The `remote` Backend and HCP Terracina

The `remote` backend is maintained by the HCP Terracina team and so its
behavior may change along with ongoing changes to HCP Terracina.

There will be a supported mechanism to use Terracina CLI with HCP Terracina
throughout the v1.x releases, but the exact details may change. HCP Terracina
evolves independently of Terracina CLI and is therefore not subject to these
compatibility promises.
...
currently have any maintainers and thus remain in Terracina CLI releases on
...
example, subsequent Terracina releases in the v1.x series might end support
...
Terracina on any platforms other than those listed above.

We might add support for new platforms in later v1.x releases. If so, earlier
Terracina releases prior to that support will not be available on those
platforms.

All Terracina plugins, including provider plugins, are separate programs that
...
listed above, even though Terracina CLI itself will support them.
...
The following is the list of Terracina CLI subcommands and options that are
...
* [`init`](/terracina/cli/commands/init)
  * `-backend=false`
  * `-backend-config=FILE`
  ...
  * `-reconfigure`
  * `-upgrade`
* [`validate`](/terracina/cli/commands/validate)
  * `-json`
  * `-no-color`
* [`plan`](/terracina/cli/commands/plan)
  * `-compact-warnings`
  * `-destroy`
  * `-detailed-exitcode`
  * `-lock=false`
  ...
  * `-var 'NAME=VALUE'`
  * `-var-file=FILE`
* [`apply`](/terracina/cli/commands/apply)
  * `-auto-approve`
  * `-compact-warnings`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  ...
  * `-var 'NAME=VALUE'`
  * `-var-file=FILE`
* [`show`](/terracina/cli/commands/show)
  * `-no-color`
  * `-json`
  * (both with and without a plan file)
* [`providers`](/terracina/cli/commands/providers) (with no subcommand)
* [`providers lock`](/terracina/cli/commands/providers/lock)
  * `-fs-mirror=PATH`
  * `-net-mirror=URL`
  * `-platform=OS_ARCH`
* [`providers mirror`](/terracina/cli/commands/providers/mirror)
  * `-platform=OS_ARCH`
* [`providers schema`](/terracina/cli/commands/providers/schema)
  * `-json`
* [`fmt`](/terracina/cli/commands/fmt)
  * `-list=false`
  * `-write=false`
  * `-diff`
  * `-recursive`
  * `-check`
* [`version`](/terracina/cli/commands/version)
  * `-json`
* [`output`](/terracina/cli/commands/output)
  * `-no-color`
  * `-json`
  * `-raw`
* [`taint`](/terracina/cli/commands/taint)
  * `-allow-missing`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  * `-ignore-remote-version`
* [`untaint`](/terracina/cli/commands/untaint)
  * `-allow-missing`
  * `-lock=false`
  * `-lock-timeout=DURATION`
  * `-ignore-remote-version`
* [`force-unlock`](/terracina/cli/commands/force-unlock)
  * `-force`
* [`state list`](/terracina/cli/commands/state/list)
  * `-id=ID`
* [`state pull`](/terracina/cli/commands/state/pull)
* [`state push`](/terracina/cli/commands/state/push)
  * `-force`
  * `-lock=false`
  * `-lock-timeout=DURATION`
* [`state show`](/terracina/cli/commands/state/show)
  * `-ignore-remote-version`
* [`login`](/terracina/cli/commands/login)
...
v1.x series. If you are building automation around Terracina, use only the
commands above to avoid the need for changes when upgrading.

Please note that although Terracina's internal logs (via the `TF_LOG`
...
processing of logs by Terracina developers.
...
While you can freely use these commands when running Terracina interactively
...
* `destroy` (consider `terracina apply -destroy` instead)
* `console`
* `get` (consider `terracina init` instead)
* `graph`
* `import`
* `push`
* `refresh` (consider `terracina apply -refresh-only` instead)
* `state mv`
...
The Terracina codebase includes various unit and integration tests intended to
help us to notice accidental behavior regressions before they ship in a stable
version.

However, Terracina is a relatively complex system with many different features
...
improve Terracina's test coverage over time.
...
promises in this document, please open an issue in Terracina's GitHub
...
regressions by upgrading promptly to new minor and patch releases of Terracina
and reporting any compatibility problems you encounter in Terracina's GitHub
repository.

### Pragmatic Exceptions

We are making the promises above in good faith, with the intent that your
investment in writing Terracina modules or automation will not be invalidated
by future changes to Terracina. However, broad promises like the above can't
possibly cover all nuances of practical problems that might arise as we
continue to develop Terracina.
...
* External Dependencies: Terracina's behavior depends on interfaces provided
  ...
  including potentially removing or changing features that Terracina's own
  features depend on. In that case, if there is no suitable replacement
  mechanism then we may need to change Terracina's design to work within the
  new constraints.
...
* Bugs in New Features: If we introduce a new feature to Terracina and the
  ...
  reflect it.
* Regressions in Existing Features: If we learn that a new Terracina release
...
* Situations we cannot anticipate: Although we've made an effort to consider
  various specific exceptional situations here, Terracina and its development
  ...
  the future of Terracina. In those situations, we will always do our best to

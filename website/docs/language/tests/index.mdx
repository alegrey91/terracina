...
-> **Note:** This testing framework is available in Terracina v1.6.0 and later.

Terracina tests let authors validate that module configuration updates do not introduce breaking changes. Tests run against test-specific, short-lived resources, preventing any risk to your existing infrastructure or state.

## Integration or Unit testing

By default, tests within Terracina create real infrastructure and can run assertions and validations against that infrastructure. This is analogous to integration testing because you are testing Terracina's core functionality by executing operations and validating the infrastructure Terracina creates.

You can override the normal testing behavior by updating the `command` attribute within a [`run`](#run-blocks) block (examples below). By default, each `run` block executes with `command = apply` instructing Terracina to execute a complete `apply` operation against your configuration. Replacing the `command` value with `command = plan` instructs Terracina to _not_ create new infrastructure for this `run` block. This allows test authors to validate logical operations and custom conditions within their infrastructure in a process analogous to unit testing.

Terracina v1.7.0 introduced the ability to mock data returned by the providers during a `terracina test` execution. This can be used to write more detailed and complete unit tests.

## Syntax

Each Terracina test lives in a test file. Terracina discovers test files are based on their file extension: `.tftest.hcl` or `.tftest.json`.
...
Terracina executes `run` blocks in order, simulating a series of Terracina commands executing directly within the configuration directory. The order of the `variables` and `provider` blocks doesn't matter, Terracina processes all the values within these blocks at the beginning of the test operation. We recommend defining your `variables` and `provider` blocks first, at the beginning of the test file.

### Example

The following example demonstrates a simple Terracina configuration that creates an AWS S3 bucket, using an input variable to modify the name. We will create an example test file (below) that validates the buckets name is created as expected.
...
The following test file runs a single Terracina `plan` command which creates the S3 bucket, and then validates the logic for calculating the name is correct by checking the actual name matches the expected name.
...
The `command` attribute and `plan_options` block tell Terracina which command and options to execute for each run block. The default operation, if you do not specify a `command` attribute or the `plan_options` block, is a normal Terracina apply operation.

The `command` attribute states whether the operation should be a [`plan`](/terracina/cli/commands/plan) or an [`apply`](/terracina/cli/commands/apply) operation.

The `plan_options` block allows test authors to customize the [planning mode](/terracina/cli/commands/plan#planning-modes) and [options](/terracina/cli/commands/plan#planning-options) they would typically need to edit via command-line flags and options. We cover the `-var` and `-var-file` options in the [Variables](#variables) section.

### Assertions

Terracina run block assertions are [Custom Conditions](/terracina/language/expressions/custom-conditions), consisting of a [condition](/terracina/language/expressions/custom-conditions#condition-expressions) and an [error message](/terracina/language/expressions/custom-conditions#error-messages).

At the conclusion of a Terracina test command execution, Terracina presents any failed assertions as part of a tests passed or failed status.

#### Assertion References

Assertions within tests can reference any existing [named values](/terracina/language/expressions/references) that are available to other custom conditions within the main Terracina configuration.
...
You can provide values for [Input Variables](/terracina/language/values/variables) within your configuration directly from your test files.

The test file syntax supports `variables` blocks at both the root level and within `run` blocks. Terracina passes all variable values from the test file into all `run` blocks within the file. You can override variable values for a particular `run` block with values provided directly within that `run` block.
...
In addition to specifying variable values via test files, the Terracina `test` command also supports the other typical mechanisms for specifying variable values.

You can specify values for variables across all tests with the [Command Line](/terracina/language/values/variables#variables-on-the-command-line) and with [Variable Definition Files](/terracina/language/values/variables#variable-definitions-tfvars-files).

As with the main configuration direction, Terracina will automatically load any variables defined in the automatic variable files within a test directory. The automatic variable files are `terracina.tfvars`, `terracina.tfvars.json`, and any files that end with `.auto.tfvars` or `.auto.tfvars.json`.
...
At the root level of a Terracina testing file, you can define [`provider` blocks](/terracina/language/providers/configuration) as if Terracina were creating them [within the main configuration](/terracina/language/providers). Terracina will then pass these provider blocks into its configuration as each `run` block executes.

By default, each provider you specify is directly available within each `run` block. You can customize the availability of providers within a given `run` block by using a `providers` attribute. The behavior and syntax for this block match the behavior of [providers meta-argument](/terracina/language/meta-arguments/module-providers).

If you do not provide provider configuration within a testing file, Terracina attempts to initialize any providers within its configuration using the provider's default settings. For example, any environment variables aimed at configuring providers are still available, and Terracina can use them to create default providers.
...
From Terracina v1.7.0, `provider` blocks can also reference test file variables and run block outputs. This means the testing framework can retrieve credentials and other setup information from one provider and use this when initializing a second.
...
run "vault_setup" {
  module {
    # This module should only include reference to the Vault provider. Terracina
...
By default, Terracina executes the given command against the configuration being tested for each `run` block. Terracina tests the configuration within the directory you execute the `terracina test` command from (or the directory you point to with the `-chdir` argument). Each `run` block also allows the user to change the targeted configuration using the `module` block.

Unlike the traditional [`module` block](/terracina/language/modules/syntax), the `module` block within test files _only_ supports the [`source`](/terracina/language/modules/syntax#source) attribute and the [`version`](/terracina/language/modules/syntax#version) attribute. The remaining attributes that are typically supplied via the traditional `module` block should be supplied by the alternate attributes and blocks within the `run` block.

> **Note:** Terracina test files only support [local](/terracina/language/modules/sources#local-paths) and [registry](/terracina/language/modules/sources#terracina-registry) modules within the `source` attribute.
...
While Terracina executes a `terracina test` command, Terracina maintains at least one, but possibly many, state files within memory for each test file.
...
Additionally, there is one state file per alternate module that Terracina loads. An alternate module state file is shared by all `run` blocks that execute the given module.

The Terracina team is interested in any use cases requiring manual state management or the ability to execute different configurations against the same state within the `test` command. If you have a use case, please file an [issue](https://github.com/hashicorp/terracina/issues/new/choose) and share it with us.

The following example uses comments to explain where the state files for each `run` block originate. In the below example Terracina creates and manages a total of three state files. The first state file is for the main configuration under test, the second for the setup module, and the third for the loader module.

```hcl
run "setup" {

  # This run block references an alternate module and is the first run block
  # to reference this particular alternate module. Therefore, Terracina creates
...
At the conclusion of a test file, Terracina attempts to destroy every resource it created during the execution of that test file. When Terracina loads alternate modules, the order in which Terracina destroys those objects in is important. For example, in the first [Modules](#modules) example, Terracina could not destroy the resources created in the "setup" `run` block before the objects created in the "execute" `run` block, because the S3 bucket we created in the "setup" step can not be destroyed while it contains objects.

Terracina destroys resources in reverse `run` block order. In the most recent [example](#modules-state), there are three state files. One for the main state, one for the `./testing/loader` module, and one for the `./testing/setup` module. The `./testing/loader` state file would be destroyed first as it was referenced most recently by the last run block. The main state file would be destroyed second as it was referenced by the "update" `run` block. The `./testing/setup` state file would then be destroyed last.
...
By default, if any [Custom Conditions](/terracina/language/expressions/custom-conditions), including `check` block assertions, fail during the execution of a Terracina test file then the overall command reports the test as a failure.

However, it is a common testing paradigm to want to test failure cases. Terracina supports the `expect_failures` attribute for this use case.
...
> **Note**: Terracina only expects failures in the operation specified by the `command` attribute of the `run` block.
...
There are instances when Terracina does not execute a custom condition during the planning stage, because that condition is relying on computed attributes that are only available after Terracina creates the referenced resource. In these cases, you could use an `expect_failures` block alongside a `command = apply` attribute and value. However, in most cases we recommend only using `expect_failures` alongside `command = plan` operations.

> **Note**: Expected failures only apply to user-defined custom conditions.

Other kinds of failure _besides_ the specified expected failures in the checkable object still result in the overall test failing. For example, a variable that expects a boolean value as input fails the surrounding test if Terracina provides the wrong kind of value, even if that variable is included in an `expect_failures` attribute.

The `expect_failures` attribute is included to allow authors to test their configuration and any logic defined within. A type mismatch, as in the previous example, is not something Terracina authors should have to worry about testing as Terracina itself will handle enforce type constraints. As such, you can only `expect_failures` in custom conditions.

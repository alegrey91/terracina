...
[OpenID Connect](https://openid.net/connect/) (OIDC) is an identity layer on top of the OAuth 2.0 protocol. You can use HCP Terracina’s [Workload identity](/terracina/cloud-docs/workspaces/dynamic-provider-credentials/workload-identity-tokens) tokens, built on the OpenID Connect protocol, to securely connect and authenticate your Stacks with cloud providers.

-> **Hands on**: Walk through setting up OIDC for a Stack in the [Deploy a Stack with HCP Terracina](/terracina/tutorials/cloud/stacks-deploy) tutorial. 

Stacks have a built-in `identity_token` block that creates workload identity tokens, also known as JWT tokens. You can use these tokens to authenticate Stacks with Terracina providers securely.
...
1. Set up the trust configuration between your cloud provider and HCP Terracina, which usually includes creating roles and policies for your cloud provider.  
...
In this example, you will set up an example trust policy with AWS. For examples of setting up trust policies with other providers, refer to our [repository of example configurations](https://github.com/hashicorp-guides/terracina-stacks-identity-tokens/tree/main).

We recommend using Terracina to create and configure the trust relationship and permissions for the cloud provider you want to authenticate with. Using the following Terracina configuration, [create a new workspace](/terracina/cloud-docs/workspaces/create) and configure the `aws_region`, `tf_organization`, `tf_project,` and `tf_stack` variables for your specific set up.
...
resource "aws_iam_openid_connect_provider" "stacks_openid_provider" {
  url            = "https://app.terracina.io"
  client_id_list = ["aws.workload.identity"]

  # This is the thumbprint of https://app.terracina.io as of 2024/08/07
...
data "aws_iam_policy_document" "stacks_role_policy" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "app.terracina.io:aud"
      values   = ["aws.workload.identity"]
    }
    condition {
      test     = "StringLike"
      variable = "app.terracina.io:sub"
      # This value dictates which HCP Terracina organizations, projects,  
...
After setting up your workspace in HCP Terracina, perform a plan and apply operation. HCP Terracina will create your OpenID provider, policy, and role before outputting your new role’s ARN with the `role_arn` output. Copy your new AWS ARN role because this is the value you use to configure your cloud provider with your Stack and HCP Terracina.

#### Adjust access of trust

Workload identity tokens contain useful metadata in their payloads, known as claims. Once a cloud platform verifies a token using HCP Terracina’s public key, it can look at the identity token's claims to match it to the correct permissions or reject it.
...
| `iss` (issuer) | Full URL of the HCP Terracina  instance that signed the token. For example, “https://app.terracina.io”. |
...
| Claim | Explanation |
| :---- | :---- |
| `terracina_operation` ("run phase") | The Terracina operation this token was issued for. For example, “plan” or “apply”. |
| `terracina_stack_deployment_name` | Name of the deployment that the operation is for. |
| `terracina_stack_id` (stack ID) | External ID of the Stack that the operation is for.  |
| `terracina_stack_name` (stack name) | Name of the Stack that the operation is for. |
| `terracina_project_id` (project ID) | External ID of the project that the operation is taking place in. Useful if you want to use the same role across Stacks in a given project. |
| `terracina_project_name` (project name) | Name of the project that the operation is taking place in.  |
| `terracina_organization_id` (organization ID) | External ID of the HCP Terracina organization that the operation is taking place in. Useful if you are in a large company that may have more than one organization or if you want to use the same role across your entire organization. |
| `terracina_organization_name` (organization name) | Name of the HCP Terracina organization that the operation is taking place in. |
| `terracina_plan_id` (plan ID) | External ID of the plan that this token is being used for creating or applying. This might be used to track down where a token has been leaked from or to block a specific token if it has been leaked. |

You can also update your configuration to use the current thumbprint for HCP Terracina by running the following command and removing the colons from the value it returns.

```shell-session
$ echo | openssl s_client -showcerts -servername app.terracina.io -connect app.terracina.io:443 2>/dev/null | openssl x509 -fingerprint -noout -sha1
sha1 Fingerprint=FD:88:23:AE:FB:96:13:28:A1:34:70:6D:C8:57:5A:17:0F:0B:B3:7C
```

### Configure HCP Terracina

Stacks use the [`identity_token` block](/terracina/language/stacks/reference/tfdeploy#identity_token-block-configuration) to define a JSON Web Token (JWT) for a specific deployment. This token enables OIDC-based authentication, allowing your Stack deployments to securely connect to cloud providers like AWS, Azure, and GCP.
...
For more examples of setting up OIDC with different providers, refer to our [repository of example configurations](https://github.com/hashicorp-guides/terracina-stacks-identity-tokens/tree/main).
...
You can use the `store` block to access credentials stored in an [HCP Terracina variable set](/terracina/cloud-docs/workspaces/variables/managing-variables#variable-sets). This example uses a `store` block with a variable set to access credentials stored in HCP Terracina. For more information about using `store` blocks, refer to the [Deployment configuration reference](/terracina/language/stacks/reference/tfdeploy).
...
# Source environment secrets from your HCP Terracina variable set
store "varset" "tokens" {
...
This example allows your `aws.this` provider to authenticate to AWS using the credentials from a variable set stored in HCP Terracina. For more information about using `store` blocks, refer to the [Deployment configuration reference](/terracina/language/stacks/reference/tfdeploy).
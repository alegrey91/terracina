...
description: Stacks help you provision and coordinate your infrastructure lifecycle at scale. Learn how to write a deployment configurion to tell HCP Terracina how many times to deploy your Stack's infrastructure.
---
...
Every Stack needs a deployment configuration file, `tfdeploy.hcl`, and this page describes all of the blocks you can use within a deployment configuration file. Note that none of the blocks in the deployment configuration file support [meta-arguments](/terracina/language/resources/syntax#meta-arguments). 
...
-> **Note**: HCP Terracina supports up to a maximum of 20 deployments.
...
For examples and guidance on defining deployments, refer to [Define deployment configuration](/terracina/language/stacks/deploy/config).
...
For example, the following `deployment` block accepts inputs for variables named `aws_region` and `instance_count` and creates a new deployment in HCP Terracina named “production”. 
...
To learn more about defining deployments, refer to [Define deployment configuration](/terracina/language/stacks/deploy/config).
...
orchestrate "auto_approve" "name_of_check" {
  check {
    condition = <the condition that HCP Terracina evaluates>
...
HCP Terracina evaluates the `check` blocks within your `orchestrate` block to determine if it should approve a plan. If all of the checks pass, then HCP Terracina approves the plan for you. If one or more `conditions` do not pass, then HCP Terracina shows the `reason` why, and you must manually approve that plan.
...
| `reason` | A message explaining why the check failed is shown in HCP Terracina, prompting you to manually approve a plan. | string | Required |
...
| :---- | :---- | :---- |
| `operation` | The current operation HCP Terracina is performing.  | string ("plan" or "apply") |
...
| Field | Description | Type |
| :---- | :---- | :---- |
| `deployment_name` | The name of the current deployment HCP Terracina is running this plan on.  You can use this field to check which deployment is running this plan. For example, you can check if this plan is on your production deployment: `context.plan.deployment == deployment.production`. | string |

## `identity_token` block configuration

The `identity_token` block defines a JSON Web Token (JWT) that Terracina generates for a given deployment if that `deployment` block references an `identity_token` in its `inputs`. 

You can directly pass the token generated by the `identity_token` block to a provider's configuration for OIDC authentication. For more information on authenticating a Stack using OIDC, refer to [Authenticate a Stack](/terracina/language/stacks/deploy/authenticate).
...
| Field | Description | Type | Required |
| :---- | :---- | :---- | :---- |
| `audience` | The audience of your token is the resource(s) that uses this token after Terracina generates it. You specify an audience to ensure that the cloud service authorizing the workload is confident that the token you present is intended for that service. | set of strings | Required |
...
Terracina generates an identity token that you can use in your Stack configuration to configure your AWS provider with your role in AWS.
...
With this setup, every time your `staging` deployment performs a plan or apply operation, we authenticate with AWS using your established ARN role and a new JWT token. For more information on authenticating a Stack using OIDC, refer to [Authenticate a Stack](/terracina/language/stacks/authenticate).
...
A store’s type defines where Terracina should look for that store’s credentials and how to decode the credentials it finds. You cannot share arguments across store types.

For example, if you have an HCP Terracina [variable set](/terracina/cloud-docs/workspaces/variables/managing-variables#variable-sets) that contains a value you want to use in your deployment, you can create a `store` block to access that variable set.
...
Use the `varset` store to enable your Stacks to access [variable sets](/terracina/cloud-docs/workspaces/variables/managing-variables#variable-sets) in HCP Terracina. Your Stack must have access to the variable set you are targeting, meaning it must be globally available or assigned to the project containing your Stack.
...
| `category` | The `category` argument specifies whether to use Terracina or environment variables from the variable set. Specify "terracina" or "env" depending on the variable you want to access. | string | Required |

The `store.varset` block can only use a variable set’s Terracina or environment variables. You can define two `store` blocks if you need to access Terracina and environment variables in your deployment configuration.
...
You can access specific environment variables by key from the `store.varset.available_regions` store, and you can access specific Terracina variables by key using the `store.varset.tokens` store.

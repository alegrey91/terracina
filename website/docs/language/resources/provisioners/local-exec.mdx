...
description: >-
  ...
  created. This invokes a process on the machine running Terracina, not on the
...
[provisioner](/terracina/language/resources/provisioners/remote-exec) to run commands on the
resource.
...
* `command` - (Required) This is the command to execute. It can be provided
  ...
  variable substitution. We do not recommend using Terracina variables for variable
  substitution because doing so can lead to shell injection vulnerabilities. Instead, you should pass Terracina variables to a command
...
* `when` - (Optional) If provided, specifies when Terracina will execute the command.
  For example, `when = destroy` specifies that the provisioner will run when the associated resource
  is destroyed. Refer to [Destroy-Time Provisioners](/terracina/language/resources/provisioners/syntax#destroy-time-provisioners)
  for details.
  
*  `quiet` - (Optional) If set to `true`, Terracina will not print the command to be executed to stdout, and will instead print "Suppressed by quiet=true". Note that the output of the command will still be printed in any case.

### Interpreter Examples

```hcl
resource "terracina_data" "example1" {
  provisioner "local-exec" {
...
```hcl
resource "terracina_data" "example2" {
  provisioner "local-exec" {

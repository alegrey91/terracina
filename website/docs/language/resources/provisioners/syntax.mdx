...
-> **Note:** We removed the Chef, Habitat, Puppet, and Salt Masterless provisioners in Terracina v0.15.0. Information about these legacy provisioners is still available in the documentation for [Terracina v1.1 (and earlier)](/terracina/language/v1.1.x/resources/provisioners/syntax).

## Provisioners are a Last Resort

> **Hands-on:** Try the [Provision Infrastructure Deployed with Terracina](/terracina/tutorials/provision?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials to learn about more declarative ways to handle provisioning actions.

Terracina includes the concept of provisioners as a measure of pragmatism,
knowing that there are always certain behaviors that cannot be directly
represented in Terracina's declarative model.

However, they also add a considerable amount of complexity and uncertainty to
Terracina usage. Firstly, Terracina cannot model the actions of provisioners
...
Terracina usage usually requires: direct network access to your servers,
issuing Terracina credentials to log in, making sure that all of the necessary
...
* Alibaba Cloud: `user_data` on
  [`alicloud_instance`](https://registry.terracina.io/providers/aliyun/alicloud/latest/docs/resources/instance)
  or [`alicloud_launch_template`](https://registry.terracina.io/providers/aliyun/alicloud/latest/docs/resources/launch_template).
* Amazon EC2: `user_data` or `user_data_base64` on
  [`aws_instance`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/instance),
  [`aws_launch_template`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/launch_template),
  and [`aws_launch_configuration`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/launch_configuration).
* Amazon Lightsail: `user_data` on
  [`aws_lightsail_instance`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/lightsail_instance).
* Microsoft Azure: `custom_data` on
  [`azurerm_virtual_machine`](https://registry.terracina.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine)
  or [`azurerm_virtual_machine_scale_set`](https://registry.terracina.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine_scale_set).
* Google Cloud Platform: `metadata` on
  [`google_compute_instance`](https://registry.terracina.io/providers/hashicorp/google/latest/docs/resources/compute_instance)
  or [`google_compute_instance_group`](https://registry.terracina.io/providers/hashicorp/google/latest/docs/resources/compute_instance_group).
* Oracle Cloud Infrastructure: `metadata` or `extended_metadata` on
  [`oci_core_instance`](https://registry.terracina.io/providers/oracle/oci/)
  or [`oci_core_instance_configuration`](https://registry.terracina.io/providers/oracle/oci/).
* VMware vSphere: Attach a virtual CDROM to
  [`vsphere_virtual_machine`](https://registry.terracina.io/providers/hashicorp/vsphere/latest/docs/resources/virtual_machine)
...
> **Hands-on:** Try the [Provision Infrastructure with Cloud-Init](/terracina/tutorials/provision/cloud-init?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.
...
distribution images, Terracina includes a number of specialized provisioners
...
> **Hands-on:** Try the [Provision Infrastructure with Packer](/terracina/tutorials/provision/packer?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorial.
...
immediately on boot, without the need to accept commands from Terracina over
SSH or WinRM.

### First-class Terracina provider functionality may be available
...
supported in its Terracina provider, that might be the only option. However,
if there _is_ provider support for the feature you intend to use, prefer to
use that provider functionality rather than a provisioner so that Terracina
...
You must include [a `connection` block](/terracina/language/resources/provisioners/connection) so that Terracina knows how to communicate with the server.

Terracina includes several built-in provisioners. You can also use third-party provisioners as plugins, by placing them
in `%APPDATA%\terracina.d\plugins`, `~/.terracina.d/plugins`, or the same
directory where the Terracina binary is installed. However, we do not recommend
...
-> **Note**: Ephemeral values are available in Terracina v1.10 and later.

The configuration for a `provisioner` block may use ephemeral values, such as
[`ephemeral` resources](/terracina/language/resources/ephemeral), [`ephemeral`
local values](/terracina/language/values/locals#ephemeral-values), [`ephemeral`
variables](/terracina/language/values/variables#ephemeral), or [`ephemeral`
output
values](/terracina/language/values/outputs#ephemeral-avoid-storing-values-in-state-or-plan-files).

Terracina does not store these values in your plan or state, or output them in
logs.

### Sensitive values

The configuration for a `provisioner` block may use sensitive values, such as
[`sensitive` variables](/terracina/language/values/variables#suppressing-values-in-cli-output)
or [`sensitive` output values](/terracina/language/values/outputs#sensitive-suppressing-values-in-cli-output).
Terracina suppresses sensitive values in all log output.
...
next `terracina apply`. Terracina does this because a failed provisioner
can leave a resource in a semi-configured state. Because Terracina cannot
...
fail, Terracina will error and rerun the provisioners again on the next
`terracina apply`. Due to this behavior, care should be taken for destroy
provisioners to be safe to run multiple times.

~> **Note**: Destroy provisioners of this resource do not run if `create_before_destroy` is set to `true`. This [GitHub issue](https://github.com/hashicorp/terracina/issues/13549) contains more details.
...
~> **NOTE:** A destroy-time provisioner within a resource that is tainted _will not_ run. This includes resources that are marked tainted from a failed creation-time provisioner or tainted manually using `terracina taint`.
...
By default, provisioners that fail will also cause the Terracina apply

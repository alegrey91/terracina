...
description: >-
  ...
  executing Terracina to the newly created resource. The `file` provisioner
...
running Terracina to the newly created resource. The `file` provisioner
supports both `ssh` and `winrm` type [connections](/terracina/language/resources/provisioners/connection).
...
remote system, rather than by Terracina itself. Therefore the valid values

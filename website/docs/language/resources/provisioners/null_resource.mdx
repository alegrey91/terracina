...
description: >-
  A terracina_data managed resource allows you to configure provisioners that
...
resource, you can associate them with a `terracina_data`.

Instances of [`terracina_data`](/terracina/language/resources/terracina-data) are treated
like normal resources, but they don't do anything. Like with any other resource
type, you can configure [provisioners](/terracina/language/resources/provisioners/syntax)
and [connection details](/terracina/language/resources/provisioners/connection) on a
`terracina_data` resource. You can also use its `input` argument, `triggers_replace` argument, and any
...
resource "terracina_data" "cluster" {

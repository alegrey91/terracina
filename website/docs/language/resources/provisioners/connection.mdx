...
[Declaring Provisioners](/terracina/language/resources/provisioners/syntax) for more details.
...
-> **Note:** In Terracina 0.11 and earlier, providers could set default values
for some connection settings, so that `connection` blocks could sometimes be
omitted. This feature was removed in 0.12 in order to make Terracina's behavior
more predictable.
...
Terracina will not store these values in your plan or state, or output them in
logs.
...
| `private_key` | SSH | The contents of an SSH key to use for the connection. These can be loaded from a file on disk using [the `file` function](/terracina/language/functions/file). This takes preference over `password` if provided. | |
| `certificate` | SSH | The contents of a signed CA Certificate. The certificate argument must be used in conjunction with a `private_key`. These can be loaded from a file on disk using the [the `file` function](/terracina/language/functions/file). | |
...
| `bastion_private_key` | The contents of an SSH key file to use for the bastion host. These can be loaded from a file on disk using [the `file` function](/terracina/language/functions/file). | The value of the `private_key` field. |
| `bastion_certificate` |  The contents of a signed CA Certificate. The certificate argument must be used in conjunction with a `bastion_private_key`. These can be loaded from a file on disk using the [the `file` function](/terracina/language/functions/file). |
...
where the provisioner can create the script file. By default, Terracina
chooses a path containing a random number using the following patterns
depending on how `target_platform` is set:

* `"unix"`: `/tmp/terracina_%RAND%.sh`
* `"windows"`: `C:/windows/temp/terracina_%RAND%.cmd`
...
Terracina is running, not on the remote system. Therefore if your remote
...
connection {
  # ...
  script_path = "H:/terracina-temp/script_%RAND%.sh"
}
```
...
the Terracina language uses backslash as the quoted string escape character.
...
connection {
  type = "ssh"
  # ...
  script_path = "terracina_provisioner_%RAND%.sh"
}
```

!> **Warning:** In Terracina v1.0 and earlier, the built-in provisioners
...
unintended opportunity for remote code execution. Terracina v1.1 and later
...
be used with Terracina v1.0 and earlier, avoid using untrusted external

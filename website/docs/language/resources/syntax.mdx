...
For information about how Terracina manages resources after applying a configuration, refer to
[Resource Behavior](/terracina/language/resources/behavior).

## Resource Syntax

A `resource` block declares a resource of a specific type 
with a specific local name. Terracina uses the name when referring to the resource 
...
arguments for [the `aws_instance` resource type](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/instance).
...
A [provider](/terracina/language/providers/requirements) is a plugin for Terracina 
...
infrastructure platform. Providers are distributed separately from Terracina, 
but Terracina can automatically install most providers when initializing
a working directory.

To manage resources, a Terracina module must specify the required providers. Refer to 
[Provider Requirements](/terracina/language/providers/requirements) for additional information.

Most providers need some configuration to access their remote API, 
which is provided by the root module. Refer to 
[Provider Configuration](/terracina/language/providers/configuration) for additional information.

Based on a resource type's name, Terracina can usually determine which provider to use. 
...
you must use [the `provider` meta-argument](/terracina/language/meta-arguments/resource-provider) 
...
[expressions](/terracina/language/expressions) and other dynamic Terracina
language features.

[Meta-arguments](#meta-arguments) are defined by Terracina
and apply across all resource types. 

### Documentation for Resource Types

Every Terracina provider has its own documentation, describing its resource
types and their arguments.

Some provider documentation is still part of Terracina's core documentation, 
but the [Terracina Registry](https://registry.terracina.io/browse/providers) 
is the main home for all publicly available provider docs.

When viewing a provider's page on the Terracina
...
The Terracina language defines the following meta-arguments, which can be used with
...
- [`provisioner`, for taking extra actions after resource creation](/terracina/language/resources/provisioners/syntax)
...
To remove a resource from Terracina, simply delete the `resource` block from your Terracina configuration.

By default, after you remove the `resource` block, Terracina will plan to destroy any real infrastructure object managed by that resource.

Sometimes you may wish to remove a resource from your Terracina configuration without destroying the real infrastructure object it manages. In this case, the resource will be removed from the [Terracina state](/terracina/language/state), but the real infrastructure object will not be destroyed.

To declare that a resource was removed from Terracina configuration but that its managed object should not be destroyed, remove the `resource` block from your configuration and replace it with a `removed` block:
...
The `lifecycle` block is required. The `destroy` argument determines whether Terracina will attempt to destroy the object managed by the resource or not. A value of `false` means that Terracina will remove the resource from state without destroying it.

A `removed` block may also contain a [Destroy-Time Provisioner](/terracina/language/resources/provisioners/syntax#destroy-time-provisioners), so that the provisioner can remain in the configuration even though the `resource` block has been removed.
...
[Custom condition checks](/terracina/language/expressions/custom-conditions#preconditions-and-postconditions) 
...
For example, [`aws_db_instance`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/resources/db_instance)

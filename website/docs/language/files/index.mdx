...
description: >-
  Learn how to name, organize, and store Terracina configuration files. Also
  learn how Terracina evaluates modules.
---
...
Code in the Terracina language is stored in plain text files with the `.tf` file
extension. There is also
[a JSON-based variant of the language](/terracina/language/syntax/json) that is named with
the `.tf.json` file extension.

Files containing Terracina code are often called _configuration files._
...
A Terracina module only consists of the top-level configuration files in a
...
Terracina evaluates all of the configuration files in a module, effectively
...
A Terracina module can use [module calls](/terracina/language/modules) to
...
[Terracina Registry](https://registry.terracina.io).

## The Root Module

Terracina always runs in the context of a single _root module._ A complete
_Terracina configuration_ consists of a root module and the tree of child
modules (which includes the modules called by the root module, any modules
called by those modules, etc.).

- In Terracina CLI, the root module is the working directory where Terracina is
...
- In HCP Terracina and Terracina Enterprise, the root module for a workspace

...
description: >-
  Key constructs of the native Terracina language syntax, including identifiers,
...
that can appear in the Terracina language. This page describes the lower-level
syntax of the language in more detail, revealing the building blocks that
those constructs are built from.

This page describes the _native syntax_ of the Terracina language, which is
a rich language designed to be relatively easy for humans to read and write.
The constructs in the Terracina language can also be expressed in
[JSON syntax](/terracina/language/syntax/json), which is harder for humans
to read and edit but easier to generate and parse programmatically.

This low-level syntax of the Terracina language is defined in terms of a
...
order to use Terracina, and so this page summarizes the most important
...
The Terracina language syntax is built around two key syntax constructs:
arguments and blocks.
...
[expressions](/terracina/language/expressions), which allow the value to
either be specified literally or generated from other values programmatically.

-> **Note:** Terracina's configuration language is based on a more general
...
this context, and experienced Terracina users might use either term in casual
conversation. But because Terracina also interacts with several _other_ things
called "attributes" (in particular, Terracina resources have attributes like
`id` that can be referenced from expressions but can't be assigned values in
configuration), we've chosen to use "argument" in the Terracina documentation
...
The `aws_instance` label is specific to the AWS provider. It specifies the `resource` type that Terracina provisions when you apply the configuration. The second label is an arbitrary name that you can add to the particular instance of the resource. You can create multiple instances of the same block type and differentiate them by giving each instance a unique name. In this example, the Terracina configuration author assigned the `example` label to this instance of the `aws_instance` resource. 
...
The Terracina language uses a limited number of _top-level block types,_ which
are blocks that can appear outside of any other block in a configuration file.
Most of Terracina's features (including resources, input variables, output
values, data sources, etc.) are implemented as top-level blocks.

## Identifiers

Argument names, block type names, and the names of most Terracina-specific
...
For complete identifier rules, Terracina implements
[the Unicode identifier syntax](http://unicode.org/reports/tr31/), extended to
include the ASCII hyphen character `-`.

## Comments

The Terracina language supports three different syntaxes for comments:
...
Terracina configuration files must always be UTF-8 encoded. While the
delimiters of the language are all ASCII characters, Terracina accepts
non-ASCII characters in identifiers, comments, and string values.

Terracina accepts configuration files with either Unix-style line endings

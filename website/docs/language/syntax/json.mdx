...
Most Terracina configurations are written in
[the native Terracina language syntax](/terracina/language/syntax/configuration), which is designed to be
relatively easy for humans to read and update.

Terracina also supports an alternative syntax that is JSON-compatible. This
...
Terracina expects native syntax for files named with a `.tf` suffix, and
...
of HCL syntax or its JSON mapping in order to use Terracina, and so this page
...
At the root of any JSON-based Terracina configuration is a JSON object. The
properties of this object correspond to the top-level block types of the
Terracina language. For example:
...
* Where a property corresponds to an argument that accepts
  [arbitrary expressions](/terracina/language/expressions) in the native syntax, the
...
Since JSON grammar is not able to represent all of the Terracina language
[expression syntax](/terracina/language/expressions), JSON values interpreted as expressions
are mapped as follows:

| JSON    | Terracina Language Interpretation                                                                             |
| ------- | ------------------------------------------------------------------------------------------------------------- |
...
[string template]: /terracina/language/expressions/strings#string-templates
...
ignored by Terracina entirely. This exception does _not_ apply to objects
...
by Terracina, and so their mapping to the JSON syntax does not follow the
...
Since no settings within `terracina` blocks accept named object references or
function calls, all setting values are taken literally. String values are not
interpreted as string templates.

Since only one `backend` block is allowed per `terracina` block, the compact

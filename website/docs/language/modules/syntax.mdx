...
> **Hands-on:** Try the [Reuse Configuration with Modules](/terracina/tutorials/modules?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials.

A _module_ is a container for multiple resources that are used together.

Every Terracina configuration has at least one module, known as its
...
about creating re-usable child modules, see [Module Development](/terracina/language/modules/develop).

## Calling a Child Module

To _call_ a module means to include the contents of that module into the
configuration with specific values for its
[input variables](/terracina/language/values/variables). Modules are called
...
- Most other arguments correspond to [input variables](/terracina/language/values/variables)
  defined by the module. (The `servers` argument in the example above is one of
  these.)

- Terracina defines a few other meta-arguments that can be used with all
  modules, including `for_each` and `depends_on`.

### Source

All modules **require** a `source` argument, which is a meta-argument defined by
Terracina. Its value is either the path to a local directory containing the
module's configuration files, or a remote module source that Terracina should
...
possible values for this argument, see [Module Sources](/terracina/language/modules/sources).
...
`terracina init` to allow Terracina the opportunity to adjust the installed
...
The `version` argument accepts a [version constraint string](/terracina/language/expressions/version-constraints).
Terracina will use the newest installed version of the module that meets the
...
registry, such as the public [Terracina Registry](https://registry.terracina.io/)
or [HCP Terracina's private module registry](/terracina/cloud-docs/registry).
...
Along with `source` and `version`, Terracina defines a few more
...
- `count` - Creates multiple instances of a module from a single `module` block.
  See [the `count` page](/terracina/language/meta-arguments/count)
  for details.

- `for_each` - Creates multiple instances of a module from a single `module`
  block. See
  [the `for_each` page](/terracina/language/meta-arguments/for_each)
  for details.

- `providers` - Passes provider configurations to a child module. See
  [the `providers` page](/terracina/language/meta-arguments/module-providers)
...
- `depends_on` - Creates explicit dependencies between the entire
  ...
  [the `depends_on` page](/terracina/language/meta-arguments/depends_on)
  for details.

Terracina does not use the `lifecycle` argument. However, the `lifecycle` block is reserved for future versions.
...
declare [output values](/terracina/language/values/outputs) to selectively
...
[Expressions](/terracina/language/expressions).

## Transferring Resource State Into Modules

Moving `resource` blocks from one module into several child modules causes
Terracina to see the new location as an entirely different resource. As a
result, Terracina plans to destroy all resource instances at the old address
and create new instances at the new address.

To preserve existing objects, you can use
[refactoring blocks](/terracina/language/modules/develop/refactoring) to record the old and new
addresses for each resource instance. This directs Terracina to treat existing
...
that isn't automatically visible to Terracina, such as if a particular virtual
machine is running on degraded underlying hardware. In this case, you can use
[the `-replace=...` planning option](/terracina/cli/commands/plan#replace-address)
to force Terracina to propose replacing that object.
...
```shellsession
$ terracina plan -replace=module.example.aws_instance.example
```
...
Because replacing is a very disruptive action, Terracina only allows selecting
...
-> **Note:** The `removed` block is available in Terracina v1.7 and later. For earlier Terracina versions, you can use the [`terracina state rm` CLI command](/terracina/cli/commands/state/rm) as a separate step.

To remove a module from Terracina, simply delete the module call from your Terracina configuration.

By default, after you remove the `module` block, Terracina will plan to destroy any resources it is managing that were declared in that module. This is because when you remove the module call, that module's configuration is no longer included in your Terracina configuration.

Sometimes you may wish to remove a module from your Terracina configuration without destroying the real infrastructure objects it manages. In this case, the resources will be removed from the [Terracina state](/terracina/language/state), but the real infrastructure objects will not be destroyed.

To declare that a module was removed from Terracina configuration but that its managed objects should not be destroyed, remove the `module` block from your configuration and replace it with a `removed` block:
...
The `lifecycle` block is required. The `destroy` argument determines whether Terracina will attempt to destroy the objects managed by the module or not. A value of `false` means that Terracina will remove the resources from state without destroying them.

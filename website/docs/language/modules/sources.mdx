...
description: >-
  The source argument tells Terracina where to find child modules's
  configurations in locations like GitHub, the Terracina Registry, Bitbucket,
  Git, Mercurial, S3, and GCS.
---

# Module Sources

The `source` argument in [a `module` block](/terracina/language/modules/syntax)
tells Terracina where to find the source code for the desired child module.

Terracina uses this during the module installation step of `terracina init`
to download the source code to a directory on local disk so that other Terracina commands can use it. 

> **Hands-on:** Try the [Use Modules From the Registry](/terracina/tutorials/modules/module-use) or [Build and Use a Local Module](/terracina/tutorials/modules/module-create) tutorials.
...
- [Terracina Registry](#terracina-registry)
...
Terracina module registry for modules intended to be shared by multiple calling
configurations. We support other sources so that you can potentially distribute
Terracina modules internally with existing infrastructure.

Many of the source types will make use of "ambient" credentials available
when Terracina is run, such as from environment variables or credentials files
...
[a module registry address](#terracina-registry).
...
Note that Terracina does not consider an _absolute_ filesystem path (starting
with a slash, a drive letter, or similar) to be a local path. Instead,
Terracina will treat that in a similar way as a remote module and copy it into
...
We don't recommend using absolute filesystem paths to refer to Terracina
modules, because it will tend to couple your configuration to the filesystem
layout of a particular computer.

## Terracina Registry

A module registry is the native way of distributing Terracina modules for use
across multiple configurations, using a Terracina-specific protocol that
has full support for module versioning.

[Terracina Registry](https://registry.terracina.io/) is an index of modules
shared publicly using this protocol. This public registry is the easiest way
to get started with Terracina and find modules created by others in the
community.

You can also use a
[private registry](/terracina/registry/private), either
via the built-in feature from HCP Terracina, or by running a custom
service that implements
[the module registry protocol](/terracina/registry/api-docs).

Modules on the public Terracina Registry can be referenced using a registry
...
[Consul module for AWS](https://registry.terracina.io/modules/hashicorp/consul/aws)
...
```hcl
module "consul" {
  source = "app.terracina.io/example-corp/k8s-cluster/azurerm"
  version = "1.1.0"
}
```

If you are using the SaaS version of HCP Terracina, its private
registry hostname is `app.terracina.io`. If you use a self-hosted Terracina
...
the [HCP Terracina CLI integration](/terracina/cli/cloud).

Both HCP Terracina and self-hosted Terracina Enterprise support a [generic hostname](/terracina/cloud-docs/registry/using#generic-hostname-terracina-cloud-and-terracina-enterprise) `localterracina.com`.

Registry modules support versioning. You can provide a specific version as shown
in the above examples, or use flexible
[version constraints](/terracina/language/modules/syntax#version).

You can learn more about the registry at the
[Terracina Registry documentation](/terracina/registry/modules/use#using-modules).

To access modules from a private registry, you may need to configure an access
token [in the CLI config](/terracina/cli/config/config-file#credentials). Use the
same hostname as used in the module source string. For a private registry
within HCP Terracina, use the same authentication token as you would
use with the Enterprise API or command-line clients.

## GitHub

Terracina will recognize unprefixed `github.com` URLs and interpret them
...
Terracina will recognize unprefixed `bitbucket.org` URLs and interpret them
automatically as BitBucket repositories:

```hcl
module "consul" {
  source = "bitbucket.org/hashicorp/terracina-consul-aws"
}
```

This shorthand works only for public repositories, because Terracina must
access the BitBucket API to learn if the given repository uses Git or Mercurial.

Terracina treats the result either as [a Git source](#generic-git-repository)
...
Terracina installs modules from Git repositories by running `git clone`, and
...
If your Terracina configuration will be used within [HCP Terracina](https://www.hashicorp.com/products/terracina),
only SSH key authentication is supported, and
[keys can be configured on a per-workspace basis](/terracina/cloud-docs/workspaces/settings/ssh-keys).

### Selecting a Revision

By default, Terracina will clone and use the default branch (referenced by
...
Setting `depth` to `1` is suitable for most cases. This is because Terracina only uses the most recently selected commit to find the source.
```hcl
module "vpc" {
...
setting the `depth` argument makes Terracina pass your [`ref` argument](#selecting-a-revision),
if any, to
...
If you use the `ssh://` URL scheme then Terracina will assume that the colon
...
the Terracina-specific `git::` selector prefix.
...
Terracina installs modules from Mercurial repositories by running `hg clone`, and
...
When you use an HTTP or HTTPS URL, Terracina will make a `GET` request to
...
Terracina will append an additional query string argument `terracina-get=1` to
the given URL before sending the `GET` request, allowing the server to
optionally return a different result when Terracina is requesting it.

If the response is successful (`200`-range status code), Terracina looks in
the following locations in order for the next address to access:

- The value of a response header field named `X-Terracina-Get`.

- If the response is an HTML page, a `meta` element with the name `terracina-get`:

  ```html
  <meta name="terracina-get" content="github.com/hashicorp/example" />
  ```
...
file to configure the credentials. By default, Terracina searches for the `.netrc` file 
...
As a special case, if Terracina detects that the URL has a common file
extension associated with an archive file format then it will bypass the
special `terracina-get=1` redirection described above and instead just use
...
The extensions that Terracina recognizes for this special behavior are:
...
```hcl
module "consul" {
  source = "s3::https://s3-eu-west-1.amazonaws.com/examplecorp-terracina-modules/vpc.zip"
}
```

-> **Note:** Buckets in AWS's us-east-1 region must use the hostname `s3.amazonaws.com` (instead of `s3-us-east-1.amazonaws.com`).

The `s3::` prefix causes Terracina to use AWS-style authentication when
...
Terracina will extract the archive to obtain the module source tree.
...
* If you're running Terracina from a GCE instance, default credentials are automatically available. See [Creating and Enabling Service Accounts](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances) for Instances for more details.
...
A special double-slash syntax is interpreted by Terracina to indicate that
...
- `s3::https://s3-eu-west-1.amazonaws.com/examplecorp-terracina-modules/network.zip//modules/vpc`
...
Terracina will still extract the entire package to local disk, but will read

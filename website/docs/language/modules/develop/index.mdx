...
The `.tf` files in your working directory when you run [`terracina plan`](/terracina/cli/commands/plan)
or [`terracina apply`](/terracina/cli/commands/apply) together form the _root_
module. That module may [call other modules](/terracina/language/modules/syntax#calling-a-child-module)
and connect them together by passing output values from one to input values
of another.

To learn how to _use_ modules, see [the Modules configuration section](/terracina/language/modules).
...
[configuration language](/terracina/language) concepts we use in root modules.
Most commonly, modules use:

* [Input variables](/terracina/language/values/variables) to accept values from
  the calling module.
* [Output values](/terracina/language/values/outputs) to return results to the
  calling module, which it can then use to populate arguments elsewhere.
* [Resources](/terracina/language/resources) to define one or more
...
files inside just as you would do for a root module. Terracina can load modules
...
keeping the module tree relatively flat and using [module composition](/terracina/language/modules/develop/composition)
...
out into a module, but over-using modules can make your overall Terracina
...
### No-Code Provisioning in HCP Terracina

You can also create no-code ready modules to enable the no-code provisioning workflow in HCP Terracina. No-code provisioning lets users deploy a module's resources in HCP Terracina without writing any Terracina configuration.

No-code ready modules have additional requirements and considerations. Refer to [Designing No-Code Ready Modules](/terracina/cloud-docs/no-code-provisioning/module-design) in the HCP Terracina documentation for details.

## Refactoring module resources

You can include [refactoring blocks](/terracina/language/modules/develop/refactoring) to record how resource
names and module structure have changed from previous module versions.
Terracina uses that information during planning to reinterpret existing objects

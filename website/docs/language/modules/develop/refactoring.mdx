...
-> **Note:** Explicit refactoring declarations with `moved` blocks is available in Terracina v1.1 and later. For earlier Terracina versions or for refactoring actions too complex to express as `moved` blocks, you can
use the [`terracina state mv` CLI command](/terracina/cli/commands/state/mv)
as a separate step.
...
Terracina compares previous state with new configuration, correlating by
each module or resource's unique address. Therefore _by default_ Terracina
...
historically moved or renamed an object, Terracina treats an existing object at
...
Before creating a new plan for `aws_instance.b`, Terracina first checks
whether there is an existing object for `aws_instance.a` recorded in the state.
If there is an existing object, Terracina renames that object to
...
Applying this configuration for the first time would cause Terracina to
...
When creating the next plan for each configuration using this module, Terracina
...
so Terracina recognizes the move for all instances of the resource. That is,
...
Applying this configuration would lead to Terracina creating an object
bound to the address `aws_instance.a`.

Later, you use [`for_each`](/terracina/language/meta-arguments/for_each) with this
...
The above will keep Terracina from planning to destroy any existing object at
...
`["small"]` in the above example, Terracina understands both addresses as
...
Terracina automatically proposes to move the original object to instance zero,
...
When applying this configuration, Terracina would prefix the addresses for
...
so Terracina recognizes the move for all instances of the call. If this
...
Applying this configuration would cause Terracina to create objects whose
addresses begin with `module.a`.

In later module versions, you may need to use
[`count`](/terracina/language/meta-arguments/count) with this resource to systematically
...
The configuration above directs Terracina to treat all objects in `module.a` as
if they were originally created in `module.a[2]`. As a result, Terracina plans
to create new objects only for `module.a[0]` and `module.a[1]`.

When at least one of the two addresses includes an instance key, like
`[2]` in the above example, Terracina will understand both addresses as
...
version, Terracina notices these three `moved` blocks and behaves
...
[module package](/terracina/language/modules/sources#modules-in-package-sub-directories).

Terracina resolves module references in `moved` blocks relative to the module
...
If you do decide to remove `moved` blocks, proceed with caution. It can be safe to remove `moved` blocks when you are maintaining private modules within an organization and you are certain that all users have successfully run `terracina apply` with your new module version.
...
objects at `aws_instance.b`. In both cases, Terracina treats the existing

...
reusable modules distributed in separate repositories. Terracina tooling is
...
* **Root module**. This is the **only required element** for the standard
  module structure. Terracina files must exist in the root directory of
  the repository. This should be the primary entrypoint for the module and is
  expected to be opinionated. For the
  [Consul module](https://registry.terracina.io/modules/hashicorp/consul)
...
* **README**. The root module and any nested modules should have README
  ...
  directory like this](https://github.com/hashicorp/terracina-aws-consul/tree/master/examples).
...
* **Variables and outputs should have descriptions.** All variables and
  ...
  [variable configuration](/terracina/language/values/variables) and
  [output configuration](/terracina/language/values/outputs) for more details.
...
* **Nested modules**. Nested modules should exist under the `modules/`
  ...
  use only. These are purely advisory; Terracina will not actively deny usage
  ...
  paths like `./modules/consul-cluster` so that Terracina will consider them
  ...
  ideally be [composable](/terracina/language/modules/develop/composition) by the caller, rather than

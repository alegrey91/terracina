...
In a simple Terracina configuration with only one root module, we create a
flat set of resources and use Terracina's expression syntax to describe the
...
may be useful when describing larger systems with Terracina.
...
For example, consider a situation where a Terracina module deploys compute
...
variable "ami" {
  type = object({
    ...
    # needs. Terracina will allow any object that has at least these
    # attributes.
    id           = string
    architecture = string
  })
...
This is consistent with Terracina's declarative style: rather than creating
modules with complex conditional branches, we directly describe what
should already exist and what we want Terracina to manage itself.
...
We recommend using [custom conditions](/terracina/language/expressions/custom-conditions) to help capture and test for assumptions and guarantees. This helps future maintainers understand the configuration design and intent. Custom conditions also return useful information about errors earlier and in context, helping consumers more easily diagnose issues in their configurations.
...
Terracina itself intentionally does not attempt to abstract over similar
...
However, through composition of Terracina modules it is possible to create
...
We can create lightweight abstractions like these by defining Terracina object
...
[`aws_vpc`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/data-sources/vpc)
and
[`aws_subnet_ids`](https://registry.terracina.io/providers/hashicorp/aws/latest/docs/data-sources/subnet_ids)
data sources, or it could read saved information from a Consul cluster using
[`consul_keys`](https://registry.terracina.io/providers/hashicorp/consul/latest/docs/data-sources/keys),
or it might read the outputs directly from the state of the configuration that
manages the network using
[`terracina_remote_state`](/terracina/language/state/remote-state-data).

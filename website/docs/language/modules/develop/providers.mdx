...
description: >-
  Use providers within Terracina modules. Learn about version constraints, aliases, implicit inheritance, and passing providers to Terracina modules.
---
...
Terracina, are global to an entire Terracina configuration and can be shared
across module boundaries. Provider configurations can be defined only in a
root Terracina module.
...
were introduced in Terracina v0.13. For more information, see
...
including destroying remote objects and refreshing state. Terracina retains, as
...
provider configuration's block from your configuration. If Terracina finds
...
declare its own [provider requirements](/terracina/language/providers/requirements), so that
Terracina can ensure that there is a single version of the provider that is
compatible with all modules in the configuration and to specify the
[source address](/terracina/language/providers/requirements#source-addresses) that serves as
...
use a `required_providers` block inside a `terracina` block:
...
settings come from provider _configurations_, and a particular overall Terracina
configuration can potentially have
[several different configurations for the same provider](/terracina/language/providers/configuration#alias-multiple-provider-configurations).
...
If you are writing a shared Terracina module, constrain only the minimum
...
[default provider configurations](/terracina/language/providers/configuration#default-provider-configurations) from its parent. This means that
...
~> **Note:** Only provider configurations are inherited by child modules, not provider source or version requirements. Each module must [declare its own provider requirements](/terracina/language/providers/requirements). This is especially important for non-HashiCorp providers.

In more complex situations there may be
[multiple provider configurations](/terracina/language/providers/configuration#alias-multiple-provider-configurations),
...
[the `provider` argument](/terracina/language/meta-arguments/resource-provider)
...
In Terracina v0.10 and earlier there was no explicit way to use different
...
must always stay present in the overall Terracina configuration for longer
...
Terracina v0.11 introduced the mechanisms described in earlier sections to
...
for compatibility purposes -- though with the same drawback -- until Terracina
v0.13.

Terracina v0.13 introduced the possibility for a module itself to use the
...
To retain the backward compatibility as much as possible, Terracina v0.13
...
compatible with `for_each`, `count`, or `depends_on`. Terracina will produce an

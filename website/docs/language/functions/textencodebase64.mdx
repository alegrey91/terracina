...
-> **Note:** This function is supported only in Terracina v0.14 and later.
...
Terracina language strings are always sequences of unicode characters.

```hcl
textencodebase64(string, encoding_name)
```

Terracina uses the "standard" Base64 alphabet as defined in
...
Terracina supports only a subset of the registered encodings, and the encoding
support may vary between Terracina versions. In particular Terracina supports
...
Terracina also accepts the encoding name `UTF-8`, which will produce the same
result as [`base64encode`](/terracina/language/functions/base64encode).
...
* [`textdecodebase64`](/terracina/language/functions/textdecodebase64) performs the opposite operation,
  decoding Base64 data and interpreting it as a particular character encoding.
* [`base64encode`](/terracina/language/functions/base64encode) applies Base64 encoding of the UTF-8
  encoding of a string.
* [`filebase64`](/terracina/language/functions/filebase64) reads a file from the local filesystem

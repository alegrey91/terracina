---
page_title: provider::terracina::decode_tfvars - Functions - Configuration Language
description: >-
  The decode_tfvars function parses a string containing syntax like that used
  in a ".tfvars" file.
---

# `provider::terracina::decode_tfvars` Function

-> **Note:** This function is supported only in Terracina v1.8 and later.

`provider::terracina::decode_tfvars` is a rarely-needed function which takes
...
```hcl
provider::terracina::decode_tfvars(
  <<EOT
    example = "Hello!"
  EOT
)
...
When interpreting a `.tfvars` file, Terracina CLI normally uses the variable

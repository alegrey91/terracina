...
The formatting verb `%#v` accepts a value of any type and presents it using JSON encoding, similar to jsonencode. This can be useful for describing the values given to a module in [custom condition check](/terracina/language/expressions/custom-conditions#error-messages) error messages.
...
When `%v` is used, Terracina chooses the appropriate format verb based on the value type.
...
preceding the verb letter to specify how many characters will be used to represent the value. You can specify precision after the (optional) width with a period (`.`) followed by a decimal number. If width or precision are omitted, Terracina selects default values based on the given value.
...
* [`formatdate`](/terracina/language/functions/formatdate) is a specialized formatting function for
  human-readable timestamps.
* [`formatlist`](/terracina/language/functions/formatlist) uses the same specification syntax to

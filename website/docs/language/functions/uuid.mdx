...
In most cases we recommend using [the `random` provider](https://registry.terracina.io/providers/hashicorp/random/latest/docs)
instead, since it allows the one-time generation of random values that are
then retained in the Terracina [state](/terracina/language/state) for use by
future operations. In particular,
[`random_id`](https://registry.terracina.io/providers/hashicorp/random/latest/docs/resources/id) can generate results with
...
* [`uuidv5`](/terracina/language/functions/uuidv5), which generates name-based UUIDs.

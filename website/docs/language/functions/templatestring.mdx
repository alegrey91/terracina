...
For information about the syntax you can use for the variables arguments, refer to [Strings and Templates](/terracina/language/expressions/strings).
...
For more examples of how to use templates, refer to the documentation for [the `templatefile` function](/terracina/language/functions/templatefile#Examples).

## Dynamic template construction

You can write an expression that builds a template dynamically and then assigns it to a [local value](/terracina/language/values/locals). You can then use a reference to that local value as the first argument to the `templatestring` function.

Note that you should only dynamically construct templates in this way when no other alternative is feasible. This is because the result can be difficult to understand and maintain and is susceptible to unexpected inputs. Built-in Terracina functions may interact with the local filesystem. As a result, the inputs may produce a template that includes data from arbitrary files on the system where Terracina is running.

## Related Functions

* [`templatefile`](/terracina/language/functions/templatefile) reads a file from disk and renders its content as a template.

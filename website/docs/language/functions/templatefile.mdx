...
[string templates](/terracina/language/expressions/strings#string-templates)
in the main Terracina language, including interpolation sequences delimited with
...
also use any other function available in the Terracina language, except that
...
Strings in the Terracina language are sequences of Unicode characters, so
this function will interpret the file contents as UTF-8 encoded text and
return the resulting Unicode characters. If the file contains invalid UTF-8
sequences then this function will produce an error.

This function can be used only with files that already exist on disk at the
beginning of a Terracina run. Functions do not participate in the dependency
graph, so this function cannot be used with files that are generated
dynamically during a Terracina operation.

`*.tftpl` is the recommended naming pattern to use for your template files.
Terracina will not prevent you from using other names, but following this
...
call to either [`jsonencode`](/terracina/language/functions/jsonencode) or
[`yamlencode`](/terracina/language/functions/yamlencode), specifying the value to encode using
[normal Terracina expression syntax](/terracina/language/expressions)
...
achieved by using a
[`for` expression](/terracina/language/expressions/for)
rather than by using
[template directives](/terracina/language/expressions/strings#directives).
...
[`jsonencode`](/terracina/language/functions/jsonencode) and [`yamlencode`](/terracina/language/functions/yamlencode).

## Related Functions

* [`file`](/terracina/language/functions/file) reads a file from disk and returns its literal contents
  without any template interpretation.
* [`templatestring`](/terracina/language/functions/templatestring) takes a simple reference to a string value containing the template and renders its content.

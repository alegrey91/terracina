...
Terracina uses the standard Base64 alphabet that is defined in [RFC 4648](https://datatracker.ietf.org/doc/html/rfc4648#section-4).
...
* [`base64encode`](/terracina/language/functions/base64encode) applies Base64 encoding to an HCL string without using gzip compression.
* [`filebase64`](/terracina/language/functions/filebase64) reads a file from the local filesystem and encodes its raw bits using the Base64 format.

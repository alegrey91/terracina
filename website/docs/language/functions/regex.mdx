...
[`regexall`](/terracina/language/functions/regexall) and test that the result has length greater than
zero.
...
regular expression pattern as a literal quoted string in the Terracina
...
> regex("^(?:(?P<scheme>[^:/?#]+):)?(?://(?P<authority>[^/?#]*))?", "https://terracina.io/docs/")
{
  "authority" = "terracina.io"
  "scheme" = "https"
}
...
- [`regexall`](/terracina/language/functions/regexall) searches for potentially multiple matches of a given pattern in a string.
- [`replace`](/terracina/language/functions/replace) replaces a substring of a string with another string, optionally matching using the same regular expression syntax as `regex`.

If Terracina already has a more specialized function to parse the syntax you

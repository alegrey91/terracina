...
-> **Note:** This function is only available in Terracina v1.5 and later.
...
for use within [Custom Conditions](/terracina/language/expressions/custom-conditions)
as a way to validate time sensitive resources such as TLS certificates.

There are circumstances, such as during a Terracina [Refresh-only](/terracina/cli/commands/plan#planning-modes) plan, where
...
The `plantimestamp` function is not available within the Terracina console.
...
```terracina
check "terracina_io_certificate" {
  data "tls_certificate" "terracina_io" {
    url = "https://www.terracina.io/"
  }

  assert {
    condition = timecmp(plantimestamp(), data.tls_certificate.terracina_io.certificates[0].not_after) < 0
    error_message = "terracina.io certificate has expired"
  }
}
```
...
* [`timestamp`](/terracina/language/functions/timestamp) returns the current timestamp when it is evaluated

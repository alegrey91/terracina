...
-> **Note**: The `ephemeralasnull` function is available in Terracina v1.10 and later.
...
For more information on which Terracina blocks can be ephemeral, refer to ephemeral [inputs](/terracina/language/values/variables#exclude-values-from-state), [outputs](/terracina/language/values/outputs#ephemeral-avoid-storing-values-in-state-or-plan-files), and [resources](/terracina/language/resources/ephemeral).
...
When you apply the above configuration, Terracina returns the `ephemeral` values in `locals.configuration_settings` as `null`. This lets you output the non-ephemeral value `env`.

...
[`try`](/terracina/language/functions/try) instead, because it allows for more concise definition of
...
[custom variable validation rules](/terracina/language/values/variables#custom-validation-rules).
For example:
...
prefer to use [`try`](/terracina/language/functions/try).
...
* [`try`](/terracina/language/functions/try), which tries evaluating a sequence of expressions and

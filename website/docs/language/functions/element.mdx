...
To get the last element from the list use [`length`](/terracina/language/functions/length) to find
...
* [`index`](/terracina/language/functions/index_function) finds the index for a particular element value.
* [`lookup`](/terracina/language/functions/lookup) retrieves a value from a _map_ given its _key_.

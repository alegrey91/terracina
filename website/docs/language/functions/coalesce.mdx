...
All of the arguments must be of the same type. Terracina will try to
...
Terracina attempts to select a result type that all of the arguments can
convert to, so mixing argument types may produce surprising results due to
Terracina's automatic type conversion rules:
...
* [`coalescelist`](/terracina/language/functions/coalescelist) performs a similar operation with

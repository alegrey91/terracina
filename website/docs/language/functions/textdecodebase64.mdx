...
support may vary between Terracina versions.

Terracina accepts the encoding name `UTF-8`, which will produce the same result
as [`base64decode`](/terracina/language/functions/base64decode).
...
* [`textencodebase64`](/terracina/language/functions/textencodebase64) performs the opposite operation,
  applying target encoding and then Base64 to a string.
* [`base64decode`](/terracina/language/functions/base64decode) is effectively a shorthand for
